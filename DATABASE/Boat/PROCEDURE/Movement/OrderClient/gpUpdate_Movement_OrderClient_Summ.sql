...
CREATE OR REPLACE FUNCTION gpUpdate_Movement_OrderClient_Summ(
 ...
 INOUT ioSummTax                     TFloat,  -- Скидка (ввод), без НДС
 ...
 INOUT ioBasis_summ_transport        TFloat,  -- Total LP - ИТОГО Сумма продажи без НДС - со ВСЕМИ Скидками (Basis+options) + TRANSPORT
 INOUT ioBasisWVAT_summ_transport    TFloat,  -- Total LP + Vat - ИТОГО Сумма продажи с НДС - со ВСЕМИ Скидками (Basis+options) + TRANSPORT
...
BEGIN
     ...
     IF COALESCE (inId, 0) = 0
        AND ioSummTax = 0 AND ioSummReal = 0 AND inVATPercent = 0 AND inDiscountTax = 0 AND inDiscountNextTax = 0 AND inTransportSumm_load = 0 AND inTransportSumm = 0
        AND inBasis_summ1_orig = 0 AND inBasis_summ2_orig = 0 AND ioBasis_summ_transport = 0 AND ioBasisWVAT_summ_transport = 0 AND inAmountInBankAccountAll = 0
     THEN
         RETURN;
     END IF;
     ...
     IF COALESCE (inId, 0) = 0
     THEN
         RAISE EXCEPTION 'Ошибка.Документ Заказ клиента не сохранен.';
     END IF;
     ...
     outSummDiscount3 := COALESCE (inBasis_summ2_orig, 0)
                       - (WITH -- выбор данных по опциям
                          ...
                          -- с учетом скидки в самих опциях
                          SELECT SUM (CASE WHEN COALESCE (tmpProdOptItems.DiscountTax,0) <> 0
                                                THEN zfCalc_SummDiscountTax (tmpProdOptItems.Sale_summ, COALESCE (tmpProdOptItems.DiscountTax,0))
                                          ELSE zfCalc_SummDiscountTax (zfCalc_SummDiscountTax (tmpProdOptItems.Sale_summ, COALESCE (inDiscountTax,0))
                                                                     , COALESCE (inDiscountNextTax,0)
                                                                      )
                                      END)
     ...
     END IF;
         ...
         THEN
             ...
             -- Обнуляется Скидка (ввод), без НДС
             ioSummTax := 0;

             -- Total LP - ИТОГО Сумма продажи без НДС - со ВСЕМИ Скидками (Basis+options) + TRANSPORT
             ioBasis_summ_transport := outBasis_summ
                                       -- минус откорректированная скидка
                                     - ioSummTax
                                       -- плюс Транспорт
                                     + COALESCE (inTransportSumm_load, 0)
                                     + COALESCE (inTransportSumm, 0)
                                      ;
             -- Total LP + Vat - ИТОГО Сумма продажи с НДС - со ВСЕМИ Скидками (Basis+options) + TRANSPORT
         ...
         -- Total LP - ИТОГО Сумма продажи без НДС - со ВСЕМИ Скидками (Basis+options) + TRANSPORT
         ioBasis_summ_transport := COALESCE (outBasis_summ,0)
                                   -- минус откорректированная скидка
                                 - COALESCE (ioSummTax, 0)
                                   -- плюс Транспорт
                                 + COALESCE (inTransportSumm_load, 0)
                                 + COALESCE (inTransportSumm, 0)
                                  ;
         ...
         -- Долг ИТОГО - с НДС - для эдит формы лодки
         outAmountIn_remAll := (COALESCE (ioBasisWVAT_summ_transport,0) - COALESCE (inAmountInBankAccountAll,0));
