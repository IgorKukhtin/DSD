...
CREATE OR REPLACE FUNCTION gpInsertUpdate_Movement_BankAccount(
 INOUT ioId                     Integer   , -- Ключ объекта <Документ>
    ...
    IN inMovementId_OrderClient Integer   , -- Заказ Клиента - !!!для inMovementId_Invoice!!!
...
BEGIN
     ...
     -- проверка
     /*IF COALESCE (inMovementId_Invoice, 0) = 0 AND COALESCE (inMovementId_OrderClient, 0) = 0
     THEN
     ...
     IF (COALESCE (inMovementId_Invoice, 0) = 0
        ...
        -- или изменили Заказ
        OR NOT EXISTS (SELECT 1 FROM Movement WHERE  Movement.Id = inMovementId_Invoice AND Movement.ParentId = inMovementId_OrderClient)
       )
     ...
     THEN
         -- сохранили <Документ> + для inMovementId_OrderClient уствновится связь на этот inMovementId_Invoice
         inMovementId_Invoice := gpInsertUpdate_Movement_Invoice (ioId               := inMovementId_Invoice
                                                                , inParentId         := inMovementId_OrderClient                 ::Integer
                                                                , inInvNumber        := NEXTVAL ('movement_Invoice_seq')    ::TVarChar
                                                                , inOperDate         := inOperDate
                                                                , inPlanDate         := NULL                                ::TDateTime
                                                                  -- это св-во в Заказе
                                                                , inVATPercent       := COALESCE (-- значение в Счете
                                                                                                  (SELECT MF.ValueData FROM MovementFloat AS MF WHERE MF.MovementId = inMovementId_Invoice AND MF.DescId = zc_MovementFloat_VATPercent())
                                                                                                  -- значение в Заказе
