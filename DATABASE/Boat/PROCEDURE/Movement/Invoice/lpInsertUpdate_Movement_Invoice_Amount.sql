...
BEGIN
    ...
    outAmount := (SELECT zfCalc_SummWVAT (tmp.Summа_WVAT, tmp.VATPercent)
                  FROM (SELECT SUM (COALESCE (MovementItem.Amount, 0) * COALESCE (MIFloat_OperPrice.ValueData, 0)) AS Summа_WVAT
