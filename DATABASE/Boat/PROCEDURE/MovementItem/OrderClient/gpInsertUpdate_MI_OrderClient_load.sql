     ...
     THEN
         ...
         ioProductId:= (SELECT tmp.ioId
                        FROM gpInsertUpdate_Object_Product
                                              ...
                                              , inTransportSumm         := COALESCE ((SELECT MF.ValueData FROM MovementFloat AS MF WHERE MF.MovementId = ioMovementId_OrderClient AND MF.DescId = zc_MovementFloat_TransportSumm()), 0)
                                              , inDateStart             := NULL
         ...
         ioMovementId_OrderClient:= (SELECT tmp.ioId
                                     FROM lpInsertUpdate_Movement_OrderClient (ioId                 := ioMovementId_OrderClient
                                                                             ...
                                                                             , inTaxKindId          := COALESCE ((SELECT MLO.ObjectId FROM MovementLinkObject AS MLO WHERE MLO.DescId = zc_MovementLinkObject_TaxKind() AND MLO.MovementId = ioMovementId_OrderClient), zc_Enum_TaxKind_Basis())
