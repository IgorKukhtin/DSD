...
BEGIN
  ...
  -- проверка
  IF inId > 0 AND NOT EXISTS (SELECT 1 FROM Object WHERE Object.Id = inId AND Object.DescId = zc_Object_Client())
  THEN
      RAISE EXCEPTION 'Ошибка.Выбран <%>.Для этого действия неообходимо выбрать Клиента.', (SELECT ObjectDesc.ItemName FROM Object JOIN ObjectDesc ON ObjectDesc.Id = Object.DescId WHERE Object.Id = inId);
  END IF;
