-- Function: lpSelect_Object_ReceiptProdModelChild_Levels()

DROP FUNCTION IF EXISTS lpSelect_Object_ReceiptProdModelChild_Levels (Integer);

CREATE OR REPLACE FUNCTION lpSelect_Object_ReceiptProdModelChild_Levels(
...
BEGIN
     ...
     CREATE TEMP TABLE _tmpReceiptProdModel_Child (ModelId Integer
                                                   -- Уровень
                                                 , Levels Integer
                                                   -- Принадлежность
                                                 , Parent TVarChar
     ...
     WITH
        ...
        INSERT INTO _tmpReceiptProdModel_Child
        SELECT -- Модель лодки у шаблона ProdModel
        ...
        WHILE EXISTS(SELECT 1
                     FROM _tmpReceiptProdModel_Child 
                         -- нашли его в сборке узлов
                         INNER JOIN ObjectLink AS ObjectLink_ReceiptGoods_Object
                                               ON ObjectLink_ReceiptGoods_Object.ChildObjectId = _tmpReceiptProdModel_Child.ObjectId
                     ...
                     WHERE _tmpReceiptProdModel_Child.Levels = vbLevels)
        LOOP
        
         WITH
             tmpReceiptGoodsChild AS (SELECT tmpReceiptProdModelChild.ModelId                  AS ModelId
                                             -- признак главный Yes/no шаблон ProdModel
                                           , tmpReceiptProdModelChild.Parent                   AS Parent
                                      ...
                                      FROM _tmpReceiptProdModel_Child  AS tmpReceiptProdModelChild
                                           -- нашли его в сборке узлов
                                           INNER JOIN ObjectLink AS ObjectLink_ReceiptGoods_Object
                                       ...
                                       WHERE tmpReceiptProdModelChild.Levels = vbLevels
                                     )
          ...
          SELECT tmpReceiptGoodsChild.ModelId
               ...
               , vbLevels + 1
               -- Принадлежность
               , (CASE WHEN vbLevels > 0 THEN tmpReceiptGoodsChild.Parent ELSE '' END||tmpReceiptGoodsChild.ObjectId_parent)::TVarChar
     ...
     WITH
       ...
       FROM _tmpReceiptProdModel_Child AS tmpRes
            -- Boat Structure
...
-- тест
-- 
SELECT * FROM lpSelect_Object_ReceiptProdModelChild_Levels (zfCalc_UserAdmin() :: Integer) AS lpSelect
WHERE lpSelect.ReceiptProdModelId = 39753