...
BEGIN
     ...
     THEN
         RAISE EXCEPTION 'Ошибка1.1.inId_site = <%> vbModelId = <%> vbMaterialOptionsId = <%> inOptName = <%>'
     ...
     THEN
         RAISE EXCEPTION 'Ошибка1.2.inId_site = <%> vbModelId = <%> vbMaterialOptionsId = <%> inOptName = <%>'
                       , inId_site, lfGet_Object_ValueData_sh (vbModelId), lfGet_Object_ValueData_sh (vbMaterialOptionsId), inOptName;
     END IF;
     -- Поиск - 1
