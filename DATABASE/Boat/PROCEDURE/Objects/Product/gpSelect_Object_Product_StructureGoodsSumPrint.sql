-- Function: gpSelect_Object_Product_StructureGoodsSumPrint ()

DROP FUNCTION IF EXISTS gpSelect_Object_Product_StructureGoodsSumPrint (Integer, TVarChar);
DROP FUNCTION IF EXISTS gpSelect_Object_Product_StructureGoodsSumPrint (Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Object_Product_StructureGoodsSumPrint(
    IN inMovementId_OrderClient       Integer   ,   --
    IN inisDetail                     Boolean   ,   --показывать все узлы
...
, tmpItem_Detail_mi AS (SELECT MovementItem.Id                           AS MovementItemId
   ...
   , tmpItem_Detail AS (-- уровень 3 - собираем Лодку
                        SELECT
                             ...
                             , tmpItem_Сhild.OperPrice
--, 0 AS OperPrice
                             , 0 AS GoodsId_basis
                             , 0 AS ReceiptLevelId
                             , 0 AS ProdColorPatternId
                             , tmpItem_Сhild.ProdOptionsId
                        FROM tmpItem_Сhild
                      --WHERE tmpItem_Сhild.ProdOptionsId = 0
                       ...
                       UNION ALL
                        ...
                        SELECT distinct
                             ...
                             , COALESCE (tmpItem_Detail.OperPrice,0) AS OperPrice
                             --, 0 AS OperPrice
                       ...
                       UNION ALL
                        ...
                        SELECT DISTINCT
                          ...
                          , CASE WHEN tmpItem_Detail.GoodsId_basis > 0 THEN 0 ELSE tmpItem_Detail.OperPrice     END AS OperPrice
--, 0 AS OperPrice
                             , 0 AS GoodsId_basis
                             , 0 AS ReceiptLevelId
      ...
      SELECT 
         ...
         --, (Object_dt.ValueData || ' '  || tmpItem_Detail.GoodsId_basis :: TVarChar) :: TVarChar AS ObjectName_dt
           ...
           , tmpItem_Detail.AmountPartner       :: TFloat AS Amount_partner_dt
             ...
             -- Цена вх без НДС
           , tmpItem_Detail.OperPrice ::TFloat
           ...
           , ROW_NUMBER() OVER (PARTITION BY tmpItem_Сhild.GroupId, tmpItem_Сhild.ObjectId
                                ORDER BY -- для сборки Hypalon
                                       ...
                                       , CASE WHEN EXISTS (SELECT 1 FROM tmpItem_Detail AS tmp WHERE tmp.GoodsId = tmpItem_Detail.ObjectId)
                                              ...
                                              ELSE 1
                                         END
                                         -- для сборки модели - Опции в конце
                                       , CASE WHEN tmpItem_Detail.ProdOptionsId > 0 AND tmpItem_Сhild.GroupId = 3
                                                   THEN 1
                                              ELSE 0
                                         END
      ...
      WHERE (tmpItem_Сhild.GroupId = 3 AND inisDetail = FALSE) OR inisDetail = TRUE
--ObjectDesc_dt.Id = zc_Object_Goods()
   --  AND inisDetail = TRUE
     ...
     /* UNION ALL  
      SELECT
           ...
           , Object_ch.Id                                 AS ObjectId_dt
           , Object_ch.ObjectCode             :: Integer  AS ObjectCode_dt
           ,  ObjectString_Article_ch.ValueData :: TVarChar AS Article_Object_dt
           , Object_ch.ValueData              :: TVarChar AS ObjectName_dt
           , '' :: TVarChar AS DescName_dt
           , Object_ProdColor_ch.ValueData                AS ProdColorName_dt
           ...
           , 0  :: TFloat   AS Amount_partner_ch
             ...
             -- Количество сборка узла
           , CAST (tmpItem_Сhild.Amount  AS NUMERIC(16, 8)) AS Amount_dt
             -- Количество
           ...
           , tmpItem_Сhild.OperPrice  :: TFloat   AS OperPrice
           ...
           , -2                    :: Integer AS NPP_1
           , ROW_NUMBER() OVER (ORDER BY CASE WHEN tmpReceiptLevel.ReceiptLevelName <> '' THEN 0 ELSE 1 END
                                       , tmpReceiptLevel.ReceiptLevelName
                                       , Object_ch.ValueData
           ...
           , ROW_NUMBER() OVER (ORDER BY CASE WHEN tmpReceiptLevel.ReceiptLevelName <> '' THEN 0 ELSE 1 END
                                       ...
                                       , CASE WHEN Object_ProdOptions.ValueData <> '' THEN 1 ELSE 0 END
                                       , Object_ProdOptions.ValueData
                               ) :: Integer AS NPP_pcp                              --tmpProdColorItems.Npp
       ...
       FROM tmpItem_Сhild
            ...
            LEFT JOIN tmpReceiptLevel ON tmpReceiptLevel.GoodsId = tmpItem_Сhild.ObjectId --CASE WHEN tmpItem_Сhild.ObjectId_basis > 0 THEN tmpItem_Сhild.ObjectId_basis ELSE tmpItem_Сhild.ObjectId END

            LEFT JOIN Object AS Object_ProdOptions ON Object_ProdOptions.Id = tmpItem_Сhild.ProdOptionsId
*/
       ORDER BY 1
              , 35
              , 36
...
-- тест
-- SELECT * FROM gpSelect_Object_Product_StructureGoodsSumPrint (inMovementId_OrderClient:= 662, inSession:= zfCalc_UserAdmin())

--SELECT * FROM gpSelect_Object_Product_StructureGoodsSumPrint (inMovementId_OrderClient:= 817, inisDetail:= TRUE, inSession:= zfCalc_UserAdmin())
