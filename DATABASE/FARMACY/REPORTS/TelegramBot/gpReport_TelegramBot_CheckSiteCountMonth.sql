     ...
     THEN
       SELECT SUM(Report.CountCurr), SUM(Report.SummCurr), SUM(Report.CountPrev), 
              string_agg(Report.CheckSourceKindName||' '||
                'Интернет заказов '||zfConvert_IntToString (COALESCE(Report.CountCurr, 0))||
                ' на сумму '||zfConvert_FloatToString (COALESCE(Report.SummCurr, 0)::TFloat)||
                ' грн. процент изменения '||CASE WHEN Report.CountPrev < Report.CountCurr THEN ' + ' ELSE '' END||
       ...
       RETURN QUERY
       SELECT 
        ('За '||zfCalc_MonthYearName(DATE_TRUNC ('month', DATE_TRUNC ('month', CURRENT_DATE) - INTERVAL '1 DAY'))||CHR(13)||'Интернет заказов '||zfConvert_IntToString (COALESCE(vbCountCurrM, 0))||
