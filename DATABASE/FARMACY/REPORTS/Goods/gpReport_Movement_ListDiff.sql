...
BEGIN
    ...
    ANALYSE tmpUnit;
     ...
     CREATE TEMP TABLE tmpData_MI ON COMMIT DROP AS
     WITH
       ...
       SELECT /*COALESCE (MIFloat_OrderId.ValueData       :: Integer AS MovementId_Order
    ...
    ANALYSE tmpData_MI;
     ...
     CREATE TEMP TABLE GoodsPromo ON COMMIT DROP AS
       SELECT DISTINCT ObjectLink_Child_retail.ChildObjectId AS GoodsId  -- здесь товар "сети"
    ...
    ANALYSE tmpCheck;
     ...
     CREATE TEMP TABLE tmpPriceRemains ON COMMIT DROP AS
     WITH
        ...
        , tmpOHFloat_End AS (SELECT ObjectHistoryFloat.*
         ...
         SELECT tmpRemains.GoodsId                                  AS GoodsId
...
-- тест
--
 select * from gpReport_Movement_ListDiff(inStartDate := ('01.05.2023')::TDateTime, inEndDate := ('12.05.2023')::TDateTime, inJuridicalId := 0, inUnitId := 0 , inRetailId:= 0 , inDiffKindId := 0, inisUnitList := TRUE ,  inSession := '3');