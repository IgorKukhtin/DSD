    ...
    , tmpPartionParam AS (SELECT tmpMIC_Info.MovementId
     ...
     , tmpMILinkObject AS (SELECT * FROM MovementItemLinkObject
        ...
        SELECT tmpData.MovementId
           ...
           , CASE WHEN tmpMovDetails.JuridicalId = 2886776  -- ПП ЛЕКОФАРМ
                  THEN 'UA083077700000026008711126473'  
                  WHEN tmpMovDetails.JuridicalId = 4721154  -- ТОВ АСНБ-2  
                  THEN 'UA093052990000026000050593496' 
                  WHEN tmpMovDetails.JuridicalId = 1311462  -- ТОВ АСНБ-4
                   AND Object_PartnerMedical.Id = 4474556   -- КНП  "ДЦПМСД №9" ДМР
                  THEN 'UA153052990000026007050594993' 
                  WHEN tmpMovDetails.JuridicalId = 3457711  -- ТОВ Орхідея-Фарм
                   AND Object_PartnerMedical.Id = 4474556   -- КНП  "ДЦПМСД №9" ДМР 
                  THEN 'UA533052990000026000050591443' 
                  WHEN tmpMovDetails.JuridicalId = 1311462  -- ТОВ АСНБ-4  
                   AND Object_PartnerMedical.Id = 4212299   -- КНП  "ДЦПМСД №11" ДМР
                  THEN 'UA153052990000026007050594993' 
                  WHEN tmpMovDetails.JuridicalId = 393038   -- ТОВ АСНБ
                   AND Object_PartnerMedical.Id = 4474509   -- КНП  "ДЦПМСД №8" ДМР
                  THEN 'UA093052990000026000050563426' 
                  WHEN tmpMovDetails.JuridicalId = 6608394  -- ТОВ ЮАФАРМ  
                   AND Object_PartnerMedical.Id = 3751525   -- КНП  "ДЦПМСД №5" ДМР
                  THEN 'UA493223130000026006000037643' 
                  WHEN tmpMovDetails.JuridicalId = 1311462  -- ТОВ АСНБ-4  
                   AND Object_PartnerMedical.Id = 3751525   -- КНП  "ДЦПМСД №5" ДМР
                  THEN 'UA153052990000026007050594993' 
                  WHEN tmpMovDetails.JuridicalId = 393052   -- ФОП Шапіро І.О.
                   AND Object_PartnerMedical.Id = 3751525   -- КНП  "ДЦПМСД №5" ДМР
           ...
           , CASE WHEN tmpMovDetails.JuridicalId = 2886776  -- ПП ЛЕКОФАРМ
                  THEN 'АТ "А_БАНК"'  
                  WHEN tmpMovDetails.JuridicalId = 472115   -- ТОВ АСНБ-2    
                  THEN 'АТ КБ "Приватбанк"' 
                  WHEN tmpMovDetails.JuridicalId = 1311462  -- ТОВ АСНБ-4
                   AND Object_PartnerMedical.Id = 4474556   -- КНП  "ДЦПМСД №9" ДМР 
                  THEN 'АТ КБ "Приватбанк"' 
                  WHEN tmpMovDetails.JuridicalId = 3457711  -- ТОВ Орхідея-Фарм 
