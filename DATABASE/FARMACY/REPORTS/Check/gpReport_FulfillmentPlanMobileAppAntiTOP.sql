...
BEGIN
     ...
     SELECT T1.UnitId, T1.UnitCode, T1.UnitName
          ...
          , T1.CountChechUser, T1.CountMobileUser, T1.CountMobileAll, T1.CountShortage, T1.QuantityMobile, T1.ProcFact 
          , CASE WHEN T1.Color_Calc = zfCalc_Color (255, 69, 0) 
                 THEN - vbAntiTOPMP_SumFine 
                 WHEN T1.Color_Calc = zfCalc_Color (173, 255, 47) 
                 THEN vbAntiTOPMP_SumFine 
                 END::TFloat
          , T1.isShowPlanMobileAppUser
