                 ...
                 UNION
                   SELECT  ObjectLink_Price_Unit.ChildObjectId      AS UnitId
                    ...
                    FROM ObjectLink AS ObjectLink_Price_Unit
                       ...
                       INNER JOIN ObjectFloat AS MCS_Value
...
--               , Object_Send.Amount_To     AS AmountSend_To
     ...
     -- !!!ResultTO!!!
  WITH tmpDataFrom AS (SELECT UnitId, GoodsId, RemainsMCS_from -- количество "излишков" в одной аптеке
                       FROM tmpData
   ...
   SELECT tmp.GoodsId
        , tmp.UnitId
        , SUM (tmp.RemainsMCS_result) AS RemainsMCS_result
   FROM 
...
-- тест
--SELECT * FROM gpReport_RemainsOverGoods_To (inUnitId:= 183292, inStartDate:= '12.01.2017' ::TDateTime, inPeriod:= 30::tfloat, inDay:= 28::tfloat,inAssortment:=1::tfloat, inisAssortment:=False, inSession:= '3'::TVarChar);  -- Аптека_1 пр_Правды_6
