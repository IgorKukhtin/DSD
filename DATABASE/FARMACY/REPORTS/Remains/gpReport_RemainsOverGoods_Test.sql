...
------------------------------------------------
       ...
       -- Goods_list - MCSValue
       UPDATE tmpGoods_list
              SET MCSValue = CASE WHEN inisSendAll = TRUE AND tmpGoods_list.UnitId = inUnitId THEN 0
                                  WHEN inisSendAll = TRUE AND tmpGoods_list.UnitId <> inUnitId THEN CEILING (COALESCE (tmp.MCSValue, 0))  -- округляем в большую сторону 
                                  ELSE COALESCE (tmp.MCSValue, 0)
                             END
       FROM tmpGoods_list AS tmp
       WHERE tmp.PriceId = tmpGoods_list.PriceId
         AND tmp.UnitId  = tmpGoods_list.UnitId
             ...
             SELECT
               ...
               , CASE WHEN ObjectBoolean_Goods_Close.ValueData = TRUE
                      ...
                      WHEN Object_Remains.RemainsStart > tmpGoods_list.MCSValue AND tmpGoods_list.MCSValue >= 0
                           THEN CASE WHEN inisSendAll = FALSE 
                                     ...
                                     ELSE  ((Object_Remains.RemainsStart - tmpGoods_list.MCSValue) / COALESCE (tmpOverSettings.MinimumLot, COALESCE (tmpOverSettings_all.MinimumLot, 1)))
                                             --*** ((Object_Remains.RemainsStart - tmpGoods_list.MCSValue) / COALESCE (tmpOverSettings.MinimumLot, COALESCE (tmpOverSettings_all.MinimumLot, 1)))
                                           * COALESCE (tmpOverSettings.MinimumLot, COALESCE (tmpOverSettings_all.MinimumLot, 1))
                                END
                      ELSE 0
               ...
               , CASE WHEN ObjectBoolean_Goods_Close.ValueData = TRUE
                      ...
                      WHEN COALESCE (Object_Remains.RemainsStart, 0) < tmpGoods_list.MCSValue AND tmpGoods_list.MCSValue > 0
                           THEN CEIL ((tmpGoods_list.MCSValue - FLOOR (COALESCE (Object_Remains.RemainsStart, 0))) / COALESCE (tmpOverSettings.MinimumLot, COALESCE (tmpOverSettings_all.MinimumLot, 1)))
