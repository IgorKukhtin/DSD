...
RETURNS TABLE (Id integer, GoodsCode Integer, GoodsName TVarChar, OperAmount TFloat, Price TFloat, OperSum TFloat)
AS
$BODY$
...
BEGIN
     ...
     WITH containerCount AS (SELECT container.Id, container.Amount, container.ObjectID FROM container  
       ...
       SELECT Object_Goods.Id
            ...
            , DD.OperAmount::TFloat
            , (DD.OperSum / DD.OperAmount)::TFloat
            , DD.OperSum::TFloat 
         FROM (
         SELECT SUM(DD.OperAmount) AS OperAmount, SUM(DD.OperSum) AS OperSum, ObjectId
           FROM
               (SELECT SUM(DD.OperAmount) AS OperAmount, 0 AS OperSum, ObjectID 
                  FROM
                ...
                LEFT JOIN MovementItemContainer AS MIContainer ON MIContainer.ContainerId = containerCount.Id
                 ...
                 GROUP BY containerCount.Id, containerCount.ObjectID, containerCount.Amount) AS DD
