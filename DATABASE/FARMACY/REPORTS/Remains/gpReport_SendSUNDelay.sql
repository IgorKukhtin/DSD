...
DROP FUNCTION IF EXISTS gpReport_SendSUNDelay (Integer, TVarChar);
...
BEGIN
    ...
    SELECT Object_From.ValueData                  AS FromName
         ...
         , (-1 * MovementItemContainer.Amount)::TFloat                                    AS AmountLoss
         , Round(-1 * MovementItemContainer.Amount * tmpObject_Price.Price, 2)::TFloat    AS SummaLoss
...
-- тест SELECT * FROM gpReport_SendSUNDelay (inUnitId := 0, inSession := '3')