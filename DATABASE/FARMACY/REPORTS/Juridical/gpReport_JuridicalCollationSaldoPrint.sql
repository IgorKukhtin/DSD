...
BEGIN
    ...
    OPEN Cursor1 FOR
      SELECT 'Акт сверки. Остатки на дату: '||to_char(DATE_TRUNC ('DAY', inStartDate), 'DD.MM.YYYY') as Title, 1 AS ID
      UNION ALL
    ...
    CREATE TEMP TABLE _tmpResult (Title TVarChar, Debet TVarChar, Kredit TVarChar, Summa TVarChar) ON COMMIT DROP;
      ...
      INSERT INTO _tmpResult VALUES ('Итого:', '=SUM(R[-1]C:R[-'||vbCount1::TVarChar||']C)', '=SUM(R[-1]C:R[-'||vbCount1::TVarChar||']C)', Null);

      -- Остаток товара

      INSERT INTO _tmpResult VALUES (NULL, NULL, NULL, NULL);
      INSERT INTO _tmpResult VALUES (NULL, NULL, NULL, NULL);
      INSERT INTO _tmpResult VALUES ('Остаток товара по виду НДС', 'БЕЗ НДС', 'НДС', 'с НДС');
      ...
      INSERT INTO _tmpResult VALUES ('Итого:', '=SUM(R[-1]C:R[-'||vbCount1::TVarChar||']C)', '=SUM(R[-1]C:R[-'||vbCount1::TVarChar||']C)', '=SUM(R[-1]C:R[-'||vbCount1::TVarChar||']C)');
...
-- SELECT * FROM gpReport_JuridicalCollationSaldoPrint ('01.01.2019'::TDateTime, 7433753, 'ООО_ИСТОК-Д', '3');