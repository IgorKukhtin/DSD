-- Function: gpSelect_Object_Price_Lite_Lite (TVarChar)
/*
  процедура так же вызывается в программе PROJECT\DSD\DPR\SavePriceToXls.dpr
  unit: SaveToXlsUnit
  row: 167 = qryPrice.SQL.Text := 'Select * from gpSelect_Object_Price_Lite_Lite...
  exe класть на //91.210.37.210:2511/d$/ (его запускает служба)
*/
DROP FUNCTION IF EXISTS gpSelect_Object_Price_Lite_Lite(Integer, Integer, Boolean,Boolean,TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Object_Price_Lite_Lite(
...
--ALTER FUNCTION gpSelect_Object_Price_Lite_Lite(Integer, Boolean,Boolean,TVarChar) OWNER TO postgres;
...
*/
-- тест
-- select * from gpSelect_Object_Price_Lite_Lite(inUnitId := 183292 , inStartDate := ('24.02.2016 17:24:00')::TDateTime , inisShowAll := 'False' , inisShowDel := 'False' ,  inSession := '3');
