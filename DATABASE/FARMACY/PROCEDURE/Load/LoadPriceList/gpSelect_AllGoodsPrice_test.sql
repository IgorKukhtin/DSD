...
BEGIN
  ...
  , tmpGoodsReprice AS (SELECT DISTINCT ResultSet.Id_retail AS GoodsId
    ...
    SELECT
        ...
        ResultSet.IsPromo,
        CASE /*-- GoodsReprice включено, переоценка в минус - блокируется
               По всем сетям запрещать переоценку в минус (если новая цена ниже чем старая), то есть пусть продается товар по старой партии со старой ценой
             , а потом после переоценки продается новая партия
             и при этом остаток >= 1
             */
             WHEN tmpGoodsReprice.GoodsId IS NOT NULL AND COALESCE (ResultSet.NewPrice,0) < COALESCE (ResultSet.LastPrice,0) AND COALESCE (ResultSet.RemainsCount,0) >=1
                THEN FALSE      
