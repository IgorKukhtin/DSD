...
--     vbUserId:= lpGetUserBySession (inSession);
     ...
     WITH
       tmpLoadPriceListItem AS (SELECT LoadPriceListItem.LoadPriceListId
                                     , COUNT (*)                                                                      AS Amount_all     -- всего позиций
                                     , SUM (CASE WHEN COALESCE (LoadPriceListItem.GoodsId, 0) <> 0 THEN 1 ELSE 0 END) AS Amount_tied    --привязанных
                                     , SUM (CASE WHEN COALESCE (LoadPriceListItem.GoodsId, 0) <> 0 THEN 0 ELSE 1 END) AS Amount_untied  -- не привязанных
