...
BEGIN
    ...
    -- Берем разницу с существующими контейнерами
     UPDATE _tmpItem_PartionDate SET Amount = _tmpItem_PartionDate.Amount - CPDContainer.Amount
     FROM (SELECT Container.Id,
                  Container.Amount
           FROM Container
           WHERE Container.DescId = zc_Container_CountPartionDate()
             AND Container.WhereObjectId = vbUnitId
             AND Container.Amount <> 0) as CPDContainer
     WHERE _tmpItem_PartionDate.ContainerId = CPDContainer.ID;
    ...
    INSERT INTO _tmpMIContainer_insert (DescId, MovementDescId, MovementId, MovementItemId, ContainerId, AccountId, Amount, OperDate
         ...
         WHERE _tmpItem_PartionDate.Amount <> 0;

    -- Результат - ЗАБАЛАНСОВЫЕ проводки по сумме "Остатки количественного учета (по дате партии)" добавляем чего нет в документе
    INSERT INTO _tmpMIContainer_insert (DescId, MovementDescId, MovementId, MovementItemId, ContainerId, AccountId, Amount, OperDate
                                      , ObjectId_analyzer, WhereObjectId_analyzer, ObjectIntId_analyzer
                                       )
         SELECT zc_MIContainer_CountPartionDate()   AS DescId
              ...
              , NULL                                AS AccountId
              , - Container.Amount                  AS OperSumm
              , vbOperDate                          AS OperDate
              , Container.ObjectId                  AS ObjectId_analyzer
              , vbUnitId                            AS WhereObjectId_analyzer
              , ContainerLinkObject.ObjectId   AS ObjectIntId_analyzer
         FROM Container
              LEFT JOIN _tmpItem_PartionDate ON _tmpItem_PartionDate.ContainerId = Container.Id
              LEFT JOIN ContainerLinkObject ON ContainerLinkObject.ContainerId = Container.Id
                                           AND ContainerLinkObject.DescId = zc_ContainerLinkObject_PartionGoods()
         WHERE Container.DescId = zc_Container_CountPartionDate()
           AND Container.WhereObjectId = vbUnitId
           AND COALESCE(_tmpItem_PartionDate.ContainerId, 0) = 0
