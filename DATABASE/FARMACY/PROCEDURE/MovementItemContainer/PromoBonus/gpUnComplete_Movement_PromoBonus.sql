-- Function: gpUnComplete_Movement_PromoBonus (Integer, TVarChar)

DROP FUNCTION IF EXISTS gpUnComplete_Movement_PromoBonus (Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpUnComplete_Movement_PromoBonus(
...
BEGIN
    ...
    -- vbUserId:= lpCheckRight(inSession, zc_Enum_Process_UnComplete_PromoBonus());
    vbUserId := inSession;
...
-- тест
-- SELECT * FROM gpUnComplete_Movement_PromoBonus (inMovementId:= 149639, inSession:= zfCalc_UserAdmin())