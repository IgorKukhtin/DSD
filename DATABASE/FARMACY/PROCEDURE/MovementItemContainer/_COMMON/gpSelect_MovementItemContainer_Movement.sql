     ...
     RETURN QUERY 
       WITH tmpMovement AS (SELECT Movement.Id AS MovementId, Movement.DescId AS MovementDescId, Movement.InvNumber, inIsDestination AS isDestination, inIsParentDetail AS isParentDetail, inIsInfoMoneyDetail AS isInfoMoneyDetail FROM Movement WHERE Movement.Id = inMovementId
                  ...
                  , tmpMIContainer_all AS (SELECT MIContainer.DescId AS MIContainerDescId
                                                ...
                                                , COALESCE(MIContainer.isActive,TRUE) AS isActive
                                                , SUM (CASE WHEN COALESCE(MIContainer.isActive,TRUE) = TRUE OR MIContainer.DescId = zc_MIContainer_Summ() OR MIContainer.DescId = zc_MIContainer_SummCurrency() THEN 1 ELSE -1 END * MIContainer.Amount) AS Amount
          ...
          , tmpMIContainer AS (SELECT tmpMIContainer_Summ_all.Id
                                    , COALESCE(tmpMIContainer_Summ_all.MovementItemId,tmpMIContainer_Count.MovementItemId) AS MovementItemId
                                    ...
                                    , COALESCE(tmpMIContainer_Summ_all.IsActive,tmpMIContainer_Summ_all.IsActive = tmpMIContainer_Count.IsActive) AS IsActive
                               ...
                               FROM tmpMIContainer_Count
                                    LEFT JOIN tmpMIContainer_Summ_all ON tmpMIContainer_Summ_all.MovementItemId = tmpMIContainer_Count.MovementItemId
