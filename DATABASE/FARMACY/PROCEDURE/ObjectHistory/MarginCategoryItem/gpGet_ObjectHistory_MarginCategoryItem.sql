-- Function: gpGet_ObjectHistory_MarginCategoryItem ()

DROP FUNCTION IF EXISTS gpGet_ObjectHistory_MarginCategoryItem (Integer, TDateTime, TVarChar);

CREATE OR REPLACE FUNCTION gpGet_ObjectHistory_MarginCategoryItem(
    IN inMarginCategoryItemId        Integer   , -- Юр.лицо
    ...
    RETURN QUERY
     ...
     FROM ObjectHistory_MarginCategoryItem
            FULL JOIN (SELECT zc_DateStart() AS StartDate
                       ...
                       ) AS Empty ON Empty.ObjectId = ObjectHistory_MarginCategoryItem.ObjectID
...
-- тест
-- SELECT * FROM gpSelect_ObjectHistory_MarginCategoryItem (0, CURRENT_TIMESTAMP)
