...
BEGIN
    ...
    vbUnitId  := CASE WHEN vbUnitKey = '' THEN '0' ELSE vbUnitKey END :: Integer;
        ...
        -- Результат - ВСЕ документы - с типом "Не подтвержден"
          WITH tmpMov_UnC AS (SELECT Movement.Id
             ...
             , tmpMICount AS (SELECT tmpMI_all.MovementId, COUNT(*) AS CountMI
                         FROM tmpMI_all
                         GROUP BY tmpMI_all.MovementId, tmpMI_all.UnitId, tmpMI_all.GoodsId
                        )
    ...
    SELECT STRING_AGG (COALESCE (Movement.Id :: TVarChar, ''), ';')::TVarChar AS RetV
         ...
         , SUM(CASE WHEN tmpMICount.CountMI <= vbExpressVIPConfirm THEN 1 ELSE 0 END)::Integer
    ...
    outExpressVIPConfirm := CASE WHEN COALESCE (outExpressVIPConfirm, 0) > 0 THEN 1 ELSE 0 END;
