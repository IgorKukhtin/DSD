...
BEGIN
    ...
    THEN 
            ...
            IF (COALESCE (vbPriceCalc,0) < inPriceSale) AND (COALESCE (vbPriceCalc,0) <> 0)
               THEN
                   IF vbPersent = 25 THEN  outError :=  'Ошибка. Запрет на отпуск товара по ПКМУ 1303 с наценкой более 25 процентов (для товара с приходной ценой до 100грн)'; END IF;
                   IF vbPersent = 20 THEN  outError :=  'Ошибка. Запрет на отпуск товара по ПКМУ 1303 с наценкой более 20 процентов (для товара с приходной ценой от 100грн до 500грн)'; END IF;
                   IF vbPersent = 15 THEN  outError :=  'Ошибка. Запрет на отпуск товара по ПКМУ 1303 с наценкой более 15 процентов (для товара с приходной ценой от 500грн до 1000грн)'; END IF;
                   IF vbPersent = 10 THEN  outError :=  'Ошибка. Запрет на отпуск товара по ПКМУ 1303 с наценкой более 10 процентов (для товара с приходной ценой свыше 1000грн)'; END IF;
            END IF;
    END IF;
