...
BEGIN
    ...
    THEN 
            ...
            IF EXISTS (SELECT 1 
               ...
               THEN
                   outError := 'Ошибка. Запрет на отпуск товара по ПКМУ 1303 со ставкой НДС=20 проц. (ТОВАР БЕЗ РЕГИСТРАЦИИ !!!)';
                   RETURN;
            END IF;
            ...
            IF (COALESCE (vbPriceCalc,0) < inPriceSale) AND (COALESCE (vbPriceCalc,0) <> 0)
               THEN
                   IF vbPersent = 25 THEN  outError :=  'Ошибка. Запрет на отпуск товара по ПКМУ 1303 с наценкой более 25 процентов (для товара с приходной ценой до 100грн).'||Chr(13)||'Сделать PrintScreen экрана с ошибкой и отправить на Skype своему менеджеру для  исправления Цены реализации (после исправления - препарат можно отпустить по рецепту)'; END IF;
                   IF vbPersent = 20 THEN  outError :=  'Ошибка. Запрет на отпуск товара по ПКМУ 1303 с наценкой более 20 процентов (для товара с приходной ценой от 100грн до 500грн).'||Chr(13)||'Сделать PrintScreen экрана с ошибкой и отправить на Skype своему менеджеру для  исправления Цены реализации (после исправления - препарат можно отпустить по рецепту)'; END IF;
                   IF vbPersent = 15 THEN  outError :=  'Ошибка. Запрет на отпуск товара по ПКМУ 1303 с наценкой более 15 процентов (для товара с приходной ценой от 500грн до 1000грн).'||Chr(13)||'Сделать PrintScreen экрана с ошибкой и отправить на Skype своему менеджеру для  исправления Цены реализации (после исправления - препарат можно отпустить по рецепту)'; END IF;
                   IF vbPersent = 10 THEN  outError :=  'Ошибка. Запрет на отпуск товара по ПКМУ 1303 с наценкой более 10 процентов (для товара с приходной ценой свыше 1000грн).'||Chr(13)||'Сделать PrintScreen экрана с ошибкой и отправить на Skype своему менеджеру для  исправления Цены реализации (после исправления - препарат можно отпустить по рецепту)'; END IF;
            END IF;
    END IF;
