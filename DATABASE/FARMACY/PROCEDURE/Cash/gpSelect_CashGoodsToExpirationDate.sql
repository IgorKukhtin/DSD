     ...
     SELECT Container.GoodsId                                                      AS ID
         -- , COALESCE(tmpObject_Price.Price,0)::TFloat                              AS Price
          ...
          , CASE WHEN COALESCE (Container.ExpirationDate, zc_DateEnd()) <= vbDate_0 THEN zc_Color_Red()        -- просрочено
                 ...
                 WHEN COALESCE (Container.ExpirationDate, zc_DateEnd()) <= vbDate_3 THEN zc_Color_Cyan()       -- Меньше 3 месяца
                 WHEN COALESCE (Container.ExpirationDate, zc_DateEnd()) <= vbDate_6 THEN zc_Color_Aqua()       -- Меньше 6 месяца
