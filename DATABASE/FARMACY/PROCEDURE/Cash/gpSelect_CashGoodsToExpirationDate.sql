...
BEGIN
    ...
    vbPartion := False;
     ...
     SELECT Container.GoodsId                                                      AS ID
          ...
          , CASE WHEN COALESCE (tmpPDContainer.ExpirationDate, tmpExpirationDate.ExpirationDate, zc_DateEnd()) <= vbOperDate THEN zc_Color_Red() ELSE   -- просрочено
            CASE WHEN COALESCE (tmpPDContainer.ExpirationDate, tmpExpirationDate.ExpirationDate, zc_DateEnd()) <= vbDate30  THEN zc_Color_Yelow() ELSE     -- Меньше 1 месяца
            CASE WHEN COALESCE (tmpPDContainer.ExpirationDate, tmpExpirationDate.ExpirationDate, zc_DateEnd()) <= vbDate180 THEN zc_Color_Cyan() ELSE      -- Меньше 6 месяца
