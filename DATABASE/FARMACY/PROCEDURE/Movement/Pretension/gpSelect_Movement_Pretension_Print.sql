...
BEGIN
    ...
    WITH tmpObjectHistory AS (SELECT *
      ...
      SELECT
           ...
           , EXTRACT (DAY FROM MovementDate_Insert.ValueData)::TVarChar                       AS InOperDateDay
           , zfCalc_MonthNameUkr (MovementDate_Insert.ValueData)||' '||
             EXTRACT (YEAR FROM MovementDate_Insert.ValueData)                                AS InOperDateMonthYear
           ...
           , COALESCE(Object_Member.ValueData, Object_Insert.ValueData, '__________________________')                           AS Manager
           , JuridicalDetailsTo.FullName                                                              AS FullNameTo
           , COALESCE( COALESCE(Object_MemberUser.ValueData, Object_User.ValueData)||
             COALESCE('тел. '||ObjectString_Member_Phone.ValueData, '') , '_________________')                         AS UserName
