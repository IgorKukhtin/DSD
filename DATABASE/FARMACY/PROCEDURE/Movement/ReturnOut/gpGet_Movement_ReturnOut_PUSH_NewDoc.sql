-- Function: gpGet_Movement_ReturnOut_PUSH_NewDoc()

DROP FUNCTION IF EXISTS gpGet_Movement_ReturnOut_PUSH_NewDoc (TDateTime, TVarChar);

CREATE OR REPLACE FUNCTION gpGet_Movement_ReturnOut_PUSH_NewDoc(
...
BEGIN
     ...
     WITH 
       ...
       tmpProtocol AS (SELECT MovementItem.Id
                            , MAX(MovementItemProtocol.OperDate)    AS OperDate
                       FROM tmpMI AS MovementItem

                            INNER JOIN MovementItemProtocol ON MovementItemProtocol.MovementItemId = MovementItem.MovementItemId
                                                           AND MovementItemProtocol.isinsert = True
       ...
       WHERE tmpProtocol.OperDate + INTERVAL '1 HOUR' <= CURRENT_TIMESTAMP
         AND tmpProtocol.OperDate + INTERVAL '1 HOUR' > inStartDate;

END;
$BODY$
  LANGUAGE PLPGSQL VOLATILE;
ALTER FUNCTION gpGet_Movement_ReturnOut_PUSH_NewDoc (TDateTime, TVarChar) OWNER TO postgres;
...
-- тест
-- 
SELECT * FROM gpGet_Movement_ReturnOut_PUSH_NewDoc (inStartDate:= '01.11.2021',  inSession:= '3')