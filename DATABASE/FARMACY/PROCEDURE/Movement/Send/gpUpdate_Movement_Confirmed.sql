...
BEGIN
   ...
   THEN
      RAISE EXCEPTION 'Ошибка. Перемещение <Отправдено-да> отмена признака <Подтвержден> запрещено.';
   END IF;

/*   IF inisConfirmed = TRUE AND 
      NOT EXISTS (SELECT 1 FROM ObjectLink_UserRole_View  WHERE UserId = vbUserId AND RoleId = zc_Enum_Role_Admin())
   THEN
      RAISE EXCEPTION 'Ошибка. Отмена признака <Подтвержден> вам запрещена, обратитесь к системному администратору';
   END IF;
*/
