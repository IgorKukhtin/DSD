...
DROP FUNCTION IF EXISTS gpUpdate_Movement_SentList(Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpUpdate_Movement_SentList(
...
BEGIN
   ...
   IF COALESCE(vbisSent, False) = True
   THEN
      RETURN;
   END IF;
   ...
   IF vbisSUN <> TRUE AND vbisVIP <> TRUE OR vbisDeferred <> TRUE
   THEN
      RETURN;
   END IF;
   ...
   -- сохранили признак
   PERFORM lpInsertUpdate_MovementBoolean (zc_MovementBoolean_Sent(), inMovementId, True);
...
/*
 ...
 12.10.19                                                                      *
 06.08.19                                                                      *
*/-- SELECT * FROM gpUpdate_Movement_SentList (inStartDate:= '01.07.2019', inEndDate:= '14.07.2019', inIsErased := FALSE, inisSUN := FALSE, inSession:= '3')
