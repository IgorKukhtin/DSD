-- Function: gpSelect_Movement_SendPartionDate()

DROP FUNCTION IF EXISTS gpSelect_Movement_SendPartionDate (TDateTime, TDateTime, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_SendPartionDate(
...
BEGIN
     ...
     -- PERFORM lpCheckRight (inSession, zc_Enum_Process_Select_Movement_SendPartionDate());
     ...
     WITH tmpStatus AS (SELECT zc_Enum_Status_Complete()   AS StatusId
       ...
       FROM (SELECT Movement.id
             FROM tmpStatus
                  JOIN Movement ON Movement.OperDate BETWEEN inStartDate AND inEndDate  AND Movement.DescId = zc_Movement_SendPartionDate() AND Movement.StatusId = tmpStatus.StatusId
            ) AS tmpMovement
...
-- тест
-- SELECT * FROM gpSelect_Movement_SendPartionDate (inStartDate:= '30.01.2014', inEndDate:= '01.02.2014', inIsErased := FALSE, inSession:= '2')