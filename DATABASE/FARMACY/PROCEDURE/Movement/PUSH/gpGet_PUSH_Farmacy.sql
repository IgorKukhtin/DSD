...
/*   IF (EXISTS (SELECT 1 FROM ObjectLink_UserRole_View  WHERE UserId = vbUserId AND RoleId in (zc_Enum_Role_PartialSale(), zc_Enum_Role_PharmacyManager()))
   ...
   THEN
     ...
     THEN
       ...
       THEN         
         INSERT INTO _PUSH (Id, Text, FormName, Button, Params, TypeParams, ValueParams)
         VALUES (22, 'По вашей аптеке есть Претензии в статусе АКТУАЛЬНА !!!'||CHR(13)|| 
                     'Перейдите в Журнал претензий по кнопке если:'||CHR(13)||CHR(13)|| 
                     '1. Позиция внутри Претензии уже НЕАКТУАЛЬНА (нашли самостоятельно и претензия ещё НЕ ПОДАНА поставщику) - измените статус в соответствующей строке.'||CHR(13)||CHR(13)|| 
                     '2. Претензия решена ПОЛНОСТЬЮ (поставщик довёз товар или забрал излишек, обменял, компенсировал другим товаром или товар нашелся ПОСЛЕ подачи претензии поставщику) - ЗАПОЛНИТЕ ДАТУ РЕШЕНИЯ ПРЕТЕНЗИИ.', 
   ...
   THEN
      ...
      WHERE T1.KeyExpireDate >= date_trunc('month', CURRENT_DATE)
       ...
       THEN         
         ...
         VALUES (23, 'У сотрудников подходит к концу срок действия ключа:'||CHR(13)||CHR(13)||vbText);
       END IF;
   END IF;
   ...
   THEN
       ...
       THEN         
         INSERT INTO _PUSH (Id, Text)
         VALUES (26, 'По вам обнаружено повторы следующих приходных накладных:'||CHR(13)||CHR(13)||vbText||CHR(13)||CHR(13)||         
                     'Просьба проверить наличие товара по ним, в случае вопросов - обратитесь к своему менеджеру!');
       END IF;   
   END IF;
