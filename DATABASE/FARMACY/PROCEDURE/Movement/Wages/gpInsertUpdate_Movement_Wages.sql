...
BEGIN
   ...
   CREATE TEMP TABLE tmpCorrectionPercentage ON COMMIT DROP AS
     SELECT 1::Integer  AS Id, 14672280::Integer  AS UserId, 13711869::Integer  AS UnitId, 50::TFloat AS Percentage;
   ...
   FROM tmpBoard
        ...
        LEFT OUTER JOIN gpSelect_Calculation_PayrollGroup(inPayrollTypeID    := Object_PayrollType.Id,
                                                          ...
                                                          inCountUser        := 0,
                                                          inCorrPercentage   := COALESCE (tmpCorrectionPercentage.Percentage, 100),
...
select * from gpSelect_CalculationPerson_Print(inOperDate := ('01.09.2020')::TDateTime , inUserID := 14672280 ,  inSession := '3');