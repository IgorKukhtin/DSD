-- Function: gpInsert_Movement_Invoice_PaperRecipeSPInsulin()

DROP FUNCTION IF EXISTS gpInsert_Movement_Invoice_PaperRecipeSPInsulin (TDateTime, TDateTime, Integer, Integer, TDateTime, TVarChar, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpInsert_Movement_Invoice_PaperRecipeSPInsulin(
    ...
    CREATE TEMP TABLE tmpInvoice (MovementId Integer, InvNumber TVarChar, JuridicalId Integer, PartnerMedicalId Integer,  ContractId Integer) ON COMMIT DROP;
     ...
     INSERT INTO tmpReport (MovementId_Check, JuridicalId, PartnerMedicalId, ContractId, SummaComp, CountSP)
       ...
       FROM (SELECT  tmp.Id                       AS MovementId
                   ...
                   , SUM (tmp.CountSP)            AS CountSP
             FROM gpReport_PaperRecipeSPInsulin(inStartDate:=inStartDate, inEndDate:=inEndDate, inJuridicalId:=inJuridicalId
...
select * from gpInsert_Movement_Invoice_PaperRecipeSPInsulin(inStartDate := ('01.03.2022')::TDateTime , inEndDate := ('12.04.2022')::TDateTime , inJuridicalId := 1311462 , inJuridicalMedicId := 10959824 , inDateInvoice := ('01.01.2017')::TDateTime , inInvoice := '0' , inisInsert := 'True' ,  inSession := '3');