...
DROP FUNCTION IF EXISTS gpSelect_Movement_Sale_InsuranceCompanies (TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_Sale_InsuranceCompanies(
...
BEGIN
   ...
   FROM Movement
        ...
        INNER JOIN MovementLinkObject AS MovementLinkObject_InsuranceCompanies
                                     ON MovementLinkObject_InsuranceCompanies.MovementId =  Movement.Id
                                    AND MovementLinkObject_InsuranceCompanies.DescId = zc_MovementLinkObject_InsuranceCompanies()
        LEFT JOIN Object AS Object_InsuranceCompanies ON Object_InsuranceCompanies.Id = MovementLinkObject_InsuranceCompanies.ObjectId
  ...
  OPEN Cursor2 FOR (
      WITH
        ...
        , tmpMI_Sum AS (SELECT  MovementItem.*
                             ...
                             , Round(COALESCE (MovementItem.Amount, 0) * MIFloat_Price.ValueData, 2) AS AmountSumm
     ...
     SELECT
         ...
         , ROUND(MovementItem.Amount * COALESCE (MovementItem.PriceSale, 0), 2) -
                       COALESCE (MovementItem.AmountSumm, 0)::TFloat AS SummIC
         , False                               AS isErased
