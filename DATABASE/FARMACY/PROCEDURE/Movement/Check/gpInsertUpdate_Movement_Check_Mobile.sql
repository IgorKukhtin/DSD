-- Function: gpInsertUpdate_Movement_Check_Mobile()

DROP FUNCTION IF EXISTS gpInsertUpdate_Movement_Check_Mobile (Integer, Integer, TDateTime, Integer, TVarChar, TVarChar, TVarChar, TVarChar, Boolean, TFloat, Boolean, TVarChar, Boolean, Integer, Boolean, TDateTime, TFloat, TVarChar);
        
CREATE OR REPLACE FUNCTION gpInsertUpdate_Movement_Check_Mobile(
...
BEGIN
    ...
    IF COALESCE (inisMobileFirstOrder, False) = TRUE
    THEN
      PERFORM lpInsertUpdate_MovementBoolean (zc_MovementBoolean_MobileFirstOrder(), ioId, True);
    END IF;
