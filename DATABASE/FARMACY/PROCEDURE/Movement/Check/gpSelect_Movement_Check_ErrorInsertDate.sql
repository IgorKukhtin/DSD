-- Function: gpSelect_Movement_Check_ErrorInsertDate()

DROP FUNCTION IF EXISTS gpSelect_Movement_Check_ErrorInsertDate (TDateTime, TDateTime, Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_Check_ErrorInsertDate(
...
BEGIN
     ...
     CREATE TEMP TABLE tmpMovement_Check ON COMMIT DROP AS (
              SELECT Movement.Id
                   , MovementLinkObject_Unit.ObjectId                    AS UnitId
                   , MLO_Insert.ObjectId                                 AS InsertId
                   , date_trunc('day', MovementDate_Insert.ValueData)    AS InsertDate
              FROM Movement
              ...
              WHERE Movement.OperDate >= DATE_TRUNC ('DAY', inStartDate) 
                ...
                AND (Object_Unit.ValueData NOT LIKE 'Апт. пункт%' OR COALESCE (inUnitId, 0) <> 0)
                AND date_trunc('day', MovementDate_Insert.ValueData) <> date_trunc('day', Movement.OperDate));
                
     ANALYSE tmpMovement_Check;
     ...
     RETURN QUERY
         SELECT       
           ...
           , Movement_Check.InsertDate                          AS InsertDate
        ...
        FROM (SELECT Movement.*
              ...
              FROM tmpMovement_Check 
                   ...
                   INNER JOIN Movement ON Movement.ID = tmpMovement_Check.ID
                   ...
                   LEFT JOIN MovementDate AS MovementDate_Insert
                                          ON MovementDate_Insert.MovementId = Movement.Id
                                         AND MovementDate_Insert.DescId = zc_MovementDate_Insert()) AS Movement_Check
                
             INNER JOIN (SELECT Movement.InsertId
                              , Movement.InsertDate 
...
select * from gpSelect_Movement_Check_ErrorInsertDate(inStartDate := ('01.01.2023')::TDateTime , inEndDate := ('16.01.2023')::TDateTime , inUnitId := 0 ,  inSession := '3');