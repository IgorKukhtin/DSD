-- Function: gpGet_Movement_PromoBonus()

DROP FUNCTION IF EXISTS gpGet_Movement_PromoBonus (Integer, TDateTime, TVarChar);

CREATE OR REPLACE FUNCTION gpGet_Movement_PromoBonus(
...
BEGIN
     ...
     -- vbUserId := PERFORM lpCheckRight (inSession, zc_Enum_Process_Get_Movement_PromoBonus());
     vbUserId := inSession;
     ...
     RETURN QUERY
         SELECT
               0 AS Id
             , CAST (NEXTVAL ('Movement_PromoBonus_seq') AS TVarChar) AS InvNumber
...
-- тест
-- SELECT * FROM gpGet_Movement_PromoBonus (inMovementId:= 1, inOperDate := CURRENT_TIMESTAMP, inSession:= '9818')