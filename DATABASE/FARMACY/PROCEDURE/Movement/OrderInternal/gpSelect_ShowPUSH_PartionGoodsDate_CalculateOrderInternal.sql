...
BEGIN
    ...
    WITH
        ...
        tmpNTZ AS (SELECT string_agg('СРОК ГОДНОСТИ : '||zfConvert_DateShortToString (tmpMI_All.PartionGoodsDate)||' '|| COALESCE(tmpMI_All.Comment, '')||Chr(13)||
        ...
        tmpListDiff AS (SELECT string_agg('СРОК ГОДНОСТИ : '||zfConvert_DateShortToString (tmpMI_All.PartionGoodsDate)||' '|| COALESCE(tmpMI_All.Comment, '')||Chr(13)||
                                     '  '||Object_Goods_Main.ObjectCode||' - '||Object_Goods_Main.Name, Chr(13)) AS Goods
                        FROM tmpMI_All
