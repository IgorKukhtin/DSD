-- Function: gpGet_Movement_InventoryHouseholdInventory()

DROP FUNCTION IF EXISTS gpGet_Movement_InventoryHouseholdInventory (Integer, TDateTime, TVarChar);

CREATE OR REPLACE FUNCTION gpGet_Movement_InventoryHouseholdInventory(
...
BEGIN
     ...
     RETURN QUERY
         SELECT
               0 AS Id
             , CAST (NEXTVAL ('Movement_InventoryHouseholdInventory_seq') AS TVarChar) AS InvNumber
...
-- тест
-- SELECT * FROM gpGet_Movement_InventoryHouseholdInventory (inMovementId:= 1, inOperDate := CURRENT_TIMESTAMP, inSession:= '9818')