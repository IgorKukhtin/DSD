-- Function: gpSelect_Movement_InventoryHouseholdInventory()

DROP FUNCTION IF EXISTS gpSelect_Movement_InventoryHouseholdInventory (TDateTime, TDateTime, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_InventoryHouseholdInventory(
...
BEGIN
     ...
     -- PERFORM lpCheckRight (inSession, zc_Enum_Process_Select_Movement_InventoryHouseholdInventory());
     ...
     WITH tmpStatus AS (SELECT zc_Enum_Status_Complete()   AS StatusId
       ...
       FROM (SELECT Movement.id
             FROM tmpStatus
                  JOIN Movement ON Movement.OperDate BETWEEN inStartDate AND inEndDate  AND Movement.DescId = zc_Movement_InventoryHouseholdInventory() AND Movement.StatusId = tmpStatus.StatusId
            ) AS tmpMovement
...
-- тест
-- SELECT * FROM gpSelect_Movement_InventoryHouseholdInventory (inStartDate:= '30.01.2020', inEndDate:= '01.02.2020', inIsErased := FALSE, inSession:= '3')