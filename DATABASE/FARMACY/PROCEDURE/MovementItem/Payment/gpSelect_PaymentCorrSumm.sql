...
BEGIN
    ...
    CREATE TEMP TABLE tmpContainer ON COMMIT DROP AS 
     SELECT CASE WHEN Container.ObjectId = zc_Enum_ChangeIncomePaymentKind_Bonus()       THEN Container.Amount ELSE 0 END ::TFloat AS ContainerAmountBonus
    ...
    CREATE TEMP TABLE tmpMI ON COMMIT DROP AS 
     SELECT SUM (CASE WHEN Movement.StatusId = zc_Enum_Status_Complete() THEN COALESCE (MIFloat_CorrBonus.ValueData, 0)       ELSE 0 END) :: TFloat AS ContainerAmountBonus
...
select * from gpSelect_PaymentCorrSumm(inMovementId := 33820004 , inJuridicalId := 1311462 ,  inSession := '3');