       ...
       WITH
         ...
         SELECT COALESCE (MI_Child.Id,0)                        AS Id
              ...
              , CASE WHEN tmpRemains.ExpirationDate < vbOperDate THEN 'Просрочено'
                     ...
                     WHEN tmpRemains.ExpirationDate <= vbDate180 THEN 'Меньше 6 месяцев'
                     ELSE ''
          ...
          OPEN Cursor1 FOR
               WITH
                 ...
                 , MI_Child AS (SELECT MovementItem.ParentId
                                     ...
                                     , SUM (CASE WHEN COALESCE (MIFloat_Expired.ValueData,0) = zc_Enum_PartionDateKind_1() THEN MovementItem.Amount ELSE 0 END) AS Amount_1   -- Меньше 1 месяца
                                     , SUM (CASE WHEN COALESCE (MIFloat_Expired.ValueData,0) = zc_Enum_PartionDateKind_6() THEN MovementItem.Amount ELSE 0 END) AS Amount_2   -- Меньше 1 месяца
