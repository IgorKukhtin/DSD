       ...
       OPEN Cursor1 FOR
          ...
          , tmpPrice AS (SELECT Price_Goods.ChildObjectId                AS GoodsId
                 ...
                 , MI_Child AS (SELECT MovementItem.ParentId
                                     ...
                                     , SUM (CASE WHEN COALESCE (MIFloat_Expired.ValueData,0) = 1 THEN MovementItem.Amount ELSE 0 END) AS Amount_1   -- Меньше 1 месяца
                                     , SUM (CASE WHEN COALESCE (MIFloat_Expired.ValueData,0) = 2 THEN MovementItem.Amount ELSE 0 END) AS Amount_2   -- Меньше 1 месяца
                                FROM  MovementItem
