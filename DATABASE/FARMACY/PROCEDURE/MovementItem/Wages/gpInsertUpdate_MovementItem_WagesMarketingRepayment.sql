-- Function: gpInsertUpdate_MovementItem_WagesMarketingRepayment()

DROP FUNCTION IF EXISTS gpInsertUpdate_MovementItem_WagesMarketingRepayment(Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpInsertUpdate_MovementItem_WagesMarketingRepayment(
...
BEGIN
    ...
    vbSumma := COALESCE((SELECT  Sum(MovementFloat_TotalSumm.ValueData)
                         FROM Movement
                              ...
                              INNER JOIN MovementBoolean AS MovementBoolean_CorrectMarketing
   ...
   --  RAISE EXCEPTION 'Ошибка. % %.', vbId, vbSumma;                           
    ...
    PERFORM lpInsertUpdate_MovementItemFloat (zc_MIFloat_MarketingRepayment(), vbId, vbSumma);
