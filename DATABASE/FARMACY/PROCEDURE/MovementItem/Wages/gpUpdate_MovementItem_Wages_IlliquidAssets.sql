...
BEGIN
    ...
    FROM (SELECT COALESCE(MovementItem.id, 0) AS Id, IPE.UnitID, IPE.UserID, ROUND(COALESCE (- IPE.SummaPenalty, 0)) AS SummaPenalty
          FROM gpReport_IlliquidReductionPlanAll(inStartDate := DATE_TRUNC ('MONTH', inOperDate) - INTERVAL '1 DAY', inPenalty := 250 ,  inProcGoods := 20 , inProcUnit := 10, inPlanAmount := 7, inSession := inSession) AS IPE
