...
1) кп-6 в вип чеках название чека-25,сумма 576,90 от 09.06.16 касса16,время 12:11:57
 он же есть в пробитых (проведенных)

По Березинке препараты которые 100% не соответствуют наличию
+??1) Наклофен амп №5--2,6уп,
+??2) Неокардил таб №30---1уп
+??3) АкваСпрей Плюс гипертонический --1уп
+??4) валусао гель--1уп,

По Калинке
+1) 3607 нет на рынке больше пол года (давно не числился и появился)
select * from _MI_Container_13_06_16 where ContainerId in (904758) AND MovementDescId = 10 -- 904758 - 1714874
select * from _MI_Container_13_06_16 where ContainerId in (1636859) AND MovementDescId = 10

+??2) 1884 числится дробное количество от флакона. (вылезло а давно нет)
+??3) 1390 проверить недостача 30шт
4) 8819 числится лишние 4 уп
select * from _MI_Container_13_06_16 where ContainerId in (772660, 772659) AND MovementDescId = 10 -- 772659 - 1714664 + 772660 - 1714665


По батумской проблема.
1) Препарат немотан №30 - 1уп - последний приход 16.01 - последняя продажа 19.01; в файле переучета в списке этого препарата нет и нет в наличии.
   Они делали полный переучет и я проверял там действительно нет его. Как же он может числиться?!
"417";"2015-11-13 12:05:55.273+03"
