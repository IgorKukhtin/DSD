...
BEGIN
     ...
     RETURN REPLACE
           ...
           (REPLACE (COALESCE (inComment, ''), 'NOM_DOG',    COALESCE (inNOM_DOG, ''))
                                             ...
                                             , 'PDV', '20')
                                             , 'SUMMA_P', zfConvert_FloatToString (ROUND(inSUMMA_P/6, 2)));
...
-- тест
-- SELECT * FROM zfCalc_Comment_pay_OrderFinance ('text: 1(NOM_DOG) 2(NOM_IVOICE) 3(DATA_DOG) 4(PDV) 5(SUMMA_P)', '11', '22', CURRENT_DATE, 44, 240);
