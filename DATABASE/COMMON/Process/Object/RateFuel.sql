...
PERFORM lpInsertUpdate_Object_Enum (inId:= zc_Enum_Process_Select_Object_RateFuel()
 ...
 FROM (SELECT Id AS RoleId FROM Object WHERE DescId = zc_Object_Role() AND ObjectCode in (-1)) AS tmpRole
      JOIN (SELECT zc_Enum_Process_InsertUpdate_Object_RateFuel() AS ProcessId) AS tmpProcess ON 1=1
 ...
 -- select  tmpData.RoleRightId, tmpRole.RoleId, tmpProcess.ProcessId
 FROM (SELECT Id AS RoleId FROM Object WHERE DescId = zc_Object_Role() AND ObjectCode in (-1)) AS tmpRole
      JOIN (SELECT zc_Enum_Process_Select_Object_RateFuel()  AS ProcessId) AS tmpProcess ON 1=1
