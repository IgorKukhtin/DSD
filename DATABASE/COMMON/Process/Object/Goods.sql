...
CREATE OR REPLACE FUNCTION zc_Enum_Process_Update_Object_Goods_isPLM() RETURNS Integer AS $BODY$BEGIN RETURN (SELECT ObjectId AS Id FROM ObjectString WHERE ValueData = 'zc_Enum_Process_Update_Object_Goods_isPLM' AND DescId = zc_ObjectString_Enum()); END; $BODY$ LANGUAGE plpgsql;
...
PERFORM lpInsertUpdate_Object_Enum (inId:= zc_Enum_Process_Update_Object_Goods_isPLM()
                                  , inDescId:= zc_Object_Process()
                                  , inCode:= 6
                                  , inName:= 'Справочник <'||(SELECT ItemName FROM ObjectDesc WHERE Id = zc_Object_Goods())||'> - изменение данных.'
                                  , inEnumName:= 'zc_Enum_Process_Update_Object_Goods_isPLM');
/*             
