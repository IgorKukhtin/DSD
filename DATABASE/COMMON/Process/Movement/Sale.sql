...
CREATE OR REPLACE FUNCTION zc_Enum_Process_Insert_Movement_Sale_mask() RETURNS Integer AS $BODY$BEGIN RETURN (SELECT ObjectId AS Id FROM ObjectString WHERE ValueData = 'zc_Enum_Process_Insert_Movement_Sale_mask' AND DescId = zc_ObjectString_Enum()); END; $BODY$ LANGUAGE plpgsql IMMUTABLE;
...
PERFORM lpInsertUpdate_Object_Enum (inId:= zc_Enum_Process_Insert_Movement_Sale_mask()
                                  , inDescId:= zc_Object_Process()
                                  , inCode:= 9
                                  , inName:= 'Документ <'||(SELECT ItemName FROM MovementDesc WHERE Id = zc_Movement_Sale())||'> - сохранение данных.'
                                  , inEnumName:= 'zc_Enum_Process_Insert_Movement_Sale_mask');
