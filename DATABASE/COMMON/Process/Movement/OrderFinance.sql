...
CREATE OR REPLACE FUNCTION zc_Enum_Process_Update_MI_OrderFinance_byReport() RETURNS Integer AS $BODY$BEGIN RETURN (SELECT ObjectId AS Id FROM ObjectString WHERE ValueData = 'zc_Enum_Process_Update_MI_OrderFinance_byReport' AND DescId = zc_ObjectString_Enum()); END; $BODY$ LANGUAGE plpgsql IMMUTABLE;
...
CREATE OR REPLACE FUNCTION zc_Enum_Process_Update_MI_OrderFinance_SB() RETURNS Integer AS $BODY$BEGIN RETURN (SELECT ObjectId AS Id FROM ObjectString WHERE ValueData = 'zc_Enum_Process_Update_MI_OrderFinance_SB' AND DescId = zc_ObjectString_Enum()); END; $BODY$ LANGUAGE plpgsql IMMUTABLE;
...
--
CREATE OR REPLACE FUNCTION zc_Enum_Process_Update_Movement_OrderFinance_SignSB() RETURNS Integer AS $BODY$BEGIN RETURN (SELECT ObjectId AS Id FROM ObjectString WHERE ValueData = 'zc_Enum_Process_Update_Movement_OrderFinance_SignSB' AND DescId = zc_ObjectString_Enum()); END; $BODY$ LANGUAGE plpgsql IMMUTABLE;
...
PERFORM lpInsertUpdate_Object_Enum (inId:= zc_Enum_Process_Update_MI_OrderFinance_byReport()
                                  , inDescId:= zc_Object_Process()
                                  , inCode:= 1
                                  , inName:= 'Элемент документа <'||(SELECT ItemName FROM MovementDesc WHERE Id = zc_Movement_OrderFinance())||'> - сохранение данных(отчет).'
                                  , inEnumName:= 'zc_Enum_Process_Update_MI_OrderFinance_byReport');
...
PERFORM lpInsertUpdate_Object_Enum (inId:= zc_Enum_Process_Update_MI_OrderFinance_SB()
                                  , inDescId:= zc_Object_Process()
                                  , inCode:= 4
                                  , inName:= 'Элемент документа <'||(SELECT ItemName FROM MovementDesc WHERE Id = zc_Movement_OrderFinance())||'> - сохранение данных.'
                                  , inEnumName:= 'zc_Enum_Process_Update_MI_OrderFinance_SB');
...
--
PERFORM lpInsertUpdate_Object_Enum (inId:= zc_Enum_Process_Update_Movement_OrderFinance_SignSB()
                                  , inDescId:= zc_Object_Process()
                                  , inCode:= 4
                                  , inName:= 'Документ <'||(SELECT ItemName FROM MovementDesc WHERE Id = zc_Movement_OrderFinance())||'> - изменение данных.'
                                  , inEnumName:= 'zc_Enum_Process_Update_Movement_OrderFinance_SignSB');
END $$;
