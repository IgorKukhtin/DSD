...
BEGIN
     ...
     IF zfCheck_User_StatusId_next (vbUserId) = TRUE AND EXISTS (SELECT 1 FROM Movement WHERE Movement.Id = inMovementId AND Movement.StatusId_next = zc_Enum_Status_UnComplete())
     THEN
     ...
     FROM (WITH tmpMI AS (SELECT MovementItem.Id AS MovementItemId, MIF_ContainerId.ValueData :: Integer AS ContainerId
                          FROM MovementItem
                               INNER JOIN MovementItemFloat AS MIF_ContainerId
                                                            ON MIF_ContainerId.MovementItemId = MovementItem.Id
                                                           AND MIF_ContainerId.DescId         = zc_MIFloat_ContainerId()
                                                           AND MIF_ContainerId.ValueData      > 0
                          WHERE MovementItem.MovementId = inMovementId
         ...
         , tmpMI_find AS (SELECT DISTINCT tmpMI.MovementItemId
                          FROM tmpMI
                               INNER JOIN Container AS Container_Summ ON Container_Summ.ParentId = tmpMI.ContainerId
           ...
           SELECT tmpMI.*
           FROM tmpMI
                LEFT JOIN tmpMI_find ON tmpMI_find.MovementItemId = tmpMI.MovementItemId
           WHERE tmpMI_find.MovementItemId IS NULL
          ) AS tmpMI
          ;
          ...
          , tmpMIB_all AS (SELECT MovementItemBoolean.*
           ...
           , tmpMI AS (SELECT MovementItem.Id AS MovementItemId
                           ...
                           , COALESCE (ObjectBoolean_Asset.ValueData, FALSE) AS isAsset
                             ...
                             -- <Остаток теория>
                           , COALESCE (MIBoolean_NotFact.ValueData, FALSE)   AS isNotFact
                       ...
                       FROM tmpMI_all AS MovementItem
                            ...
                            LEFT JOIN tmpMIB_all AS MIBoolean_NotFact
...
IF vbUnitId = zc_Unit_RK()
THEN
    -- !!!Оптимизация!!! : zc_Container_Count + zc_Container_Summ
    CREATE TEMP TABLE tmpContainer_all_0 ON COMMIT DROP AS
       (WITH tmpContainer_data AS (SELECT Container.Id
                                        , Container.ParentId
                                        , Container.DescId
                                        , Container.ObjectId
                                        , Container.Amount
                                          -- Остаток на дату - на конец vbOperDate
                                   ...
                                   WHERE ContainerLinkObject.ObjectId = vbUnitId
       ...
       SELECT tmpContainer_data.*
       FROM tmpContainer_data)
      ;
     ...
     , tmpContainerList_count AS (SELECT Container.*
         ...
         , tmpContainer_count AS (SELECT tmpContainerList.ParentId AS ContainerId_Goods
           ...
           , tmpContainer_all AS (SELECT tmpContainerList.Id       AS ContainerId_Goods
             ...
             -- Остаток факт - инвентаризация + движение
           , tmpContainer_all_union AS (SELECT tmpContainer_all.ContainerId_Goods
                                             , tmpContainer_all.GoodsId
                                             , COALESCE (CLO_GoodsKind.ObjectId, 0) AS GoodsKindId

                                               -- Остаток на дату
                                             , COALESCE (tmpContainer_all.OperCount, 0) AS OperCount_onDate
                                               -- Остаток факт - инвентаризация
                                             , COALESCE (_tmpItem.OperCount, 0) AS OperCount

                                               -- здесь только движение до конца месяца
                                             , tmpContainer_all.OperCount_add
                                               -- здесь ABS движение до конца месяца
                                            , tmpContainer_all.OperCount_find
                                        ...
                                        FROM tmpContainer_all
                                             ...
                                             LEFT JOIN (SELECT _tmpItem.ContainerId_Goods, SUM(_tmpItem.OperCount) AS OperCount FROM _tmpItem GROUP BY _tmpItem.ContainerId_Goods
                                                         ...
                                                         ON _tmpItem.ContainerId_Goods = tmpContainer_all.ContainerId_Goods
                                       ...
                                       UNION ALL
                                        ...
                                        -- факт ввод остатка
                                        FROM _tmpItem
                                             LEFT JOIN tmpContainer_all ON tmpContainer_all.ContainerId_Goods = _tmpItem.ContainerId_Goods
                                        -- вдруг не нашли
                                        WHERE tmpContainer_all.ContainerId_Goods IS NULL
                                        GROUP BY _tmpItem.ContainerId_Goods
                                               , _tmpItem.GoodsId
                                               , _tmpItem.GoodsKindId
                                      )
             ...
             , tmpContainer_minus AS (SELECT tmpContainer_all.ContainerId_Goods
                                      ...
                                      FROM tmpContainer_all_union AS tmpContainer_all
                                      WHERE vbIsLastOnMonth_RK = TRUE
                                        AND (tmpContainer_all.OperCount + tmpContainer_all.OperCount_add) < 0
                                     )
           ...
           , tmpContainer_add AS (SELECT tmpContainer_all.ContainerId_Goods
                                       ...
                                       , SUM (tmpContainer_all.OperCount + tmpContainer_all.OperCount_add)
                                             OVER (PARTITION BY tmpContainer_all.GoodsId, tmpContainer_all.GoodsKindId
                                       ...
                                       , ROW_NUMBER() OVER (PARTITION BY tmpContainer_all.GoodsId, tmpContainer_all.GoodsKindId
                                  ...
                                  FROM tmpContainer_all_union AS tmpContainer_all
                                       ...
                                       INNER JOIN (SELECT tmpContainer_minus.GoodsId
                                                   ...
                                                   AND tmpContainer_minus.GoodsKindId = tmpContainer_all.GoodsKindId
    ...
    , tmpContainer_partion AS (SELECT DD.ContainerId_Goods
        ...
        SELECT COALESCE (tmpMI_find.MovementItemId, 0)            AS MovementItemId
             ...
             , COALESCE (ObjectLink_InfoMoneyDestination.ChildObjectId, 0)
               ...
               -- Расч. остаток на дату
             , tmpContainer.OperCount_onDate
               --
             + CASE WHEN tmpContainer.OperCount + tmpContainer.OperCount_add < 0 AND tmpContainer_partion_find.Amount = tmpContainer_minus.OperCount_minus
                         -- уменьшили расчетный остаток, чтоб разница с фактом стала больше, тогда в итоге юудет +
                         THEN 1 * (tmpContainer.OperCount + tmpContainer.OperCount_add) -- здесь значение с минусом

                    WHEN tmpContainer_partion.Amount > 0
                         -- увеличили расчетный остаток, чтоб разница с фактом стала меньше, тогда в итоге будет -
                         THEN 1 * tmpContainer_partion.Amount -- здесь значение с плюсом, т.к. уже было * -1
        ...
        FROM tmpContainer_all_union AS tmpContainer
             ...
             LEFT JOIN (SELECT tmpContainer_partion.GoodsId, tmpContainer_partion.GoodsKindId, SUM (tmpContainer_partion.Amount) AS Amount
                        ...
                        AND tmpContainer_partion_find.GoodsKindId = tmpContainer.GoodsKindId
             ...
             LEFT JOIN (SELECT tmpContainer_minus.GoodsId
                        ...
                        AND tmpContainer_minus.GoodsKindId = tmpContainer.GoodsKindId
...
-- select *  from Container where Id = 5630242
     ...
     INSERT INTO _tmpRemainsCount (MovementItemId, ContainerId_Goods, ContainerId_count, GoodsId, GoodsKindId, InfoMoneyGroupId, InfoMoneyDestinationId, OperCount, OperCount_add, OperCountCount, OperCount_find, OperCountCount_find, OperSumm_item)
        ...
        FROM _tmpRemainsSumm
             ...
             LEFT JOIN (SELECT MAX (_tmpItem.MovementItemId) AS MovementItemId, _tmpItem.ContainerId_Goods
                             , MAX (CASE WHEN _tmpItem.isNotFact = TRUE THEN 1 ELSE 0 END) AS isNotFact_value
                        FROM _tmpItem GROUP BY _tmpItem.ContainerId_Goods
     ...
     THEN
          RAISE EXCEPTION 'Ошибка.Остаток для партии товара <%> <%> <%> <%> не уникален.<%> <%>', lfGet_Object_ValueData ((SELECT GoodsId FROM _tmpItem WHERE ContainerId_Goods = (SELECT ContainerId_Goods FROM _tmpItem GROUP BY ContainerId_Goods HAVING COUNT(*) > 1 ORDER BY 1 LIMIT 1) LIMIT 1))
                                                                                           ...
                                                                                           , (SELECT COUNT(*) FROM (SELECT ContainerId_Goods FROM _tmpItem GROUP BY ContainerId_Goods HAVING COUNT(*) > 1) AS tmp)
                                                                                            ;
     END IF;
       ...
       (SELECT _tmp.MovementItemId
        ...
        FROM (WITH -- где будем искать Ирну
         ...
         , tmpContainer_summ AS (SELECT tmpMI.GoodsId
                                      , COALESCE (tmpMI.GoodsKindId, 0) AS GoodsKindId
                                        -- нашли счет
            ...
            -- где надо проверить с/с и потом найти альтернативную цену
          , tmpHistoryCost_find_all AS (SELECT _tmpItem.ContainerId_Goods
                       ...
                       , tmp_list_1 AS (SELECT DISTINCT tmpHistoryCost_list.ContainerId_Goods, tmpHistoryCost_list.GoodsId
                                        FROM -- нашли у каких ContainerId_Goods ВСЕ цены = 0
                                             (SELECT tmpHistoryCost_find_all.ContainerId_Goods, tmpHistoryCost_find_all.GoodsId
                                              FROM tmpHistoryCost_find_all
                                              GROUP BY tmpHistoryCost_find_all.ContainerId_Goods, tmpHistoryCost_find_all.GoodsId
                                              HAVING MAX (tmpHistoryCost_find_all.Price) = 0
                                             ) AS tmpHistoryCost_list
                                       )

                         -- альтернатива
                       , tmp_list_2 AS (SELECT tmp_list_1.GoodsId                       AS GoodsId
                                             , Container_Summ_new.ObjectId              AS AccountId
                                             , CLO_JuridicalBasis_new.ObjectId          AS JuridicalId_Basis
                                             , CLO_InfoMoney_new.ObjectId               AS InfoMoneyId
                                             , CLO_InfoMoneyDetail_new.ObjectId         AS InfoMoneyId_Detail
                                             , COALESCE (CLO_GoodsKind_new.ObjectId, 0) AS GoodsKindId
                                             , MAX (HistoryCost.Price)                  AS Price
                                        FROM (SELECT DISTINCT tmp_list_1.GoodsId FROM tmp_list_1) AS tmp_list_1
                                             -- альтернатива
                                             JOIN Container AS Container_Count_new ON Container_Count_new.ObjectId      = tmp_list_1.GoodsId
                                                                                  AND Container_Count_new.DescId        = zc_Container_Count()
                                                                                  AND Container_Count_new.WhereObjectId = vbUnitId
                                             ...
                                             -- св-ва
                                             LEFT JOIN ContainerLinkObject AS CLO_InfoMoney_new ON CLO_InfoMoney_new.ContainerId = Container_Summ_new.Id
                                                                                               AND CLO_InfoMoney_new.DescId      = zc_ContainerLinkObject_InfoMoney()
                                             LEFT JOIN ContainerLinkObject AS CLO_InfoMoneyDetail_new ON CLO_InfoMoneyDetail_new.ContainerId = Container_Summ_new.Id
                                                                                                     AND CLO_InfoMoneyDetail_new.DescId      = zc_ContainerLinkObject_InfoMoneyDetail()
                                             LEFT JOIN ContainerLinkObject AS CLO_JuridicalBasis_new ON CLO_JuridicalBasis_new.ContainerId = Container_Summ_new.Id
                                        ...
                                        GROUP BY tmp_list_1.GoodsId
                                               , Container_Summ_new.ObjectId
                                               , CLO_JuridicalBasis_new.ObjectId
                                               , CLO_InfoMoney_new.ObjectId
                                               , CLO_InfoMoneyDetail_new.ObjectId
                                               , COALESCE (CLO_GoodsKind_new.ObjectId, 0)
                                       )
              ...
              , tmpHistoryCost_find AS (SELECT Container_Summ.ContainerId_Goods
                                             ...
                                             , MAX (tmp_list_2.Price) AS Price
                                        FROM -- нашли у каких ContainerId_Goods ВСЕ цены = 0
                                             tmp_list_1 AS tmpHistoryCost_list
                                             ...
                                             -- альтернатива
                                             JOIN tmp_list_2 ON tmp_list_2.GoodsId              = tmpHistoryCost_list.GoodsId
                                                            AND tmp_list_2.AccountId            = Container_Summ.AccountId
                                                            AND tmp_list_2.JuridicalId_Basis    = CLO_JuridicalBasis.ObjectId
                                                            AND tmp_list_2.InfoMoneyId          = CLO_InfoMoney.ObjectId
                                                            AND tmp_list_2.InfoMoneyId_Detail   = CLO_InfoMoneyDetail.ObjectId
                                                            AND tmp_list_2.GoodsKindId          = Container_Summ.GoodsKindId
...
--                               -1 * CAST (_tmpRemainsCount.OperCount * COALESCE (HistoryCost.Price, 0) AS NUMERIC (16,4))
                   ...
                   - CASE WHEN vbIsLastOnMonth_RK = TRUE -- AND vbUserId IN (zc_Enum_Process_Auto_PrimeCost())
                          THEN COALESCE (_tmpRemainsSumm.OperSumm_add, 0)
                          ELSE 0
                     END
             ...
             UNION ALL
              ...
              FROM _tmpRemainsSumm
                   LEFT JOIN _tmpRemainsCount ON _tmpRemainsCount.ContainerId_Goods = _tmpRemainsSumm.ContainerId_Goods
                   LEFT JOIN (SELECT _tmpItem.ContainerId_Goods, SUM (_tmpItem.OperCount) AS OperCount
                                   , MAX (CASE WHEN _tmpItem.isNotFact = TRUE THEN 1 ELSE 0 END) AS isNotFact_value
                              FROM _tmpItem GROUP BY _tmpItem.ContainerId_Goods
...
--                               -1 * CAST (_tmpRemainsCount.OperCount * COALESCE (HistoryCost.Price, 0) AS NUMERIC (16,4))
        ...
        SELECT tmp_all.MovementItemId
             ...
             , tmp_all.ContainerId
             , tmp_all.AccountId
             , tmp_all.OperSumm - 0 AS OperSumm -- !!! - COALESCE (HistoryCost.Summ_diff, 0) - если есть "погрешность" при округлении, добавили сумму!!!
        FROM tmp_all
...
-- , (select _tmpItem.ContainerId_Goods from _tmpItem where _tmpItem.MovementItemId = 314931327 )
-- , (select _tmpItem.PartionGoodsId from _tmpItem where _tmpItem.MovementItemId = 314931327 )
-- , (select _tmpItem.OperCount from _tmpItem where _tmpItem.MovementItemId = 314931327 )
-- , (select _tmpItem.OperCountCount from _tmpItem where _tmpItem.MovementItemId = 314931327 )

 , (select count(*) FROM _tmpItemSumm JOIN _tmpItem ON _tmpItemSumm.MovementItemId = _tmpItem.MovementItemId where _tmpItem.GoodsId = 2697 and _tmpItem.GoodsKindId = 8349)
 , (select sum (_tmpItem.OperCount) from _tmpItem WHERE _tmpItem.GoodsId = 2697 and _tmpItem.GoodsKindId = 8349)
 , (select sum (_tmpItemSumm.OperSumm) from _tmpItemSumm JOIN _tmpItem ON _tmpItemSumm.MovementItemId = _tmpItem.MovementItemId where _tmpItem.GoodsId = 2697 and _tmpItem.GoodsKindId = 8349)
 , (select sum (_tmpRemainsSumm.OperSumm_add) from _tmpRemainsSumm where _tmpRemainsSumm.GoodsId = 2697 and _tmpRemainsSumm.GoodsKindId = 8349 and _tmpRemainsSumm.OperSumm_add <> 0)
