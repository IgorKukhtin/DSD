...
/*
     ...
     CREATE TEMP TABLE _tmpItem (MovementItemId Integer
                               ...
                               , OperCount TFloat, OperCount_Partner TFloat, tmpOperSumm_Partner TFloat, OperSumm_Partner TFloat, OperSumm_Partner_ChangePercent TFloat
                               , ContainerId_ProfitLoss_10700 Integer, ContainerId_ProfitLoss_10800 Integer
     ...
     INSERT INTO _tmpItem (MovementItemId
                         ...
                         , OperCount, OperCount_Partner, tmpOperSumm_Partner, OperSumm_Partner, OperSumm_Partner_ChangePercent
                         , ContainerId_ProfitLoss_10700, ContainerId_ProfitLoss_10800
        ...
        SELECT
            ...
            , 0 AS !!del!!!ContainerId_ProfitLoss_10500
        ...
        FROM 
             (SELECT
                  ...
                  , COALESCE (MIFloat_AmountChangePercent.ValueData, 0) AS !!del!!!OperCount_ChangePercent
                  ...
                  , COALESCE (CAST (MIFloat_AmountPartner.ValueData * lfObjectHistory_PriceListItem.ValuePrice AS NUMERIC (16, 2)), 0) AS !!del!!!tmp!!del!!!OperSumm_PriceList
     ...
     SELECT -- Расчет Итоговой суммы прайс-листа по Контрагенту
            CASE WHEN vbPriceWithVAT_PriceList OR vbVATPercent_PriceList = 0
                    -- если цены с НДС или %НДС=0, тогда ничего не делаем
                    THEN _tmpItem.!!del!!!tmp!!del!!!OperSumm_PriceList
                 WHEN vbVATPercent_PriceList > 0
                    -- если цены без НДС, тогда получаем сумму с НДС
                    THEN CAST ( (1 + vbVATPercent_PriceList / 100) * _tmpItem.!!del!!!tmp!!del!!!OperSumm_PriceList AS NUMERIC (16, 2))
            END
         ...
         ,  CASE WHEN vbPriceWithVAT OR vbVATPercent = 0
                 ...
                 WHEN vbVATPercent > 0
                    ...
                    THEN CASE WHEN vbDiscountPercent > 0 THEN CAST ( (1 + vbVATPercent / 100) * CAST ( (1 - vbDiscountPercent/100) * _tmpItem.tmpOperSumm_Partner AS NUMERIC (16, 2)) AS NUMERIC (16, 2))
                         ...
                         END
            END
            INTO vb!!del!!!OperSumm_PriceList, vbOperSumm_Partner, vbOperSumm_Partner_ChangePercent
     FROM (SELECT SUM (_tmpItem.!!del!!!tmp!!del!!!OperSumm_PriceList) AS !!del!!!tmp!!del!!!OperSumm_PriceList
     ...
     SELECT SUM (_tmpItem.!!del!!!OperSumm_PriceList), SUM (_tmpItem.OperSumm_Partner), SUM (_tmpItem.OperSumm_Partner_ChangePercent) INTO vb!!del!!!OperSumm_PriceList_byItem, vbOperSumm_Partner_byItem, vbOperSumm_Partner_ChangePercent_byItem FROM _tmpItem;

     -- если не равны ДВЕ Итоговые суммы прайс-листа по Контрагенту
     IF COALESCE (vb!!del!!!OperSumm_PriceList, 0) <> COALESCE (vb!!del!!!OperSumm_PriceList_byItem, 0)
     THEN
         -- на разницу корректируем самую большую сумму (теоретически может получиться Значение < 0, но эту ошибку не обрабатываем)
         UPDATE _tmpItem SET !!del!!!OperSumm_PriceList = _tmpItem.!!del!!!OperSumm_PriceList - (vb!!del!!!OperSumm_PriceList_byItem - vb!!del!!!OperSumm_PriceList)
         WHERE _tmpItem.MovementItemId IN (SELECT MAX (_tmpItem.MovementItemId) FROM _tmpItem WHERE _tmpItem.!!del!!!OperSumm_PriceList IN (SELECT MAX (_tmpItem.!!del!!!OperSumm_PriceList) FROM _tmpItem)
                                          );
     END IF;
     ...
     -- для теста
     -- RETURN QUERY SELECT _tmpItem.MovementItemId, inMovementId, vbOperDate, _tmpItem.ContainerId_Goods, _tmpItem.GoodsId, _tmpItem.GoodsKindId, _tmpItem.AssetId, _tmpItem.PartionGoods, _tmpItem.PartionGoodsDate, _tmpItem.OperCount, _tmpItem.!!del!!!tmp!!del!!!OperSumm_PriceList, _tmpItem.!!del!!!OperSumm_PriceList, _tmpItem.tmpOperSumm_Partner, _tmpItem.OperSumm_Partner, _tmpItem.OperSumm_Partner_ChangePercent, _tmpItem.AccountId_Partner, _tmpItem.InfoMoneyDestinationId, _tmpItem.InfoMoneyId, _tmpItem.BusinessId_To, _tmpItem.isPartionCount, _tmpItem.isPartionSumm, _tmpItem.PartionGoodsId FROM _tmpItem;
     -- return;
     ...
     INSERT INTO _tmpItemSumm (MovementItemId, ContainerId_ProfitLoss_40208, !!del!!!ContainerId_ProfitLoss_10500, ContainerId_ProfitLoss_10800, ContainerId, AccountId, OperSumm, !!del!!!OperSumm_ChangePercent, OperSumm_Partner)
        SELECT
              _tmpItem.MovementItemId
            , 0 AS ContainerId_ProfitLoss_40208 -- Счет - прибыль (ОПиУ - разница в весе : с/с2 - с/с3)
            , 0 AS !!del!!!ContainerId_ProfitLoss_10500 -- Счет - прибыль (ОПиУ - скидки в весе : с/с1 - с/с2)
            , 0 AS ContainerId_ProfitLoss_10800 -- Счет - прибыль (ОПиУ - себестоимости реализации : с/с3)
            ...
            , SUM (ABS (_tmpItem.!!del!!!OperCount_ChangePercent * COALESCE (HistoryCost.Price, 0))) AS !!del!!!OperSumm_ChangePercent
        ...
        WHERE zc_isHistoryCost() = TRUE -- !!!если нужны проводки!!!
          AND (_tmpItem.OperCount * COALESCE (HistoryCost.Price, 0) <> 0                -- !!!
            OR _tmpItem.!!del!!!OperCount_ChangePercent * COALESCE (HistoryCost.Price, 0) <> 0  -- здесь нули !!!НЕ НУЖНЫ!!! 
     ...
     UPDATE _tmpItemSumm SET ContainerId_ProfitLoss_40208 = _tmpItem_byDestination.ContainerId_ProfitLoss_40208 -- Счет - прибыль (ОПиУ - разница в весе : с/с2 - с/с3)
                           , !!del!!!ContainerId_ProfitLoss_10500 = _tmpItem_byDestination.!!del!!!ContainerId_ProfitLoss_10500 -- Счет - прибыль (ОПиУ - скидки в весе : с/с1 - с/с2)
                           , ContainerId_ProfitLoss_10800 = _tmpItem_byDestination.ContainerId_ProfitLoss_10800 -- Счет - прибыль (ОПиУ - себестоимости реализации : с/с3)
     FROM _tmpItem
          JOIN
          (SELECT -- для учета разница в весе : с/с2 - с/с3
                ...
                , lpInsertFind_Container (inContainerDescId   := zc_Container_Summ()
                                        ...
                                        , inObjectId_1        := _tmpItem_byProfitLoss.ProfitLossId_ChangePercent
                                         ) AS !!del!!!ContainerId_ProfitLoss_10500
     ...
     INSERT INTO _tmpMIContainer_insert (Id, DescId, MovementId, MovementItemId, ContainerId, ParentId, Amount, OperDate, IsActive)
       ...
       FROM (SELECT _tmpItemSumm.ContainerId_ProfitLoss_40208 AS ContainerId_ProfitLoss
                  , SUM (_tmpItemSumm.!!del!!!OperSumm_ChangePercent - _tmpItemSumm.OperSumm_Partner) AS OperSumm
             FROM _tmpItemSumm
      ...
      UNION ALL
       ...
       SELECT 0, zc_MIContainer_Summ() AS DescId, inMovementId, 0 AS MovementItemId, _tmpItem_group.ContainerId_ProfitLoss, 0 AS ParentId, _tmpItem_group.OperSumm, vbOperDate, FALSE
       FROM (SELECT _tmpItemSumm.!!del!!!ContainerId_ProfitLoss_10500 AS ContainerId_ProfitLoss
                  , SUM (_tmpItemSumm.OperSumm - _tmpItemSumm.!!del!!!OperSumm_ChangePercent) AS OperSumm
             FROM _tmpItemSumm
             GROUP BY _tmpItemSumm.!!del!!!ContainerId_ProfitLoss_10500
            ) AS _tmpItem_group
       WHERE _tmpItem_group.OperSumm <> 0
      UNION ALL
     ...
     -- 4.1.1. создаем контейнеры для Проводки - Прибыль (Сумма возвратов и Скидка по акциям и Скидка дополнительная)
     UPDATE _tmpItem SET ContainerId_ProfitLoss_10700 = _tmpItem_byDestination.ContainerId_ProfitLoss_10700
                       ...
                       , !!del!!!ContainerId_ProfitLoss_10500 = _tmpItem_byDestination.!!del!!!ContainerId_ProfitLoss_10500
     FROM (SELECT -- для Сумма возвратов
           ...
           FROM (SELECT -- определяем ProfitLossId - для Сумма возвратов
                        CASE WHEN _tmpItem_group.ProfitLossGroupId = zc_Enum_ProfitLossGroup_10000() -- 10000; "Результат основной деятельности"
                              AND _tmpItem_group.InfoMoneyDestinationId_calc = zc_Enum_InfoMoneyDestination_20900() -- Ирна      -- select * from lfSelect_Object_InfoMoney() where InfoMoneyDestinationId = zc_Enum_InfoMoneyDestination_20900()
                                  THEN zc_Enum_ProfitLoss_10102() -- Сумма возвратов 10102; "Ирна"
                             WHEN _tmpItem_group.ProfitLossGroupId = zc_Enum_ProfitLossGroup_10000() -- 10000; "Результат основной деятельности"
                                  THEN zc_Enum_ProfitLoss_10101() -- Сумма возвратов 10101; "Продукция"
                             ELSE lpInsertFind_Object_ProfitLoss (inProfitLossGroupId      := _tmpItem_group.ProfitLossGroupId
                                                                , inProfitLossDirectionId  := _tmpItem_group.ProfitLossDirectionId
                                                                , inInfoMoneyDestinationId := _tmpItem_group.InfoMoneyDestinationId_calc
                                                                , inInfoMoneyId            := NULL
                 ...
                 FROM (SELECT -- здесь !!!тоже!!! что и для с/с (но из с/с нельзя брать т.к. может быть что с/с=0)
                            ...
                            , CASE WHEN vbMemberId_From = 0
                                    AND _tmpItem.InfoMoneyDestinationId IN (zc_Enum_InfoMoneyDestination_10100())  -- Мясное сырье -- select * from lfSelect_Object_InfoMoney() where InfoMoneyDestinationId = zc_Enum_InfoMoneyDestination_10100()
                                        THEN zc_Enum_ProfitLossDirection_10100() -- 10100; "Сумма возвратов"
                                   WHEN _tmpItem.InfoMoneyDestinationId IN (zc_Enum_InfoMoneyDestination_20900()  -- Ирна      -- select * from lfSelect_Object_InfoMoney() where InfoMoneyDestinationId = zc_Enum_InfoMoneyDestination_20900()
                                                                          , zc_Enum_InfoMoneyDestination_30100()) -- Продукция -- select * from lfSelect_Object_InfoMoney() where InfoMoneyDestinationId = zc_Enum_InfoMoneyDestination_30100()
                                        THEN zc_Enum_ProfitLossDirection_10100() -- 10100; "Сумма возвратов"
                                   WHEN vbMemberId_From <> 0
                       ...
                       FROM (SELECT  _tmpItem.InfoMoneyDestinationId
                                   ...
                                   , CASE WHEN _tmpItem.GoodsKindId = zc_GoodsKind_WorkProgress() THEN zc_InfoMoneyDestination_WorkProgress() ELSE _tmpItem.InfoMoneyDestinationId END AS InfoMoneyDestinationId_calc
                             FROM _tmpItem
                             WHERE _tmpItem.!!del!!!OperSumm_PriceList <> 0 OR _tmpItem.OperSumm_Partner <> 0 OR _tmpItem.OperSumm_Partner_ChangePercent <> 0
                             GROUP BY _tmpItem.InfoMoneyDestinationId
                                    , _tmpItem.BusinessId_To
                                    , _tmpItem.GoodsKindId
                            ) AS _tmpItem
     ...
     -- 4.1.2. формируются Проводки - Прибыль (Сумма возвратов и Скидка по акциям и Скидка дополнительная)
     INSERT INTO _tmpMIContainer_insert (Id, DescId, MovementId, MovementItemId, ContainerId, ParentId, Amount, OperDate, IsActive)
       -- Сумма возвратов
       SELECT 0, zc_MIContainer_Summ() AS DescId, inMovementId, 0 AS MovementItemId, _tmpItem_group.ContainerId_ProfitLoss, 0 AS ParentId, _tmpItem_group.OperSumm, vbOperDate, FALSE
       FROM (SELECT _tmpItem.ContainerId_ProfitLoss_10700 AS ContainerId_ProfitLoss
                  , -1 * SUM (_tmpItem.!!del!!!OperSumm_PriceList) AS OperSumm
             FROM _tmpItem
      ...
      UNION ALL
       -- Скидка по акциям
       SELECT 0, zc_MIContainer_Summ() AS DescId, inMovementId, 0 AS MovementItemId, _tmpItem_group.ContainerId_ProfitLoss, 0 AS ParentId, _tmpItem_group.OperSumm, vbOperDate, FALSE
       FROM (SELECT _tmpItem.ContainerId_ProfitLoss_10800 AS ContainerId_ProfitLoss
                  , 1 * SUM (_tmpItem.!!del!!!OperSumm_PriceList - _tmpItem.OperSumm_Partner) AS OperSumm
             FROM _tmpItem
             GROUP BY _tmpItem.ContainerId_ProfitLoss_10800
            ) AS _tmpItem_group
       WHERE _tmpItem_group.OperSumm <> 0
      UNION ALL
       -- Скидка дополнительная
       SELECT 0, zc_MIContainer_Summ() AS DescId, inMovementId, 0 AS MovementItemId, _tmpItem_group.ContainerId_ProfitLoss, 0 AS ParentId, _tmpItem_group.OperSumm, vbOperDate, FALSE
       FROM (SELECT _tmpItem.!!del!!!ContainerId_ProfitLoss_10500 AS ContainerId_ProfitLoss
                  , 1 * SUM (_tmpItem.OperSumm_Partner - _tmpItem.OperSumm_Partner_ChangePercent) AS OperSumm
             FROM _tmpItem
             GROUP BY _tmpItem.!!del!!!ContainerId_ProfitLoss_10500
            ) AS _tmpItem_group
       WHERE _tmpItem_group.OperSumm <> 0
       ;
     ...
     FROM (SELECT ABS (_tmpCalc.OperSumm) AS OperSumm
           ...
           FROM (SELECT _tmpItemSumm.MovementItemId
                      ...
                      , (_tmpItemSumm.!!del!!!OperSumm_ChangePercent - _tmpItemSumm.OperSumm_Partner) AS OperSumm -- !!!если>0, значит товар-кредит, т.е. у покупателя меньше чем отгрузка!!!
                 FROM _tmpItemSumm
                ) AS _tmpCalc
     ...
     FROM (SELECT ABS (_tmpCalc.OperSumm) AS OperSumm
           ...
           FROM (SELECT _tmpItemSumm.MovementItemId
                      , _tmpItemSumm.ContainerId
                      , _tmpItemSumm.AccountId
                      , _tmpItemSumm.!!del!!!ContainerId_ProfitLoss_10500 AS ContainerId_ProfitLoss
                      , zc_Enum_Account_100301 () AS AccountId_ProfitLoss   -- 100301; "прибыль текущего периода"
                      , (_tmpItemSumm.OperSumm - _tmpItemSumm.!!del!!!OperSumm_ChangePercent) AS OperSumm -- !!!по идее >0, значит товар-кредит!!!
                 FROM _tmpItemSumm
                ) AS _tmpCalc
           WHERE _tmpCalc.OperSumm <> 0
          ) AS _tmpItem_byProfitLoss
     ...
     FROM (SELECT ABS (_tmpCalc.OperSumm) AS OperSumm
           ...
           FROM (SELECT _tmpCalc_all.MovementItemId
                 ...
                 FROM (SELECT _tmpItem.MovementItemId
                            ...
                            , zc_Enum_Account_100301 () AS AccountId_ProfitLoss   -- 100301; "прибыль текущего периода"
                            , -1 * _tmpItem.!!del!!!OperSumm_PriceList AS OperSumm -- !!!минус, значит долг-дебет!!!
                       FROM _tmpItem
                      ) AS _tmpCalc_all
     ...
     FROM (SELECT ABS (_tmpCalc.OperSumm) AS OperSumm
           ...
           FROM (SELECT _tmpCalc_all.MovementItemId
                 ...
                 FROM (SELECT _tmpItem.MovementItemId
                            ...
                            , _tmpItem.ContainerId_Transit
                            , _tmpItem.AccountId_Transit
                            , _tmpItem.ContainerId_ProfitLoss_10800 AS ContainerId_ProfitLoss
                            , zc_Enum_Account_100301 () AS AccountId_ProfitLoss   -- 100301; "прибыль текущего периода"
                            , 1 * (_tmpItem.!!del!!!OperSumm_PriceList - _tmpItem.OperSumm_Partner) AS OperSumm -- !!!не минус, значит долг-кредит, т.е. уменьшаем его на скидку!!!
                       FROM _tmpItem
                      ) AS _tmpCalc_all
                      LEFT JOIN (SELECT vbOperDate AS OperDate UNION SELECT vbOperDatePartner AS OperDate) AS tmpOperDate ON tmpOperDate.OperDate = vbOperDate
                                                                                                                         OR  (tmpOperDate.OperDate = vbOperDatePartner
     ...
     FROM (SELECT ABS (_tmpCalc.OperSumm) AS OperSumm
           ...
           FROM (SELECT _tmpCalc_all.MovementItemId
                 ...
                 FROM (SELECT _tmpItem.MovementItemId
                            ...
                            , _tmpItem.ContainerId_Transit
                            , _tmpItem.AccountId_Transit
                            , _tmpItem.!!del!!!ContainerId_ProfitLoss_10500 AS ContainerId_ProfitLoss
                            , zc_Enum_Account_100301 () AS AccountId_ProfitLoss   -- 100301; "прибыль текущего периода"
