-- Function: gpSetErased_Movement_ProductionUnion (Integer, TVarChar)

DROP FUNCTION IF EXISTS gpSetErased_Movement_ProductionUnion (Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpSetErased_Movement_ProductionUnion(
...
BEGIN
     ...
     vbUserId:= lpCheckRight(inSession, zc_Enum_Process_SetErased_ProductionUnion());
...
-- тест
-- SELECT * FROM gpSetErased_Movement_ProductionUnion (inMovementId:= 149639, inSession:= zfCalc_UserAdmin())
