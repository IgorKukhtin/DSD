-- Function: lpSelect_Movement_Recalc_commerc (Integer, Integer, Integer, TDateTime, Integer)

DROP FUNCTION IF EXISTS lpSelect_Movement_Recalc_commerc (Integer, Integer, Integer, TDateTime, Integer);

CREATE OR REPLACE FUNCTION lpSelect_Movement_Recalc_commerc(
...
RETURNS TABLE (-- Договор (условие бонуса)
...
BEGIN
    ...
    , tmpContractCondition_all AS (SELECT Object_ContractCondition_View.ContractId
        ...
        , tmpContractCondition AS (SELECT tmpContractCondition_all.ContractId
                                   ...
                                   WHERE tmpContractCondition_all.PaidKindId_byBase = inPaidKindId
                                  )
    ...
    , tmpContract AS (SELECT tmpContractConditionKind.JuridicalId
        ...
        SELECT
              ...
              tmpContract.ContractId_master :: Integer AS ContractId_bonus
              -- Договор(начисление)
            , CASE WHEN tmpContract.InfoMoneyId_Condition <> 0 AND tmpContractBonus.ContractId_find > 0
              ...
              END :: Integer AS ContractId
              -- Вид Условия договора - Только % бонуса за отгрузку-возврат
            , tmpContract.ContractConditionKindID :: Integer AS ContractConditionKindId
              -- Вид бонуса
            , tmpContract.BonusKindId :: Integer AS BonusKindId
              -- Форма оплаты (Договор начисление)
            , tmpContract.PaidKindId_calc :: Integer AS PaidKindId
              --
            , CASE WHEN tmpContract.InfoMoneyId_Condition = zc_Enum_InfoMoney_21512() -- Маркетинг + Маркетинговый бюджет
              ...
              END :: Integer AS InfoMoneyId
              -- % бонуса
            , tmpContract.Value :: TFloat AS BonusValue
        ...
        WHERE tmpContract.ContractId_child      = inContractId
          AND tmpContract.PaidKindId_byBase     = inPaidKindId
...
-- тест
-- SELECT * FROM lpSelect_Movement_Recalc_commerc (inContractId:= 888997, inJuridicalId:= 862910, inPaidKindId:= zc_Enum_PaidKind_FirstForm(), inOperDate:= '01.07.2026', inUserId:= 5)
