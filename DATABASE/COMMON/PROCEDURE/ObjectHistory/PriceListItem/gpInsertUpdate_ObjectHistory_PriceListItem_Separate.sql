...
BEGIN
   ...
   vbPriceListId := (SELECT Object.Id FROM Object WHERE Object.DescId = zc_Object_PriceList() AND Object.ValueData Like '%расчет цен по дням - обвалка%');
   ...
   CREATE TEMP TABLE _tmpData (GoodsId Integer, OperDate TDateTime, Price TFloat) ON COMMIT DROP;
      ...
      WITH tmpMovement AS (SELECT Movement.Id        AS MovementId
         ...
         , tmpMIContainer AS (SELECT tmpMI_Child.MovementId                          AS MovementId
           ...
           SELECT tmp.GoodsId
                , tmp.OperDate
                , CAST (AVG (tmp.Summ / tmp.Amount) AS NUMERIC (16,2) ) ::TFloat  AS Price   -- средняя цена на дату
