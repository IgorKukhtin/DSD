-- Function: gpSelect_ObjectHistory_PricePlanItem ()

DROP FUNCTION IF EXISTS gpSelect_ObjectHistory_PricePlanItem (Integer, TDateTime, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_ObjectHistory_PricePlanItem(
...
BEGIN
   ...
   , tmpPrice AS (SELECT ObjectHistory_PricePlanItem.Id                   AS PricePlanItemId
                       , ObjectHistory_PricePlanItem.ObjectId             AS PricePlanItemObjectId
   ...
   , tmpData AS (-- сохраненные данные
                 SELECT COALESCE (tmpPrice.PricePlanItemId,0)       AS PricePlanItemId
                      , COALESCE (tmpPrice.PricePlanItemObjectId,0) AS PricePlanItemObjectId
       ...
       -- Результат
       SELECT
             tmpPrice.PricePlanItemId       AS Id
           , tmpPrice.PricePlanItemObjectId AS ObjectId

           , Object_PriceList.Id            AS PriceListId
           , Object_PriceList.ObjectCode    AS PriceListCode
           , Object_PriceList.ValueData     AS PriceListName

           , Object_Goods.Id                AS GoodsId     --ObjectLink_PricePlanItem_Goods.ChildObjectId AS GoodsId
       ...
       FROM tmpData AS tmpPrice
          ...
          LEFT JOIN ObjectDate AS ObjectDate_Protocol_Insert
                               ON ObjectDate_Protocol_Insert.ObjectId = tmpPrice.PricePlanItemObjectId
                              AND ObjectDate_Protocol_Insert.DescId = zc_ObjectDate_Protocol_Insert()
          LEFT JOIN ObjectDate AS ObjectDate_Protocol_Update
                               ON ObjectDate_Protocol_Update.ObjectId = tmpPrice.PricePlanItemObjectId
                              AND ObjectDate_Protocol_Update.DescId = zc_ObjectDate_Protocol_Update()

          LEFT JOIN ObjectLink AS ObjectLink_Insert
                               ON ObjectLink_Insert.ObjectId = tmpPrice.PricePlanItemObjectId
                              AND ObjectLink_Insert.DescId = zc_ObjectLink_Protocol_Insert()
          LEFT JOIN Object AS Object_Insert ON Object_Insert.Id = ObjectLink_Insert.ChildObjectId

          LEFT JOIN ObjectLink AS ObjectLink_Update
                               ON ObjectLink_Update.ObjectId = tmpPrice.PricePlanItemObjectId
   ...
   , tmpMinMax AS (SELECT tmp.PriceListId
       ...
       -- Результат
       SELECT
             ObjectHistory_PricePlanItem.Id AS Id
           , ObjectLink_PricePlanItem_PriceList.ObjectId
           ...
           , CASE WHEN COALESCE (ObjectBoolean_PriceWithVAT.ValueData, FALSE) = TRUE
                  THEN CAST (ObjectHistoryFloat_PricePlanItem_Value.ValueData - ObjectHistoryFloat_PricePlanItem_Value.ValueData * (COALESCE (ObjectFloat_VATPercent.ValueData, 0) / (COALESCE (ObjectFloat_VATPercent.ValueData, 0) + 100)) AS NUMERIC (16, 2))
                  ELSE ObjectHistoryFloat_PricePlanItem_Value.ValueData
             END ::TFloat AS PriceNoVAT

             -- расчет цены с НДС, до 2 знаков
           , CASE WHEN COALESCE (ObjectBoolean_PriceWithVAT.ValueData, FALSE) <> TRUE
                  THEN CAST ((ObjectHistoryFloat_PricePlanItem_Value.ValueData + ObjectHistoryFloat_PricePlanItem_Value.ValueData * (COALESCE (ObjectFloat_VATPercent.ValueData, 0) / 100)) AS NUMERIC (16, 2))
                  ELSE CAST (ObjectHistoryFloat_PricePlanItem_Value.ValueData AS NUMERIC (16, 2))
             END ::TFloat AS PriceWVAT

           , COALESCE (tmpMinMax.ValuePrice_min, 0) :: TFloat AS ValuePrice_min
           , COALESCE (tmpMinMax.ValuePrice_max, 0) :: TFloat AS ValuePrice_max

           , CAST (CASE WHEN tmpMinMax.ValuePrice_min > 0 AND ObjectHistoryFloat_PricePlanItem_Value.ValueData > 0
                             THEN 100 * (ObjectHistoryFloat_PricePlanItem_Value.ValueData - tmpMinMax.ValuePrice_min) / tmpMinMax.ValuePrice_min
                        WHEN COALESCE (tmpMinMax.ValuePrice_min, 0) = 0 AND ObjectHistoryFloat_PricePlanItem_Value.ValueData > 0
                             THEN 100
                        ELSE 0
                   END  AS NUMERIC (16,0))  :: TFloat AS Diff_min
           ...
           --
           , CASE WHEN Object_Measure.Id = zc_Measure_Sh() AND ObjectFloat_Weight.ValueData<>0
                     THEN (CASE WHEN COALESCE (ObjectBoolean_PriceWithVAT.ValueData, FALSE) = TRUE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) / (1 + COALESCE (ObjectFloat_VATPercent.ValueData, 0) / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END) / ObjectFloat_Weight.ValueData
                     ELSE (CASE WHEN COALESCE (ObjectBoolean_PriceWithVAT.ValueData, FALSE) = TRUE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) / (1 + COALESCE (ObjectFloat_VATPercent.ValueData, 0) / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END)
             END   :: TFloat AS ValuePrice_kg
           , CASE WHEN Object_Measure.Id = zc_Measure_Sh() AND ObjectFloat_Weight.ValueData<>0
                     THEN (CASE WHEN COALESCE (ObjectBoolean_PriceWithVAT.ValueData, FALSE) = FALSE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) * (1 + COALESCE (ObjectFloat_VATPercent.ValueData, 0) / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END) / ObjectFloat_Weight.ValueData
                     ELSE (CASE WHEN COALESCE (ObjectBoolean_PriceWithVAT.ValueData, FALSE) = FALSE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) * (1 + COALESCE (ObjectFloat_VATPercent.ValueData, 0) / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END)
             END   :: TFloat AS ValuePriceWithVAT_kg
       ...
       FROM ObjectLink AS ObjectLink_PricePlanItem_PriceList
            ...
            INNER JOIN Object AS Object_PriceList ON Object_PriceList.Id = ObjectLink_PricePlanItem_PriceList.ChildObjectId
          ...
          LEFT JOIN ObjectDate AS ObjectDate_Protocol_Insert
                             ON ObjectDate_Protocol_Insert.ObjectId = ObjectHistory_PricePlanItem.ObjectId
                            AND ObjectDate_Protocol_Insert.DescId = zc_ObjectDate_Protocol_Insert()
          LEFT JOIN ObjectDate AS ObjectDate_Protocol_Update
                             ON ObjectDate_Protocol_Update.ObjectId = ObjectHistory_PricePlanItem.ObjectId
                            AND ObjectDate_Protocol_Update.DescId = zc_ObjectDate_Protocol_Update()

          LEFT JOIN ObjectLink AS ObjectLink_Insert
                             ON ObjectLink_Insert.ObjectId = ObjectHistory_PricePlanItem.ObjectId
                            AND ObjectLink_Insert.DescId = zc_ObjectLink_Protocol_Insert()
          LEFT JOIN Object AS Object_Insert ON Object_Insert.Id = ObjectLink_Insert.ChildObjectId

          LEFT JOIN ObjectLink AS ObjectLink_Update
                             ON ObjectLink_Update.ObjectId = ObjectHistory_PricePlanItem.ObjectId
                            AND ObjectLink_Update.DescId = zc_ObjectLink_Protocol_Update()
          LEFT JOIN Object AS Object_Update ON Object_Update.Id = ObjectLink_Update.ChildObjectId

          LEFT JOIN tmpMinMax ON tmpMinMax.PriceListId              = Object_PriceList.Id
                             AND tmpMinMax.GoodsId                  = ObjectLink_PricePlanItem_Goods.ChildObjectId
                             AND COALESCE (tmpMinMax.GoodsKindId,0) = COALESCE (ObjectLink_PricePlanItem_GoodsKind.ChildObjectId,0)
       ...
       WHERE ObjectLink_PricePlanItem_PriceList.DescId = zc_ObjectLink_PricePlanItem_PriceList()
         ...
         AND (ObjectHistoryFloat_PricePlanItem_Value.ValueData > 0 OR inPriceListId > 0)
...
-- тест
-- SELECT * FROM gpSelect_ObjectHistory_PricePlanItem (zc_PriceList_ProductionSeparateHist() , CURRENT_TIMESTAMP, true, inSession:= zfCalc_UserAdmin())
--where goodsid =  2131
