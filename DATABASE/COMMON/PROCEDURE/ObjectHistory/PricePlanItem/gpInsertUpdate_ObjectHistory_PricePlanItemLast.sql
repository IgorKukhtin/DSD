-- Function: gpInsertUpdate_ObjectHistory_PricePlanItemLast (Integer, Integer, Integer, TDateTime, TFloat, Boolean, TVarChar)

DROP FUNCTION IF EXISTS gpInsertUpdate_ObjectHistory_PricePlanItemLast (Integer, Integer, Integer, Integer, TDateTime, TFloat, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpInsertUpdate_ObjectHistory_PricePlanItemLast(
...
BEGIN
   ...
   IF EXISTS (SELECT 1 FROM Object_Role_Process_View WHERE ProcessId = zc_Enum_Process_Update_PricePlanItem() AND UserId = vbUserId)
      OR vbUserId = zc_Enum_Process_Auto_PrimeCost()
   THEN
       -- разрешено ВСЕ
       vbPricePlanItemId:= 0;

   ELSEIF EXISTS (SELECT 1 FROM Object_MemberPriceList_View AS MemberPriceList_View WHERE MemberPriceList_View.UserId = vbUserId)
       OR 1=1
   THEN
