-- Function: gpInsertUpdate_ObjectHistory_PricePlanItemLast_zero (Integer, Integer, Integer, TDateTime, TFloat, Boolean, TVarChar)

DROP FUNCTION IF EXISTS gpInsertUpdate_ObjectHistory_PricePlanItemLast_zero (Integer, Integer, Integer, Integer, TDateTime, TFloat, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpInsertUpdate_ObjectHistory_PricePlanItemLast_zero(
...
BEGIN
   ...
   vbUserId:= lpCheckRight (inSession, zc_Enum_Process_InsertUpdate_OH_PricePlanItem_zero());


   -- если не назначена роль <Прайс-лист - изменение в любом прайсе>
   IF NOT EXISTS (SELECT 1 FROM Object_Role_Process_View WHERE ProcessId = zc_Enum_Process_Update_PricePlanItem() AND UserId = vbUserId)
   ...
   vbPricePlanItemId := lpGetInsert_Object_PricePlanItem (inPriceListId, inGoodsId, inGoodsKindId, vbUserId);
 
   -- Вставляем или меняем объект историю цен
   ioId := lpInsertUpdate_ObjectHistory (ioId, zc_ObjectHistory_PricePlanItem(), vbPricePlanItemId, inOperDate, vbUserId);
   -- Устанавливаем цену
   PERFORM lpInsertUpdate_ObjectHistoryFloat (zc_ObjectHistoryFloat_PricePlanItem_Value(), ioId, inValue);

   --
   IF inIsLast = TRUE AND EXISTS (SELECT Id FROM ObjectHistory WHERE DescId = zc_ObjectHistory_PricePlanItem() AND ObjectId = vbPricePlanItemId AND StartDate > inOperDate)
   THEN
         ...
         FROM ObjectHistory
              LEFT JOIN ObjectHistoryFloat AS ObjectHistoryFloat_Value
                                          ...
                                          AND ObjectHistoryFloat_Value.DescId = zc_ObjectHistoryFloat_PricePlanItem_Value()
         WHERE ObjectHistory.DescId = zc_ObjectHistory_PricePlanItem() AND ObjectHistory.ObjectId = vbPricePlanItemId AND ObjectHistory.StartDate > inOperDate;

         -- удалили
         DELETE FROM ObjectHistoryDate WHERE ObjectHistoryId IN (SELECT Id FROM ObjectHistory WHERE DescId = zc_ObjectHistory_PricePlanItem() AND ObjectId = vbPricePlanItemId AND StartDate > inOperDate);
         DELETE FROM ObjectHistoryFloat WHERE ObjectHistoryId IN (SELECT Id FROM ObjectHistory WHERE DescId = zc_ObjectHistory_PricePlanItem() AND ObjectId = vbPricePlanItemId AND StartDate > inOperDate);
         DELETE FROM ObjectHistoryString WHERE ObjectHistoryId IN (SELECT Id FROM ObjectHistory WHERE DescId = zc_ObjectHistory_PricePlanItem() AND ObjectId = vbPricePlanItemId AND StartDate > inOperDate);
         DELETE FROM ObjectHistoryLink WHERE ObjectHistoryId IN (SELECT Id FROM ObjectHistory WHERE DescId = zc_ObjectHistory_PricePlanItem() AND ObjectId = vbPricePlanItemId AND StartDate > inOperDate);
         DELETE FROM ObjectHistory WHERE Id IN (SELECT Id FROM ObjectHistory WHERE DescId = zc_ObjectHistory_PricePlanItem() AND ObjectId = vbPricePlanItemId AND StartDate > inOperDate);
   ...
   -- сохранили протокол
   PERFORM lpInsert_ObjectHistoryProtocol (inObjectId:= vbPricePlanItemId, inUserId:= vbUserId, inStartDate:= outStartDate, inEndDate:= outEndDate, inPrice:= inValue, inAddXML:= '', inIsUpdate:= TRUE, inIsErased:= FALSE);
