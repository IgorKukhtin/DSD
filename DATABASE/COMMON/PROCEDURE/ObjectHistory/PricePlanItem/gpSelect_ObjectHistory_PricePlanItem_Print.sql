-- Function: gpSelect_ObjectHistory_PricePlanItem_Print ()


DROP FUNCTION IF EXISTS gpSelect_ObjectHistory_PricePlanItem_Print (Integer, TDateTime, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_ObjectHistory_PricePlanItem_Print(
...
BEGIN
     ...
     RETURN QUERY
       SELECT
           ...
           , ObjectHistory_PricePlanItem.ObjectId
           ...
           , CASE WHEN vbPriceWithVAT_pl = TRUE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) / (1 + vbVATPercent_pl / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END :: TFloat  AS ValuePrice
           , CASE WHEN vbPriceWithVAT_pl = FALSE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) * (1 + vbVATPercent_pl / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END :: TFloat AS ValuePriceWithVAT

           , CASE WHEN Object_Measure.Id = zc_Measure_Sh() AND ObjectFloat_Weight.ValueData<>0
                     THEN (CASE WHEN vbPriceWithVAT_pl = TRUE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) / (1 + vbVATPercent_pl / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END) / ObjectFloat_Weight.ValueData
                     ELSE (CASE WHEN vbPriceWithVAT_pl = TRUE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) / (1 + vbVATPercent_pl / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END)
             END   :: TFloat AS ValuePrice_kg
           , CASE WHEN Object_Measure.Id = zc_Measure_Sh() AND ObjectFloat_Weight.ValueData<>0
                     THEN (CASE WHEN vbPriceWithVAT_pl = FALSE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) * (1 + vbVATPercent_pl / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END) / ObjectFloat_Weight.ValueData
                     ELSE (CASE WHEN vbPriceWithVAT_pl = FALSE THEN COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) * (1 + vbVATPercent_pl / 100) ELSE COALESCE (ObjectHistoryFloat_PricePlanItem_Value.ValueData, 0) END)
       ...
       FROM ObjectLink AS ObjectLink_PricePlanItem_PriceList
            ...
            LEFT JOIN Object AS Object_Goods
                             ON Object_Goods.Id = ObjectLink_PricePlanItem_Goods.ChildObjectId
       ...
       WHERE ObjectLink_PricePlanItem_PriceList.DescId = zc_ObjectLink_PricePlanItem_PriceList()
         ...
         AND (ObjectHistoryFloat_PricePlanItem_Value.ValueData <> 0 ) --OR ObjectHistory_PricePlanItem.StartDate <> zc_DateStart())
...
-- тест
-- SELECT * FROM gpSelect_ObjectHistory_PricePlanItem_Print (zc_PriceList_ProductionSeparate(), CURRENT_TIMESTAMP, inSession:= zfCalc_UserAdmin())
-- SELECT * FROM gpSelect_ObjectHistory_PricePlanItem_Print (zc_PriceList_Basis(), CURRENT_TIMESTAMP, inSession:= zfCalc_UserAdmin())
