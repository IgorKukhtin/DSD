-- Function: gpMovementItem_Visit_SetErased (Integer, Integer, TVarChar)

DROP FUNCTION IF EXISTS gpMovementItem_Visit_SetErased (Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpMovementItem_Visit_SetErased(
...
BEGIN
  ...
  -- vbUserId:= lpCheckRight(inSession, zc_Enum_Process_SetErased_MI_Visit());
...
-- тест
-- SELECT * FROM gpMovementItem_Visit_SetErased (inMovementId:= 55, inJuridicalId = 1, inSession:= '2')
