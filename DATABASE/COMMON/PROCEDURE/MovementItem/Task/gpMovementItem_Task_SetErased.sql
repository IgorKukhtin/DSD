-- Function: gpMovementItem_Task_SetErased (Integer, Integer, TVarChar)

DROP FUNCTION IF EXISTS gpMovementItem_Task_SetErased (Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpMovementItem_Task_SetErased(
...
BEGIN
  vbUserId:= lpCheckRight(inSession, zc_Enum_Process_SetErased_MI_Task());
...
-- тест
-- SELECT * FROM gpMovementItem_Task_SetErased (inMovementId:= 55, inJuridicalId = 1, inSession:= '2')
