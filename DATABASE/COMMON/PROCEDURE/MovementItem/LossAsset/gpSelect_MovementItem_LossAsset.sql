-- Function: gpSelect_MovementItem_LossAsset()

DROP FUNCTION IF EXISTS gpSelect_MovementItem_LossAsset (Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_MovementItem_LossAsset(
...
BEGIN
     ...
     -- vbUserId := PERFORM lpCheckRight (inSession, zc_Enum_Process_Select_MovementItem_LossAsset());
...
-- тест
-- SELECT * FROM gpSelect_MovementItem_LossAsset (inMovementId:= 25173, inIsErased:= FALSE, inSession:= '9818')
-- SELECT * FROM gpSelect_MovementItem_LossAsset (inMovementId:= 25173, inIsErased:= FALSE, inSession:= '2')
