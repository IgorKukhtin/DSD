...
BEGIN
     ...
     WITH -- данные из Модели для данного документа
          tmpObject AS (SELECT * FROM lpSelect_Object_SignInternalItem (vbSignInternalId, vbMovementDescId, vbObjectDescId, 0))
     ...
     SELECT inMovementId                      AS Id
          ...
          , COALESCE (tmpSignNo.SignInternalId, tmpSign.SignInternalId) :: Integer AS SignInternalId
            -- Пользователи - кто уже подписал
          , tmpSign.strSign       :: TVarChar AS strSign
            -- Пользователи - кто остался подписать
...
-- тест
-- SELECT * FROM lpSelect_MI_IncomeFuel_Sign (inMovementId:= 4136053)
