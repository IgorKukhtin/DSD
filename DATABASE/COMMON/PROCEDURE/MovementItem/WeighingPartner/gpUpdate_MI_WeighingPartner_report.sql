...
BEGIN
     ...
     vbCountTare1 := (SELECT MF.ValueData FROM MovementItemFloat AS MF WHERE MF.DescId = zc_MIFloat_CountTare1() AND MF.MovementItemId = inId);
     vbCountTare2 := (SELECT MF.ValueData FROM MovementItemFloat AS MF WHERE MF.DescId = zc_MIFloat_CountTare2() AND MF.MovementItemId = inId);
    ...
    IF COALESCE (inCountTare1,0) <> 0 AND COALESCE (inCountTare2,0) <> 0
     ...
     IF (COALESCE (inCountTare1,0) <> COALESCE (vbCountTare1)) AND (COALESCE (inCountTare2,0) <> COALESCE (vbCountTare2))
    AND COALESCE (inCountTare1,0) <> 0 AND COALESCE (inCountTare2,0) <> 0
     ...
     --Находим тару
     IF COALESCE (inCountTare1,0) <> COALESCE (vbCountTare1,0)
     THEN
         ...
         -- сохранили связь с <>
         PERFORM lpInsertUpdate_MovementItemLinkObject (zc_MILinkObject_Box1(), inId, CASE WHEN COALESCE (vbCountTare1,0) <> 0 THEN vbBoxId1 ELSE NULL END::Integer);
     END IF;

     IF COALESCE (inCountTare2,0) <> COALESCE (vbCountTare2,0)
     THEN
        ...
        -- сохранили связь с <>
        PERFORM lpInsertUpdate_MovementItemLinkObject (zc_MILinkObject_Box2(), inId, CASE WHEN COALESCE (vbCountTare2,0) <> 0 THEN vbBoxId2 ELSE NULL END::Integer);
     END IF;
