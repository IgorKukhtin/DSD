...
then
    RAISE EXCEPTION 'Ошибка.Повторите действие через 3 мин.';
     ...
     -- Нашли
     -- vbMovementId_sale:= (SELECT Movement.ParentId FROM Movement WHERE Movement.Id = inMovementId_sale);
...
-- тест
-- SELECT * FROM gpSelect_MI_WeighingPartner_PartionGoodsQ(inMovementId_sale := 29774297  , inShowAll := 'False' , inIsErased := 'False' ,  inSession := '9457');
