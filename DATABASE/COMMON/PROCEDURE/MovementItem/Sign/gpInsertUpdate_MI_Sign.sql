-- Function: gpInsertUpdate_MI_Sign()

DROP FUNCTION IF EXISTS gpInsertUpdate_MI_Sign (Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpInsertUpdate_MI_Sign(
...
BEGIN
     ...
     THEN
          RAISE EXCEPTION 'Ошибка.Нет информации для формирования электронной подписи в документе № <%> от <%>.', (SELECT InvNumber FROM Movement WHERE Id = inMovementId), zfConvert_DateToString ((SELECT OperDate FROM Movement WHERE Id = inMovementId));
     END IF;
     ...
     THEN -- если Подтвердил электронную подпись
         ...
         THEN
             RAISE EXCEPTION 'Ошибка.Пользователь <%> уже Подтвердил электронную подпись в документе № <%> от <%>.', lfGet_Object_ValueData (vbUserId), (SELECT InvNumber FROM Movement WHERE Id = inMovementId), zfConvert_DateToString ((SELECT OperDate FROM Movement WHERE Id = inMovementId));

         ELSEIF vbIndexNo = 0 -- если его НЕТ среди НЕ подписанных
         THEN
             RAISE EXCEPTION 'Ошибка.Пользователю <%> не назначена электронная подпись для документа № <%> от <%>.', lfGet_Object_ValueData (vbUserId), (SELECT InvNumber FROM Movement WHERE Id = inMovementId), zfConvert_DateToString ((SELECT OperDate FROM Movement WHERE Id = inMovementId));

         ELSEIF vbIndexNo <> 1 -- если НЕ первый среди НЕ подписанных
         THEN
             RAISE EXCEPTION 'Ошибка.Пользователь <%> сможет подтвердить электронную подпись для документа № <%> от <%> только после <%>.', lfGet_Object_ValueData (vbUserId), (SELECT InvNumber FROM Movement WHERE Id = inMovementId), zfConvert_DateToString ((SELECT OperDate FROM Movement WHERE Id = inMovementId))
     ...
     ELSE -- если Отменил электронную подпись
         ...
         THEN
             RAISE EXCEPTION 'Ошибка.Пользователь <%> уже Отменил электронную подпись в документе № <%> от <%>.', lfGet_Object_ValueData (vbUserId), (SELECT InvNumber FROM Movement WHERE Id = inMovementId), zfConvert_DateToString ((SELECT OperDate FROM Movement WHERE Id = inMovementId));

         ELSEIF vbIndex = 0 -- если его НЕТ среди подписанных
         THEN
             RAISE EXCEPTION 'Ошибка.Пользователю <%> не назначена отмена электронной подписи для  документа № <%> от <%>.', lfGet_Object_ValueData (vbUserId), (SELECT InvNumber FROM Movement WHERE Id = inMovementId), zfConvert_DateToString ((SELECT OperDate FROM Movement WHERE Id = inMovementId));

         ELSEIF zfCalc_Word_Split (vbStrIdSign, ',', vbIndex + 1) <> '' -- если НЕ последний среди подписанных
         THEN
             RAISE EXCEPTION 'Ошибка.Пользователь <%> сможет Отменить электронную подпись для документа № <%> от <%> только после <%>.', lfGet_Object_ValueData (vbUserId), (SELECT InvNumber FROM Movement WHERE Id = inMovementId), zfConvert_DateToString ((SELECT OperDate FROM Movement WHERE Id = inMovementId))
...
-- тест
-- SELECT * FROM gpInsertUpdate_MI_Sign (inMovementId:= 4136053, inIsSign:= TRUE, inSession:= '12894') -- Степаненко О.М.
-- SELECT * FROM gpInsertUpdate_MI_Sign (inMovementId:= 4136053, inIsSign:= FALSE, inSession:= '12894') -- Степаненко О.М.

-- SELECT * FROM gpInsertUpdate_MI_Sign (inMovementId:= 4136053, inIsSign:= TRUE, inSession:= '9463') -- Махота Д.П.
-- SELECT * FROM gpInsertUpdate_MI_Sign (inMovementId:= 4136053, inIsSign:= FALSE, inSession:= '9463') -- Махота Д.П.
