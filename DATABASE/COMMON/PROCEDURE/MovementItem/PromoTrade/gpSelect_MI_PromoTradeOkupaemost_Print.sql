...
BEGIN
    ...
    SELECT tmpData.RetailName
         ...
         , CASE WHEN tmpData.Num IN (1,2) THEN tmpData.Persent_Condition ELSE tmpData.Persent_Condition_new END  ::TFloat AS Persent_Condition    --Комм. условия:
         ...
         , CASE WHEN COALESCE (tmpData.PartnerCount,0) <> 0 THEN  tmpData.Summ / tmpData.PartnerCount ELSE 0 END ::TFloat AS Summ_one             --Стоимость ввода 1 SKU в 1ТТ:
         , tmpData.PartnerCount                                                                                                                   --кол-во ТТ
         , SUM (CASE WHEN tmpData.Num IN (1,2) THEN tmpData.SummPromo * 12 ELSE 0 END) ::TFloat AS SummPromo_1                                    --ИТОГО "Ожидаемый среднемесячный объем продаж, грн" вкладка "Окупаемость" * 12
         , SUM (CASE WHEN tmpData.Num IN (2,4) THEN tmpData.SummPromo * 12 ELSE 0 END) ::TFloat AS SummPromo_2
         , SUM (CASE WHEN tmpData.Num IN (1,2) THEN tmpData.Due_Pass_year1 ELSE 0 END) ::TFloat AS Due_Pass_year1                                 --ИТОГО "Ожидаемый проход1 в год, грн" вкладка "Окупаемость"
         , SUM (CASE WHEN tmpData.Num IN (2,4) THEN tmpData.Due_Pass_year2 ELSE 0 END) ::TFloat AS Due_Pass_year2
       
         , SUM (CASE WHEN tmpData.Num IN (1,2) THEN tmpData.TotalCost ELSE 0 END)      ::TFloat AS TotalCost_1                                    --Итого затрат, ГОД
         , SUM (CASE WHEN tmpData.Num IN (2,4) THEN tmpData.TotalCost ELSE 0 END)      ::TFloat AS TotalCost_2
         , SUM (CASE WHEN tmpData.Num IN (1,2) THEN tmpData.Profit1 ELSE 0 END)        ::TFloat AS Profit1                                        --Прибыль, грн год
         , SUM (CASE WHEN tmpData.Num IN (2,4) THEN tmpData.Profit2 ELSE 0 END)        ::TFloat AS Profit2
         , SUM (CASE WHEN tmpData.Num IN (1,2) THEN tmpData.PaybackPeriod1 ELSE 0 END) ::TFloat AS PaybackPeriod1                                 --Плановая окупаемость по сети, мес
