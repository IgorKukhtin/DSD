...
DROP FUNCTION IF EXISTS gpSelect_MovementItemChild_PriceCorrective (Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_MovementItemChild_PriceCorrective(
...
BEGIN
    ...
    -- Договор
    vbContractId:= (SELECT MLO.ObjectId 
                    FROM MovementLinkObject AS MLO 
                    WHERE MLO.MovementId = inMovementId 
                      AND MLO.DescId IN (zc_MovementLinkObject_Contract(), zc_MovementLinkObject_ContractFrom()));
...
-- SELECT * FROM gpSelect_MovementItemChild_PriceCorrective (inMovementId:= 3662505 ::Integer , inisErased := 'False'::Boolean , inSession := '5'::TVarChar);
