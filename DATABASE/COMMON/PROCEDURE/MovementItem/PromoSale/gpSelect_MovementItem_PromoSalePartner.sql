-- Function: gpSelect_MovementItem_PromoSalePartner()

DROP FUNCTION IF EXISTS gpSelect_MovementItem_PromoSalePartner (Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_MovementItem_PromoSalePartner(
...
BEGIN
    ...
    -- vbUserId := PERFORM lpCheckRight (inSession, zc_Enum_Process_Select_MovementItem_PromoSalePartner());
    vbUserId:= lpGetUserBySession (inSession);

    RETURN QUERY
        SELECT
            MI_PromoSalePartner.Id                     AS Id              --идентификатор
          , MI_PromoSalePartner.ObjectId               AS PartnerId       --ИД объекта <партнер>
        ...
        FROM Movement AS Movement_PromoSalePartner
            ...
            INNER JOIN Object AS Object_Partner
                              ON Object_Partner.Id = MI_PromoSalePartner.ObjectId
            LEFT OUTER JOIN ObjectLink AS ObjectLink_Partner_Juridical
                                       ON ObjectLink_Partner_Juridical.ObjectId = MI_PromoSalePartner.ObjectId
            ...
            LEFT OUTER JOIN ObjectLink AS ObjectLink_Partner_Area
                                       ON ObjectLink_Partner_Area.ObjectId = MI_PromoSalePartner.ObjectId
                                      AND ObjectLink_Partner_Area.DescId = zc_ObjectLink_Partner_Area()
            LEFT OUTER JOIN Object AS Object_Area
                                   ON Object_Area.Id = ObjectLink_Partner_Area.ChildObjectId
            LEFT JOIN MovementItemLinkObject AS MILinkObject_Contract
                                             ON MILinkObject_Contract.MovementItemId = MI_PromoSalePartner.Id
