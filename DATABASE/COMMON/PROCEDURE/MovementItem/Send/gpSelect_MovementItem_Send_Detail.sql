/*SELECT max (MovementItem.MovementId)/*MovementItem.Id
...
select * from Movement where id = 20081622
*/
...
BEGIN
       ...
       WITH tmpMI_Master AS (SELECT MovementItem.Id
         ...
         , tmpDiff AS (SELECT tmpMI_Master.Id
          ...
          , tmpAll AS (SELECT COALESCE (tmpMI_Detail.Id,0) AS Id
                            ...
                            , ROW_NUMBER () OVER (ORDER BY COALESCE (tmpMI_Detail.Id,999)) ::integer AS ord  --№ п.п.
                       FROM tmpMI_Master
     ...
     RETURN QUERY
       SELECT
           ...
           , ROW_NUMBER () OVER (ORDER BY MovementItem.Id) ::integer AS ord   --№ п.п.
...
--
 ...
 --SELECT * FROM gpSelect_MovementItem_Send_Detail (inMovementId:= 20081622, inShowAll:= TRUE, inIsErased:= FALSE, inSession:= '9818')