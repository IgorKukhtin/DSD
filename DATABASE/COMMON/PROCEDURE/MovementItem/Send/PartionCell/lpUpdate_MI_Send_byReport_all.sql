...
BEGIN
     ...
     INSERT INTO _tmpItem_PartionCell (MovementId, MovementItemId, Amount, DescId_MILO, PartionCellId, PartionCellId_new, isRePack, isMany)
       WITH -- 1 раз: дл€ ѕарти€ дата в строчной части - если установлена (тогда не смотрим в документ)
            tmpMI_PartionDate AS (SELECT MovementItem.MovementId                  AS MovementId
                                       ...
                                       , 0 AS PartionCellId_new
                                         -- Ќесколько партий в ячейке да/нет)
          ...
          , tmpMI AS (-- ≈сли ƒата парти€ - в документе
                     ...
                     UNION ALL
                      -- ≈сли ƒата парти€ - в строчной части
...
--    RAISE EXCEPTION 'ќшибка. <%>', (select count(*) from _tmpItem_PartionCell);
     ...
     THEN
           -- 0.ѕроверка
           IF     -- в этой €чейке-1 несколько партий
