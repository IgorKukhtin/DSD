...
--    RAISE EXCEPTION 'Ошибка. <%>', (select count(*) from _tmpItem_PartionCell);
     ...
     -- 1.13. если выполняется "обнулили ячейку"
     ...
     -- 1.14. если выполняется "обнулили ячейку"
     ...
     -- 1.15. если выполняется "обнулили ячейку"
     ...
     -- 1.16. если выполняется "обнулили ячейку"
     ...
     -- 1.17. если выполняется "обнулили ячейку"
     ...
     -- 1.18. если выполняется "обнулили ячейку"
     ...
     -- 1.19. если выполняется "обнулили ячейку"
     ...
     -- 1.20. если выполняется "обнулили ячейку"
     ...
     -- 1.21. если выполняется "обнулили ячейку"
     ...
     -- 1.22. если выполняется "обнулили ячейку"
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-2 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-2 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-2 несколько партий
           ...
           END IF;
            ...
            ;
             ...
             THEN
                 RAISE EXCEPTION 'Ошибка.Место хранение-2 не определено.%Нельзя поставить в отбор.% <%>', CHR (13), CHR (13)
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-3 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-3 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-3 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-4 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-4 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-4 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-5 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-5 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-5 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-6 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-6 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-6 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-7 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-7 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-7 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-8 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-8 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-8 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-9 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-9 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-9 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-10 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-10 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-10 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-11 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-11 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-11 несколько партий
     ...
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-12 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-12 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-12 несколько партий
     ...
     -- 2.13. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_13_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-13 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-13 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-13 несколько партий
     ...
     -- 2.14. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_14_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-14 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-14 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-14 несколько партий
     ...
     -- 2.15. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_15_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-15 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-15 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-15 несколько партий
     ...
     -- 2.16. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_16_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-16 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-16 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-16 несколько партий
     ...
     -- 2.17. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_17_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-17 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-17 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-17 несколько партий
     ...
     -- 2.18. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_18_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-18 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-18 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-18 несколько партий
     ...
     -- 2.19. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_19_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-19 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-19 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-19 несколько партий
     ...
     -- 2.20. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_20_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-20 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-20 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-20 несколько партий
     ...
     -- 2.21. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_21_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-21 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-21 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-21 несколько партий
     ...
     -- 2.22. если выполняется поставить ячейку В ОТБОР - zc_PartionCell_RK
     IF inPartionCellId_22_new = zc_PartionCell_RK()
     THEN
           -- 0.Проверка
           IF     -- в этой ячейке-22 несколько партий
           ...
           THEN
               RAISE EXCEPTION 'Ошибка.Для ячейки №-22 <%> %заполнено несколько партий с разной датой.%Необходимо сначала перенести в отбор ячейку <%>.'
                                -- в этой ячейке-22 несколько партий
