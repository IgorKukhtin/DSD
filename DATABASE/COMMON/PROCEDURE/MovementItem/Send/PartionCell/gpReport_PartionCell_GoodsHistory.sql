...
BEGIN
     ...
     RETURN QUERY
     WITH
      --все строки док. с указанной ячейкой
    ...
    , tmpCell_full AS (SELECT *
                       FROM (SELECT tmp1.*
                                  ...
                                  , tmp2.OperDate_new
                                  , tmp2.UserName_new
                                  , ROW_NUMBER () OVER (PARTITION BY tmp1.GoodsId, tmp1.GoodsKindName, tmp1.PartionGoodsDate ORDER BY tmp2.OperDate_new DESC) AS Ord_new
...
--inGoodsId := 2272 , inGoodsKindId := 0
...
WITH
      tmpMI AS (SELECT MovementItem.Id
                ...
                FROM MovementItemLinkObject AS MILinkObject_PartionCell_1
                     ...
                     INNER JOIN MovementLinkObject AS MovementLinkObject_To
                                                   ON MovementLinkObject_To.MovementId = Movement.Id
                                                  AND MovementLinkObject_To.DescId = zc_MovementLinkObject_To()
                                                  AND (MovementLinkObject_To.ObjectId = 8459 OR 8459 = 0)
               ...
               WHERE MILinkObject_PartionCell_1.ObjectId  = 10239508               --11347398 --10456344 --"R-1-4-2"
                 AND MILinkObject_PartionCell_1.DescId IN (zc_MILinkObject_PartionCell_1()
                                                          ,zc_MILinkObject_PartionCell_2()
                                                          ,zc_MILinkObject_PartionCell_3()
                                                          ,zc_MILinkObject_PartionCell_4()
                  ...
                  AND COALESCE (MovementBoolean_isRePack.ValueData, FALSE) = False
                )
      ...
      , tmpProtocol_All AS (SELECT *
                            ...
                            FROM (SELECT
                                       ...
                                       , REPLACE(REPLACE(CAST (XPATH ('/XML/Field[@FieldName = "Ячейка-21"] /@FieldValue', MovementItemProtocol.ProtocolData :: XML) AS TEXT), '{', ''), '}','')   AS PartionCell_21
                                       , REPLACE(REPLACE(CAST (XPATH ('/XML/Field[@FieldName = "Ячейка-22"] /@FieldValue', MovementItemProtocol.ProtocolData :: XML) AS TEXT), '{', ''), '}','')   AS PartionCell_22
                                       , CASE WHEN False = TRUE THEN MovementItemProtocol.MovementItemId ELSE 0 END AS MovementItemId
                                       , MovementItemProtocol.OperDate
                                       , MovementItemProtocol.UserId
                                       , tmpMI.PartionGoodsDate
                                       , CASE WHEN False = TRUE THEN tmpMI.MovementId ELSE 0 END      AS MovementId
                                       , CASE WHEN False = TRUE THEN tmpMI.InvNumber_full ELSE '' END AS InvNumber_full
                                  FROM MovementItemProtocol
                                       INNER JOIN tmpMI ON tmpMI.Id = MovementItemProtocol.MovementItemId
                                  WHERE MovementItemProtocol.OperDate >= ('01.12.2024')::TDateTime AND MovementItemProtocol.OperDate <=  ('14.03.2025')::TDateTime
                                  ) AS tmp
                                 LEFT JOIN Object AS Object_GoodsKind ON Object_GoodsKind.ValueData = tmp.GoodsKindName 
                            WHERE tmp.PartionCell_1  = 'N-5-3-2'
                               OR tmp.PartionCell_2  = 'N-5-3-2'
                               OR tmp.PartionCell_3  = 'N-5-3-2'
                               OR tmp.PartionCell_4  = 'N-5-3-2'
                               OR tmp.PartionCell_5  = 'N-5-3-2'
                               OR tmp.PartionCell_6  = 'N-5-3-2'
                               OR tmp.PartionCell_7  = 'N-5-3-2'
                               OR tmp.PartionCell_8  = 'N-5-3-2'
                               OR tmp.PartionCell_9  = 'N-5-3-2'
                               OR tmp.PartionCell_10 = 'N-5-3-2'
                               OR tmp.PartionCell_11 = 'N-5-3-2'
                               OR tmp.PartionCell_12 = 'N-5-3-2'
                               OR tmp.PartionCell_13 = 'N-5-3-2'
                               OR tmp.PartionCell_14 = 'N-5-3-2'
                               OR tmp.PartionCell_15 = 'N-5-3-2'
                               OR tmp.PartionCell_16 = 'N-5-3-2'
                               OR tmp.PartionCell_17 = 'N-5-3-2'
                               OR tmp.PartionCell_18 = 'N-5-3-2'
                               OR tmp.PartionCell_19 = 'N-5-3-2'
                               OR tmp.PartionCell_20 = 'N-5-3-2'
                               OR tmp.PartionCell_21 = 'N-5-3-2'
                               OR tmp.PartionCell_22 = 'N-5-3-2'
                            )
   ...
   , tmpCell_full AS 
    (SELECT *
                       FROM (SELECT tmp1.*
                             ...
                             FROM (SELECT *
                                   FROM tmpCell AS tmp1
                              WHERE tmp1.PartionCellName = 'N-5-3-2'
AND tmp1.Ord = 1
 AND  tmp1.Goodsid= 2272 ) AS tmp1                                  
                                  LEFT JOIN gpReport_PartionCell_history(tmp1.PartionGoodsDate::TDateTime
                                                                             , tmp1.GoodsId
                                                                             , COALESCE (tmp1.GoodsKindId,0)
                                                                             , 8459
                                                                             , False
                                                                             , False
                                                                             , '9457') AS tmp2 ON tmp2.PartionGoodsDate = tmp1.PartionGoodsDate
                                                                                                 AND tmp2.PartionCellName_old = 'N-5-3-2'
                                                                                                 AND tmp2.GoodsId = tmp1.GoodsId
                                                                                                 AND COALESCE (tmp2.GoodsKindName,'') = COALESCE (tmp1.GoodsKindName,'')
                                                                                                -- AND (COALESCE (tmp2.GoodsKindName,'') = COALESCE (tmp1.GoodsKindName,'') OR COALESCE (tmp1.GoodsKindId,0)=0)
 

                              ) AS tmp
                     --  WHERE tmp.Ord_new = 1
--where tmp.Goodsid= 2817546 
                      )

--inGoodsId := 9073793 , inGoodsKindId := 0 ,
/*
select *   
 FROM tmpCell AS tmp1
   WHERE tmp1.PartionCellName = 'N-5-3-2'
AND tmp1.Ord = 1
 AND  tmp1.Goodsid= 2272 
*/
--tmpCell_full

/*
select * from gpReport_PartionCell_history('05.12.2024'::TDateTime
                                                                             , 2272
                                                                             , 0
                                                                             , 8459
                                                                             , False
