-- Function: gpSelect_MovementItem_ReturnIn_commerc()

DROP FUNCTION IF EXISTS gpSelect_MovementItem_ReturnIn_commerc (Integer, Integer, TDateTime, Boolean, Boolean, TVarChar);


CREATE OR REPLACE FUNCTION gpSelect_MovementItem_ReturnIn_commerc(
...
RETURNS TABLE (Id Integer, LineNum Integer, GoodsId Integer, GoodsCode Integer, GoodsName TVarChar
             ...
             , Price TFloat, PriceIn TFloat, PriceEffie TFloat
...
BEGIN
       ...
       WITH -- Акции по товарам на дату
          ...
          , tmpMI AS (SELECT MovementItem.Id                               AS MovementItemId
                      ...
                      FROM (SELECT FALSE AS isErased UNION ALL SELECT inIsErased AS isErased WHERE inIsErased = TRUE) AS tmpIsErased
                           ...
                           LEFT JOIN MovementItemFloat AS MIFloat_PriceEffie
   ...
   , tmpMI_parent AS (SELECT MovementItem.ObjectId                         AS GoodsId
                      ...
                      GROUP BY MovementItem.ObjectId
                             ...
                             , COALESCE (MIFloat_PriceEffie.ValueData,0)
                     )
   , tmpMI_parent_find AS (SELECT tmp.MovementItemId
                                ...
                                , tmpMI_parent.PriceEffie
                                , tmpMI_parent.ChangePercent
                           FROM tmpMI_parent
                                LEFT JOIN (SELECT MAX (tmpMI.MovementItemId) AS MovementItemId, tmpMI.GoodsId, tmpMI.GoodsKindId, tmpMI.Price, tmpMI.PriceIn, tmpMI.PriceEffie
                                           FROM tmpMI
                                           WHERE tmpMI.isErased = FALSE
                                           GROUP BY tmpMI.GoodsId, tmpMI.GoodsKindId, tmpMI.Price, tmpMI.PriceIn, tmpMI.PriceEffie
   ...
   , tmpResult AS (SELECT tmpMI.MovementItemId
                        ...
                        , COALESCE (tmpMI.PriceEffie, tmpMI_parent_find.PriceEffie)   AS PriceEffie
          ...
          , tmpPriceListBasis AS (SELECT lfSelect.GoodsId     AS GoodsId
                                       , lfSelect.GoodsKindId AS GoodsKindId
                                       , lfSelect.StartDate
                                       --, CASE WHEN vbPriceWithVAT_plin = FALSE OR vbVATPercent_plin = 0 THEN lfSelect.ValuePrice ELSE lfSelect.ValuePrice / vbVATPercent_plin END AS Price_PriceList
                                       --, CASE WHEN vbPriceWithVAT_plin = TRUE  OR vbVATPercent_plin = 0 THEN lfSelect.ValuePrice ELSE lfSelect.ValuePrice * vbVATPercent_plin END AS Price_PriceList_vat
   ...
   , tmpStickerProperty AS (SELECT ObjectLink_Sticker_Goods.ChildObjectId              AS GoodsId
       ...
       FROM tmpGoods
            ...
            -- привязываем 2 раза вх. цены по виду товара и без
            LEFT JOIN tmpPriceListBasis ON tmpPriceListBasis.GoodsId = tmpGoods.GoodsId
                                    AND tmpPriceListBasis.GoodsKindId IS NULL
            LEFT JOIN tmpPriceListBasis AS tmpPriceListBasis_kind
                                     ON tmpPriceListBasis_kind.GoodsId = tmpGoods.GoodsId
                                    AND COALESCE (tmpPriceListBasis_kind.GoodsKindId,0) = COALESCE (tmpGoods.GoodsKindId,0)
      ...
      UNION ALL
       SELECT
           ...
           , tmpResult.PriceEffie    :: TFloat  AS PriceEffie
       ...
       FROM tmpResult
            ...
            -- привязываем 2 раза вх. цены по виду товара и без
            LEFT JOIN tmpPriceListBasis ON tmpPriceListBasis.GoodsId = tmpResult.GoodsId
                                    AND tmpPriceListBasis.GoodsKindId IS NULL
            LEFT JOIN tmpPriceListBasis AS tmpPriceListBasis_kind
                                     ON tmpPriceListBasis_kind.GoodsId = tmpResult.GoodsId
                                    AND COALESCE (tmpPriceListBasis_kind.GoodsKindId,0) = COALESCE (tmpResult.GoodsKindId,0)
