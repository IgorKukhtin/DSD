...
BEGIN
     ...
     -- !!!скидка - вес упаковки!!!
     IF vbIsBarCode = TRUE AND EXISTS (SELECT 1 FROM MovementLinkObject AS MLO WHERE MLO.MovementId = inMovementId AND MLO.DescId = zc_MovementLinkObject_From() AND MLO.ObjectId = zc_Unit_RK())
     THEN
