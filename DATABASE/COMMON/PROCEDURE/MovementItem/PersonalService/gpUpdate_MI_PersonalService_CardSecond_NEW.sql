...
BEGIN
     ...
     INSERT INTO _tmpMI (MovementItemId, PersonalId, UnitId, PositionId, InfoMoneyId, PersonalServiceListId, SummCardSecondRecalc)
           WITH -- все Сотрудники из vbPersonalServiceListId - по ним и будет формироваться Инфа
             ...
             , tmpPersonal2 AS (SELECT DISTINCT
                                       tmpPersonal_all.PersonalId
                                ...
                                FROM tmpPersonal_all
                                  -- INNER JOIN Object AS Object_PersonalServiceList ON Object_PersonalServiceList.DescId = zc_Object_PersonalServiceList()
                               UNION
                                SELECT DISTINCT
             ...
             , tmpPersonal AS (
                                SELECT DISTINCT
                                     ...
                                     , tmpMI.UnitId
                                     , tmpMI.PositionId
                                     , tmpMI.InfoMoneyId  -- 60101 Заработная плата
                                     , CLO_PersonalServiceList.ObjectId AS PersonalServiceListId
                          FROM tmpPersonal2 AS tmpMI
