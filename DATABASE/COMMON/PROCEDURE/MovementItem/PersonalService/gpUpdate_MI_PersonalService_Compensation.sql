...
BEGIN
     ...
     WITH 
        ...
        , tmpReport AS (SELECT tmpMI.MovementItemId
                          ...
                          FROM tmpMI
                               LEFT JOIN gpReport_HolidayCompensation (inStartDate:= vbOperDate
                                                                     , inUnitId:= tmpMI.UnitId
                                                                     , inMemberId:= tmpMI.MemberId
                                                                     , inSession:= inSession) AS tmp 
                                                                                              ON tmp.MemberId = tmpMI.MemberId
                                                                                             AND tmp.PersonalId = tmpMI.PersonalId
                                                                                             AND tmp.PositionId = tmpMI.PositionId
                                                                                             AND tmp.UnitId = tmpMI.UnitId
                          WHERE tmpMI.Ord = 1
                         )
     ...
     PERFORM lpInsertUpdate_MovementItemFloat (zc_MIFloat_SummCompensation(), _tmpMI.MovementItemId, (_tmpMI.Day_diff * _tmpMI.AmountCompensation))  --сумма компенсации
           ...
           , lpInsertUpdate_MovementItemFloat (zc_MIFloat_PriceCompensation(), _tmpMI.MovementItemId, _tmpMI.AmountCompensation)                     -- Ср. ЗП за день
     FROM _tmpMI
     WHERE COALESCE (_tmpMI.Day_diff,0) > 0;
