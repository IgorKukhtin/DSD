...
BEGIN
     ...
     -- если Заполнение дата предварительный план = НЕТ
     IF EXISTS (SELECT 1 FROM ObjectBoolean AS OB WHERE OB.ObjectId  = vbOrderFinanceId AND OB.DescId = zc_ObjectBoolean_OrderFinance_OperDate() AND OB.ValueData = FALSE)
     THEN
         -- схема - Мясное сырье
        ...
        ;
         ...
         THEN
             RAISE EXCEPTION 'Ошибка.Для группы УП = <Говядина>%введено <Согласовано к оплате> = <%>.%Не может быть больше лимита суммы = <%>.'
                            , CHR (13)
         ...
         THEN
             RAISE EXCEPTION 'Ошибка.Для группы УП = Для <Живой вес>%введено <Согласовано к оплате> = <%>.%Не может быть больше лимита суммы = <%>.'
                            , CHR (13)
         ...
         THEN
             RAISE EXCEPTION 'Ошибка.Для группы УП = <Прочее мясное сырье>%введено <Согласовано к оплате> = <%>.%Не может быть больше лимита суммы = <%>.'
                            , CHR (13)
     ...
     -- если Заполнение № заявки 1С = НЕТ
     ELSEIF EXISTS (SELECT 1 FROM ObjectBoolean AS OB WHERE OB.ObjectId  = vbOrderFinanceId AND OB.DescId = zc_ObjectBoolean_OrderFinance_InvNumber() AND OB.ValueData = FALSE)
        -- если Заполнение № счета = НЕТ
        AND EXISTS (SELECT 1 FROM ObjectBoolean AS OB WHERE OB.ObjectId  = vbOrderFinanceId AND OB.DescId = zc_ObjectBoolean_OrderFinance_InvNumber_Invoice() AND OB.ValueData = FALSE)
     THEN

           -- 2. схема - Сырье, упаковочные и расходные материалы
        ...
        ;
         ...
         -- Только когда Сумма заполнена - делаем проверку
         IF vbSum_1 > 0
         THEN
             -- Результат - после InsertUpdate
             SELECT tmpMI.Sum_1_sum, tmpMI.Sum_2_sum
                    INTO vbSum_1_sum, vbSum_2_sum
             FROM (-- MI
                   ...
                   -- Результат
                   SELECT -- Платежный план на неделю
                          SUM (COALESCE (MIFloat_AmountPlan_next.ValueData, 0)
                              ) AS Sum_1_sum
                          -- Согласовано к оплате
                        , SUM (COALESCE (MIFloat_AmountPlan_1.ValueData, 0) + COALESCE (MIFloat_AmountPlan_2.ValueData, 0) + COALESCE (MIFloat_AmountPlan_3.ValueData, 0) + COALESCE (MIFloat_AmountPlan_4.ValueData, 0) + COALESCE (MIFloat_AmountPlan_5.ValueData, 0)
                              ) AS Sum_2_sum
               ...
               IF vbSum_1_sum > vbSum_1
               THEN
                   RAISE EXCEPTION 'Ошибка.Заполнено <Платежный план на неделю> = <%>.%Не может быть больше лимита суммы = <%>.'
                                  , zfConvert_FloatToString (vbSum_1_sum)
                                  , CHR (13)
                                  , zfConvert_FloatToString (vbSum_1)
                                   ;
               ...
               -- Проверка-2
               IF vbSum_2_sum > vbSum_1
               THEN
                   RAISE EXCEPTION 'Ошибка.Заполнено <Согласовано к оплате> = <%>.%Не может быть больше лимита суммы = <%>.'
                                  , zfConvert_FloatToString (vbSum_2_sum)
                                  , CHR (13)
                                  , zfConvert_FloatToString (vbSum_1)
                                   ;
         ...
         END IF; -- IF vbSum_1 > 0 - Только когда Сумма заполнена - делаем проверку
...
-- тест
-- SELECT * FROM lpCheck_Movement_OrderFinance (inMovementId:= 33510045, inUserId:= 5)
