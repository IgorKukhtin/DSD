-- Function: gpGet_Object_CommercLocal()

DROP FUNCTION IF EXISTS gpGet_Object_CommercLocal(Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpGet_Object_CommercLocal(
...
$BODY$BEGIN
   ...
   -- PERFORM lpCheckRight(inSession, zc_Enum_Process_Get_Object_CommercLocal());
   ...
   THEN
       ...
       SELECT
           ...
           , lfGet_ObjectCode(0, zc_Object_CommercLocal()) AS Code
   ...
   ELSE
     ...
     FROM Object AS Object_CommercLocal
       ...
       WHERE Object_CommercLocal.DescId = zc_Object_CommercLocal()
         AND Object_CommercLocal.Id = inId;
   END IF;
...
-- тест
-- SELECT * FROM gpGet_Object_CommercLocal(0, '2')