    ...
    RETURN QUERY
       SELECT tmp.Number
            ...
            , CASE WHEN inLevelChild = 'TareWeight'
                        THEN tmp.Number
                   WHEN inLevelChild = 'ChangePercent'
                        THEN tmp.Number
       ...
       FROM (SELECT tmp.Number
                  , gpGet_ToolsWeighing_Value ('Scale_' || inScaleNum, inLevelChild, '', inLevelChild || CASE WHEN inLevelChild = 'PriceList' THEN 'Id' ELSE '' END || '_' || CASE WHEN tmp.Number < 10 THEN '0' ELSE '' END || tmp.Number, '0', inSession) AS Value
             FROM (SELECT GENERATE_SERIES (1, vbCount) AS Number) AS tmp
            ) AS tmp
      ...
      UNION ALL
       SELECT 1000 AS Number
            ...
            , '0' :: TVarChar AS Value
       FROM gpGet_ToolsWeighing_Value ('Scale_' || inScaleNum, 'Default', '', 'isTareWeightEnter', 'FALSE', inSession) AS tmp
