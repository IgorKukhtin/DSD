...
BEGIN
     ...
     IF EXISTS (SELECT ObjectLink_UserRole_View.UserId FROM ObjectLink_UserRole_View WHERE RoleId IN (zc_Enum_Role_Admin(), 428386) AND ObjectLink_UserRole_View.UserId = vbUserId)
     THEN vbUserId:= 0;
     END IF;
