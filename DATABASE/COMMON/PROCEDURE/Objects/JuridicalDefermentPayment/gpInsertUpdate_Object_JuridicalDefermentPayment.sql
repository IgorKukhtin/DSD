                     ...
                     FROM
                         (SELECT Object_Juridical.Id                        AS JuridicalId
                               ...
                               , ObjectLink_Contract_PaidKind.ChildObjectId AS PaidKindId
                                 -- поставщики
                               , CASE WHEN ObjectLink_Contract_InfoMoney.ChildObjectId IN (zc_Enum_InfoMoney_30101() -- √отова€ продукци€
                                                                                         , zc_Enum_InfoMoney_30102() -- “ушенка
                                                                                         , zc_Enum_InfoMoney_30201() -- ћ€сное сырье
                                                                                         , zc_Enum_InfoMoney_30301() -- ћ€сное сырье
                                                                                          )
                                           THEN FALSE
                                      WHEN tmpPost.JuridicalId IS NULL
                                           THEN FALSE
                                      ELSE TRUE
                                 END AS isIncome
                          ...
                          WHERE Object_Juridical.DescId = zc_Object_Juridical()
                            ...
                            -- убрали «акрытые
                            AND ObjectLink_Contract_ContractStateKind.ChildObjectId IS NULL
                         ...
                         FROM (SELECT tmp.OperDate
                               ...
                               FROM (SELECT MIContainer.OperDate
                                     ...
                                     FROM ContainerLinkObject AS CLO_Juridical
                                          ...
                                          INNER JOIN (SELECT DISTINCT tmpJuridical.JuridicalId, tmpJuridical.ContractId, tmpJuridical.isIncome, tmpJuridical.PaidKindId
                                                      FROM tmpJuridical_4 AS tmpJuridical
                                                     ) AS tmpJuridical
                                                       ON tmpJuridical.JuridicalId = CLO_Juridical.ObjectId
                                                      AND tmpJuridical.ContractId  = CLO_Contract.ObjectId
                                                      AND tmpJuridical.PaidKindId  = CLO_PaidKind.ObjectId
...
-- тест
-- SELECT * FROM gpInsertUpdate_Object_JuridicalDefermentPayment (inStartDate:= '01.09.2025', inEndDate:= CURRENT_DATE, inSession:= '9457');
