...
BEGIN
   ...
   IF EXISTS (SELECT 1 FROM ObjectLink_UserRole_View WHERE UserId = vbUserId AND RoleId IN (409476 )) -- Логист - Днепр
   THEN
       -- сохранили связь с <>
   ...
   ELSE
       IF EXISTS (SELECT 1 FROM ObjectLink_UserRole_View WHERE UserId = vbUserId AND RoleId IN (97837)) -- Бухгалтер
       THEN
           -- сохранили свойство <>
