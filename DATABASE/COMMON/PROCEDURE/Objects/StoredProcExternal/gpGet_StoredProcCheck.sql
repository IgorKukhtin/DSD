...
BEGIN
      ...
      -- 1. за большой период
      IF zfConvert_StringToDate (inValue1) < zfConvert_StringToDate (inValue2) - INTERVAL '3 MONTH'
         --AND TRIM (inStoredProc) ILIKE 'gpSelect_Report_Wage%'
          AND TRIM (inStoredProc) NOT ILIKE '%gpSelect%'
      THEN
          RETURN TRUE;

      -- проверка если сильное опоздание
      ELSEIF EXISTS (SELECT 1 FROM _replica.table_update_data
      ...
      ELSE
          ...
          ELSEIF vbEndDate <= (CASE WHEN TRIM (inStoredProc) ILIKE 'gpSelect_Movement_Service'
                                   ...
                                   --
                                   ELSE CURRENT_DATE - INTERVAL '5 DAY'
                               END)
