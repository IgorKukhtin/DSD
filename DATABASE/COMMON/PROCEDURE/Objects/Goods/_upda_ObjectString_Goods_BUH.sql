with tmpAll_1 as (select Object_Goods.*
   ...
   , tmpAll_2 as (select tmpAll_1.*, objectProtocol.UserId, objectProtocol.OperDate, objectProtocol.ProtocolData
                       , REPLACE(REPLACE(REPLACE(CAST (XPATH ('/XML/Field[@FieldName = "Значение"]/@FieldValue', objectProtocol.ProtocolData :: XML) AS TEXT), '{', ''), '}',''), '"','') AS GoodsName_xml
...
-- select * from tmpAll_check  

--
select tmpAll_1.Id, tmpAll_1.ValueData AS GoodName_curr, tmpAll_2.GoodsName_xml, tmpAll_2.OperDate
