...
--   PERFORM lpCheckRight(inSession, zc_Enum_Process_JuridicalGroup());
   ...
   PERFORM lpCheck_Object_CycleLink(ioId, zc_ObjectLink_JuridicalGroup_JuridicalGroup(), inJuridicalGroupId);

   -- Вставляем объект
   ioId := lpInsertUpdate_Object(ioId, zc_Object_JuridicalGroup(), inCode, inName);
   -- Вставляем ссылку
   PERFORM lpInsertUpdate_ObjectLink(zc_ObjectLink_JuridicalGroup_JuridicalGroup(), ioId, inJuridicalGroupId);
