...
-- SELECT * FROM gpInsertUpdate_HistoryCost (inStartDate:= '01.12.2019', inEndDate:= '25.12.2019', inBranchId:= 0 /*филиал Львов*/, inItearationCount:= 50, inInsert:= -1, inDiffSumm:= 1, inSession:= '2') WHERE ContainerId in (2459386, 2459377) -- ORDER BY ABS (Price) DESC -- Price <> PriceNext-- WHERE CalcSummCurrent <> CalcSummNext
