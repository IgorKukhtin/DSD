...
--  RETURNS VOID
  RETURNS TABLE (vbItearation Integer, vbCountDiff Integer, Price TFloat, PriceNext TFloat, Price_external TFloat, PriceNext_external TFloat, FromContainerId Integer
               , ContainerId Integer
               , GoodsId Integer, GoodsKindId Integer, InfoMoneyId Integer, InfoMoneyId_Detail Integer, AccountId Integer, JuridicalId_basis Integer
               , isInfoMoney_80401 Boolean
               , CalcSummCurrent TFloat, CalcSummNext TFloat, CalcSummCurrent_external TFloat, CalcSummNext_external TFloat
     ...
     INSERT INTO _tmpUnit_branch (UnitId)
        ...
        WHERE COALESCE (inBranchId, 0) <= 0
          ...
          AND ObjectLink_Unit_Branch.ChildObjectId IN (8374    -- 4. филиал Одесса
                                                     ...
                                                     , 13257082-- Филиал Казахстан
                                                     , 13261282-- Филиал Европа
                                                      )
      UNION
       SELECT ObjectLink_Unit_Branch.ObjectId AS UnitId
        FROM ObjectLink AS ObjectLink_Unit_Branch
...
END IF;
         ...
         */
         ...
         THEN
             ...
             THEN
                 IF EXTRACT (HOUR FROM CLOCK_TIMESTAMP()) < 8 AND EXTRACT (HOUR FROM CLOCK_TIMESTAMP()) > 0
                 THEN
                     DELETE FROM historycost_2024_12_3;
                 ELSE
                     truncate table historycost_2024_12_3;
                 END IF;

             END IF;
             -- сохранили для теста
             INSERT INTO historycost_2024_12_3
                SELECT * FROM HistoryCost -- WHERE ((StartDate BETWEEN inStartDate AND inEndDate) OR (EndDate BETWEEN inStartDate AND inEndDate))
    ...
    RAISE INFO ' start - 2 del .<%>', CLOCK_TIMESTAMP();
             ...
             -- для теста
             IF EXTRACT (HOUR FROM CLOCK_TIMESTAMP()) <= 21
             THEN
                 -- удалили для теста
                 truncate table historycost_2024_12_1;
                 -- сохранили для теста
                 INSERT INTO historycost_2024_12_1
             ...
             ELSE
                 -- удалили для теста
                 truncate table historycost_2024_12_2;
                 -- сохранили для теста
                 INSERT INTO historycost_2024_12_2
                    SELECT * FROM HistoryCost
                    WHERE HistoryCost.StartDate   = inStartDate
                      AND HistoryCost.ContainerId IN (SELECT DISTINCT _tmpHistoryCost_del.ContainerId FROM _tmpHistoryCost_del)
                     ;
