     ...
     RETURN QUERY
        ...
        tmpMovement AS (SELECT Movement.*
          ...
          , tmpJuridical AS (SELECT ObjectLink_Partner_Juridical.ObjectId AS PartnerId
                             ...
                             WHERE ObjectLink_Partner_Juridical.ObjectId IN (SELECT DISTINCT tmpMLO.ObjectId FROM tmpMLO WHERE tmpMLO.DescId = zc_MovementLinkObject_To() AND inFileName ILIKE 'ПН%'
                                                                            UNION 
                                                                             SELECT DISTINCT tmpMLO.ObjectId FROM tmpMLO WHERE tmpMLO.DescId = zc_MovementLinkObject_From() AND inFileName ILIKE 'ВН%'
                                                                            )
           ...
           SELECT CASE WHEN tmpData.Ord = 0
                       THEN 
                            CASE WHEN inFileName ILIKE 'ПН%' THEN 'ПН'
                                 WHEN inFileName ILIKE 'ВН%' THEN 'ВН'
                                 ELSE ''
                            END
                     ||';'||tmpData.InvNumber
