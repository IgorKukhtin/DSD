...
-- Contract
         ...
         LEFT JOIN ObjectString AS ObjectString_Partner_GLNCodeCorporate_vch
                               ...
                               -- Дільниця обліку і реалізації м`ясної сировини + Склад МИНУСОВКА
                               AND MovementLinkObject_From.ObjectId IN (133049
                                                                      , 8445
                                                                       )
 ...
 , tmpMI AS (SELECT MovementItem.ObjectId AS GoodsId
    ...
    , tmpUKTZED AS (SELECT tmp.GoodsGroupId, lfGet_Object_GoodsGroup_CodeUKTZED_onDate (tmp.GoodsGroupId, vbOperDatePartner) AS CodeUKTZED
      ...
      SELECT COALESCE (Object_GoodsByGoodsKind_View.Id, Object_Goods.Id) AS Id
           ...
           , CASE WHEN 1=1 OR COALESCE (tmpObject_GoodsPropertyValueGroup.ArticleGLN, COALESCE (tmpObject_GoodsPropertyValue.ArticleGLN, '')) <> ''
                  THEN COALESCE (tmpObject_GoodsPropertyValueGroup.ArticleGLN, COALESCE (tmpObject_GoodsPropertyValue.ArticleGLN, ''))
                  ELSE COALESCE (tmpObject_GoodsPropertyValueGroup.Article,    COALESCE (tmpObject_GoodsPropertyValue.Article, ''))
             END AS ArticleGLN_Juridical
