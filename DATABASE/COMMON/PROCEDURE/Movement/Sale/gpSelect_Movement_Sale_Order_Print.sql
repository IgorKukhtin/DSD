...
SELECT MovementItem.ObjectId AS GoodsId 
     ...
     FROM MovementLinkMovement AS MovementLinkMovement_Order
       ...
       SELECT Object_Goods.ObjectCode  			 AS GoodsCode
           ...
           , tmpOrder.Count :: TFloat	                 AS OrderCount
           , tmpOrder.AmountSecond :: TFloat	         AS OrderCountSecond
           
           , CASE WHEN (SUM (MovementItem.Amount)-COALESCE(tmpOrder.Count,0)-COALESCE(tmpOrder.AmountSecond,0))>0 
                  THEN (SUM (MovementItem.Amount)-COALESCE(tmpOrder.Count,0)-COALESCE(tmpOrder.AmountSecond,0))
                  ELSE 0 END                    ::TFloat AS CountDiff_B

           , CASE WHEN (SUM (MovementItem.Amount)-COALESCE(tmpOrder.Count,0)-COALESCE(tmpOrder.AmountSecond,0))<0 
                  THEN (SUM (MovementItem.Amount)-COALESCE(tmpOrder.Count,0)-COALESCE(tmpOrder.AmountSecond,0))
                  ELSE 0 END                    ::TFloat AS CountDiff_M
       ...
       FROM MovementItem
            ...
            LEFT JOIN tmpOrder ON tmpOrder.GoodsId = MovementItem.ObjectId
        ...
        WHERE MovementItem.MovementId = inMovementId
          ...
          AND MovementItem.Amount <> 0
           ...
           , tmpWeighing.GoodsId
           , tmpWeighing.Count
           , tmpOrder.AmountSecond, tmpOrder.Count
           , Object_GoodsKind.Id             
