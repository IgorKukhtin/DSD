...
--         , Movement.InvNumber                         AS InvNumber
          ...
          -- , COALESCE (Object_Partner.ValueData, Object_To.ValueData) AS ToName
           ...
           , CASE -- !!!захардкодил временно для Запорожье!!!
                  WHEN MovementLinkObject_From.ObjectId IN (301309) -- Склад ГП ф.Запорожье
                   AND vbPaidKindId = zc_Enum_PaidKind_SecondForm()
                       THEN FALSE
                  WHEN COALESCE (ObjectLink_Contract_JuridicalDoc_Next.ChildObjectId, ObjectLink_Contract_JuridicalDocument.ChildObjectId) > 0
           ...
           , CASE WHEN View_Contract.InvNumber ILIKE '%обмен%' OR View_Contract.InvNumber ILIKE '%обмін%' THEN TRUE ELSE FALSE END :: Boolean AS isPrintText
