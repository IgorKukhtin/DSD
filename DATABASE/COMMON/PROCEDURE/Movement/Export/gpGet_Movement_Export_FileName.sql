...
BEGIN
     ...
     -- Результат
     SELECT CASE WHEN ObjectLink_PersonalServiceList_Bank.ChildObjectId = 76968 -- ПАТ "БАНК ВОСТОК"
                      THEN 'Vostok_'
                 WHEN ObjectLink_PersonalServiceList_Bank.ChildObjectId = 76970 -- ПАТ "ОТП БАНК"
                      THEN 'OTP_'
            END AS FileNamePrefix
          , CASE WHEN ObjectLink_PersonalServiceList_Bank.ChildObjectId = 76968 -- ПАТ "БАНК ВОСТОК"
                      THEN '.txt'
                 WHEN ObjectLink_PersonalServiceList_Bank.ChildObjectId = 76970 -- ПАТ "ОТП БАНК"
                      THEN '.xml'
                 ELSE '.csv'
