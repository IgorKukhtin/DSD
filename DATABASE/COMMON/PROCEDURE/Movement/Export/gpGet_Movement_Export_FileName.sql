...
BEGIN
     ...
     -- Результат
     SELECT CASE WHEN ObjectLink_PersonalServiceList_Bank.ChildObjectId IN (76968, 6314382) -- АТ VST BANK + ПАТ "БАНК ВОСТОК" -- 76968
          ...
          , CASE WHEN ObjectLink_PersonalServiceList_Bank.ChildObjectId IN (76968, 6314382) -- АТ VST BANK + ПАТ "БАНК ВОСТОК"
                  AND ObjectLink_PersonalServiceList_PSLExportKind.ChildObjectId = zc_Enum_PSLExportKind_iBank()
                      THEN '.txt'
                 WHEN ObjectLink_PersonalServiceList_PSLExportKind.ChildObjectId = zc_Enum_PSLExportKind_iBank()
                      THEN '.txt'
