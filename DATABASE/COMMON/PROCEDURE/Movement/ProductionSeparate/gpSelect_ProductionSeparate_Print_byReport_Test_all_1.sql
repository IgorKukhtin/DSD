-- Function: gpSelect_ProductionSeparate_Print_byReport_Test_all_1()

DROP FUNCTION IF EXISTS gpSelect_ProductionSeparate_Print_byReport_Test_all_1 (TDateTime, TDateTime, Integer, Integer, Integer, Integer, Integer, Boolean, Boolean,  TVarChar, TVarChar, TVarChar);
DROP FUNCTION IF EXISTS gpSelect_ProductionSeparate_Print_byReport_Test_all_1 (TDateTime, TDateTime, Integer, Integer, Integer, Integer, Integer, Boolean, Boolean, Boolean,  TVarChar, TVarChar, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_ProductionSeparate_Print_byReport_Test_all_1(
    ...
    IN inisAll              Boolean   , -- FALSE - по 1 поставщику, TRUE - по всем поставщикам ,
    IN inisDetail           Boolean   , -- FALSE - свернуть все, TRUE - развернуть детально ,  
    IN inPartionGoods_main  TVarChar  , --главная партия 
    IN inPartionGoods_main2 TVarChar  , --партия для группировки
...
BEGIN
    ...
    IF COALESCE (inisAll,False) = FALSE
    THEN
        inPartionGoods_main := LEFT (inPartionGoods_main ::TVarChar, length (inPartionGoods_main)-11 );
    ELSE 
        inPartionGoods_main := LEFT (inPartionGoods_main ::TVarChar, 4);
        inPartionGoods_main2:= LEFT (inPartionGoods_main2::TVarChar, 7);
    END IF;
   ...
   WITH 
                          ...
                          UNION
                           SELECT Movement.Id                           AS MovementId
                            ...
                            WHERE COALESCE (inMovementId,0) = 0
                              ...
                              AND ((MovementString_PartionGoods.ValueData ILIKE '%'||inPartionGoods_main||'%'  AND inisGroup = TRUE) 
                                -- OR (MovementString_PartionGoods.ValueData ILIKE '%'||vbPartionGoods_main||'%'  AND inisGroup = TRUE) 
                                 OR inisGroup = FALSE)
                       )
    ...
    , tmpMI_MasterALL AS (
     ...
     , tmpMovement AS (SELECT *
                       ...
                       WHERE (tmp.PartionGoods_main2 = inPartionGoods_main2 AND inisAll = false) 
                          OR (tmp.PartionGoods_main2 ILIKE '%'||inPartionGoods_main2||'%' AND inisAll = TRUE)
     ...
     , tmpGroupAll AS (SELECT * 
                       ...
                       FROM (SELECT CASE WHEN inisGroup = TRUE THEN 0 ELSE tmpResult.MovementId END AS MovementId
                                  , CASE WHEN inisDetail = FALSE THEN '' ELSE tmpResult.PartionGoods_main END AS PartionGoods_main
     ...
     , tmpGroup AS (SELECT tmp.MovementId
         ...
         --группировака данных шапки печати по документам или по гл. партии      CASE WHEN inisDetail = TRUE THEN
     , tmpMain_Group AS (
                         WITH 
                         tmp AS (SELECT tmpCursor1.MovementId
                         ...
                         SELECT tmp.MovementId
                              ...
                              , tmp.SummIncome
                              , tmp.HeadCountIncome
                              , tmp.CountPackerIncome
                              , tmp.AmountPartnerIncome
                              , tmp.HeadCount1
                              , tmp.PriceIncome
                              , tmp.PriceIncome1
                              , tmp.PriceIncome2
                        ...
                        UNION 
                         SELECT '' ::TVarChar                  AS MovementId
                              ...
                              , SUM (tmp.HeadCountIncome)      AS HeadCountIncome
                              , SUM (tmp.CountPackerIncome)    AS CountPackerIncome
                              , SUM (tmp.AmountPartnerIncome)  AS AmountPartnerIncome
                              , AVG (tmp.HeadCount1)           AS HeadCount1
                              , AVG (tmp.PriceIncome)          AS PriceIncome 
                              , AVG (tmp.PriceIncome1)         AS PriceIncome1
                              , AVG (tmp.PriceIncome2)         AS PriceIncome2
                              , SUM (tmp.SummCostIncome)       AS SummCostIncome
                              , SUM (tmp.CountDocIncome)       AS CountDocIncome
                              , SUM (tmp.Count_CountPacker)    AS Count_CountPacker
