...
-- Function: lpSelect_Movement_PromoSalePartner_Detail()

DROP FUNCTION IF EXISTS lpSelect_Movement_PromoSalePartner_Detail (Integer);

CREATE OR REPLACE FUNCTION lpSelect_Movement_PromoSalePartner_Detail(
...
BEGIN
     RETURN QUERY
        WITH tmpPartner_all AS 
                      (SELECT Movement_PromoSalePartner.Id                           AS MovementId
...
-- тест
-- SELECT * FROM lpSelect_Movement_PromoSalePartner_Detail (inMovementId:= 2641111) AS tmp LEFT JOIN Object AS Object_Partner ON Object_Partner.Id = tmp.PartnerId LEFT JOIN Object_Contract_InvNumber_View AS Object_Contract ON Object_Contract.ContractId = tmp.ContractId
