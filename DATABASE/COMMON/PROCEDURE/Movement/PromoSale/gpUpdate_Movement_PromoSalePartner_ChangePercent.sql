-- Function: gpUpdate_Movement_PromoSalePartner_ChangePercent()

DROP FUNCTION IF EXISTS gpUpdate_Movement_PromoSalePartner_ChangePercent (Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpUpdate_Movement_PromoSalePartner_ChangePercent(
...
BEGIN
     ...
     --vbUserId:= lpCheckRight (inSession, zc_Enum_Process_Update_Movement_PromoSale_ChangePercent());
     vbUserId:= lpCheckRight (inSession, zc_Enum_Process_InsertUpdate_Movement_PromoSale());

     -- zc_MovementFloat_ChangePercent - тянуть  из договоров zc_Movement_PromoSalePartner, если дог не указан - будет 0, если несколько дог, то скидка должна быть одинаковой
     -- врем. таблица
...
-- тест
-- SELECT * FROM gpUpdate_Movement_PromoSalePartner_ChangePercent (inMovementId:= 2641111, inSession:= zfCalc_UserAdmin())
