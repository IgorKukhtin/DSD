-- Function: gpInsert_Movement_TaxCorrective_isAutoPrepay()

DROP FUNCTION IF EXISTS gpInsert_Movement_TaxCorrective_isAutoPrepay (TDateTime, TDateTime, TVarChar);

CREATE OR REPLACE FUNCTION gpInsert_Movement_TaxCorrective_isAutoPrepay (
...
--РК на ПП:
--1.Если с-до кон. больше 0, а с-до нач. меньше 0, то сумма НН на ПП= модуль с-до нач.
--2.Если с-до кон. меньше 0, и с-до нач. меньше 0, и модуль с-до кон.меньше модуля с-до нач., то сумма НН на ПП= модуль с-до нач. минус модуль с-до кон.
