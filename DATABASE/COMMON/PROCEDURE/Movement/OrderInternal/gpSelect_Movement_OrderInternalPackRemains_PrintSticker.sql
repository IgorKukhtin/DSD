...
BEGIN
   ...
   , tmpData AS (SELECT *
                 FROM (SELECT tmpMI.MovementItemId
                            ...
                            , tmpMI.UpdateDate
                            , tmpMI.Amount
                            , COALESCE (tmpMI.AmountPackAllTotal,0)  AS AmountPackAllTotal 
                            , (COALESCE (tmpMI.AmountPackAllTotal,0) - COALESCE (tmpMI_2.AmountPackAllTotal,0) ) AS AmountPack_diff
                ...
                WHERE (((tmp.Amount = tmp.Amount_max AND inAmount = 0)
                      OR (tmp.Amount = inAmount AND inAmount <> 0) 
                        ) AND COALESCE (tmp.AmountPack_diff,0) <> 0) 
                     OR (inisAll = TRUE AND tmp.Amount = tmp.Amount_max)   -- печать все - это в последней строке итого
                 )
   ...
   , tmpMILO AS (SELECT MovementItemLinkObject.*
       ...
       FROM tmpData 
            ...
            LEFT JOIN MovementItemLinkObject AS MILinkObject_GoodsComplete
                                             ON MILinkObject_GoodsComplete.MovementItemId = tmpMI_Master.Id
            ...
            LEFT JOIN MovementItemLinkObject AS MILinkObject_GoodsKindComplete
                                             ON MILinkObject_GoodsKindComplete.MovementItemId = tmpMI_Master.Id
