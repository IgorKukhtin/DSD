...
DROP FUNCTION IF EXISTS gpGet_Movement_QualityDoc (Integer, Integer, TDateTime, TVarChar);
...
BEGIN
     ...
     THEN
         inMovementId:= (SELECT MovementChildId FROM MovementLinkMovement WHERE MovementId = inMovementId_Sale AND DescId = zc_MovementLinkMovement_QualityDoc());
     END IF;

     -- если надо - создаем
     IF COALESCE (inMovementId, 0) = 0
     THEN inMovementId:= lpInsertUpdate_Movement_QualityDoc (ioId              := inMovementId
                                                               , inInvNumber       := NEXTVAL ('Movement_QualityDoc_seq') :: TVarChar
     ...
     RETURN QUERY 
       ...
       FROM Movement
            ...
            LEFT JOIN MovementLinkMovement AS MovementLinkMovement_QualityDoc
                                           ON MovementLinkMovement_QualityDoc.MovementChildId = Movement.Id 
                                          AND MovementLinkMovement_QualityDoc.DescId = zc_MovementLinkMovement_QualityDoc()

            LEFT JOIN Movement AS Movement_Sale ON Movement_Sale.Id = MovementLinkMovement_QualityDoc.MovementId
                                               AND Movement_Sale.StatusId = zc_Enum_Status_Complete()
            LEFT JOIN MovementLinkObject AS MovementLinkObject_From
                                         ON MovementLinkObject_From.MovementId = MovementLinkMovement_QualityDoc.MovementId
                                        AND MovementLinkObject_From.DescId = zc_MovementLinkObject_From()
            LEFT JOIN Object AS Object_From ON Object_From.Id = MovementLinkObject_From.ObjectId

            LEFT JOIN MovementLinkObject AS MovementLinkObject_To
                                         ON MovementLinkObject_To.MovementId = MovementLinkMovement_QualityDoc.MovementId
...
ALTER FUNCTION gpGet_Movement_QualityDoc (Integer, Integer, TDateTime, TVarChar) OWNER TO postgres;
...
-- тест
-- SELECT * FROM gpGet_Movement_QualityDoc (inMovementId:= 1, inMovementId_Sale:= 2, inOperDate:= '01.01.2015', inSession:= zfCalc_UserAdmin())
