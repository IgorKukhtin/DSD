...
--           , MovementString_InvNumberPartnerEDI.ValueData  AS InvNumberPartnerEDI
     ...
     , tmpObject_GoodsPropertyValue_basis AS
       ...
       SELECT
           ...
           , (CASE WHEN vbDocumentTaxKindId = zc_Enum_DocumentTaxKind_Prepay() THEN 'ПРЕДОПЛАТА ЗА КОЛБ.ИЗДЕЛИЯ' ELSE COALESCE (tmpObject_GoodsPropertyValue.Name, COALESCE (tmpObject_GoodsPropertyValue_basis.Name, Object_Goods.ValueData || CASE WHEN COALESCE (Object_GoodsKind.Id, zc_Enum_GoodsKind_Main()) = zc_Enum_GoodsKind_Main() THEN '' ELSE ' ' || Object_GoodsKind.ValueData END)) END) :: TVarChar AS GoodsName
           , (CASE WHEN vbDocumentTaxKindId = zc_Enum_DocumentTaxKind_Prepay() THEN 'ПРЕДОПЛАТА ЗА КОЛБ.ИЗДЕЛИЯ' ELSE COALESCE (tmpObject_GoodsPropertyValue.Name, COALESCE (tmpObject_GoodsPropertyValue_basis.Name, Object_Goods.ValueData)) END) :: TVarChar AS GoodsName_two
