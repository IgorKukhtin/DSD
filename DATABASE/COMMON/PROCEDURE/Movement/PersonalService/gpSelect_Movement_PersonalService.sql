     ...
     WITH tmpStatus AS (SELECT zc_Enum_Status_Complete()   AS StatusId
        ...
        , tmpMemberPersonalServiceList
                     AS (SELECT Object_PersonalServiceList.Id AS PersonalServiceListId
                        ...
                        UNION
                         ...
                         FROM Object AS Object_PersonalServiceList
                         WHERE Object_PersonalServiceList.DescId = zc_Object_PersonalServiceList()
                           AND EXISTS (SELECT 1 FROM Object_RoleAccessKeyGuide_View WHERE UserId = vbUserId AND AccessKeyId_PersonalService IN (zc_Enum_Process_AccessKey_PersonalServiceAdmin(), zc_Enum_Process_AccessKey_PersonalServiceProduction()))
                        UNION
        ...
        , tmpMovement AS (SELECT Movement.Id
                          ...
                          WHERE inIsServiceDate = FALSE
                            -- Волошина Е.А.
                            AND ((tmpRoleAccessKey.AccessKeyId > 0 AND vbUserId <> 140094) AND tmpMemberPersonalServiceList.PersonalServiceListId > 0)
