...
if inUserId <> zc_Enum_Process_Auto_PrimeCost() OR 1=0
...
ELSE
    ...
    ;
     ...
     --
     vbStartDate_calc:= CASE WHEN vbStartDate < vbStartDate_60 THEN vbStartDate ELSE vbStartDate_60 END;
     vbEndDate_calc  := CASE WHEN vbEndDate   > vbEndDate_60   THEN vbEndDate   ELSE vbEndDate_60   END;
  ...
  , tmpMI_promo_all AS (SELECT MI_PromoGoods.Id AS MovementItemId
      ...
      , tmpMI_promo AS (SELECT -- т.е. если установлено "для всех GoodsKindId" - запишем в него, иначе получится дублирование
        ...
        , tmpMI_sale AS (SELECT tmpMI_promo.MovementItemId AS MovementItemId_promo
                              , SUM (CASE WHEN tmpMovement_sale.OperDatePartner BETWEEN vbStartDate AND vbEndDate THEN COALESCE (MIFloat_AmountPartner.ValueData, 0) ELSE 0 END) AS AmountPartner
                              , SUM (CASE WHEN tmpMovement_sale.OperDatePartner BETWEEN vbStartDate AND vbEndDate AND MIFloat_PromoMovement.MovementItemId > 0 THEN COALESCE (MIFloat_AmountPartner.ValueData, 0) ELSE 0 END) AS AmountPartnerPromo
                                --
   ...
   , tmpMI_ReturnIn AS (SELECT tmpMI_promo.MovementItemId AS MovementItemId_promo
                             , SUM (CASE WHEN tmpMovement_ReturnIn.OperDatePartner BETWEEN vbStartDate AND vbEndDate THEN COALESCE (MIFloat_AmountPartner.ValueData, 0) ELSE 0 END) AS AmountPartner
                             , SUM (CASE WHEN tmpMovement_ReturnIn.OperDatePartner BETWEEN vbStartDate AND vbEndDate AND MIFloat_PromoMovement.MovementItemId > 0 THEN COALESCE (MIFloat_AmountPartner.ValueData, 0) ELSE 0 END) AS AmountPartnerPromo
                               --
                ...
                SELECT MIN (tmp.tmpId) + MIN (tmp.tmpId_2)
                FROM (SELECT -- 1.1. Объем продаж в аналогичный период, кг (итого)
                             CASE WHEN inUserId = zc_Enum_Process_Auto_PrimeCost() AND 1=1 THEN 0
                             ELSE
                             CASE lpInsertUpdate_MovementItemFloat (inDescId        := zc_MIFloat_AmountReal()
                                                                  , inMovementItemId:= tmpMI_promo_all.MovementItemId
                                                                  , inValueData     := COALESCE (tmpMI_sale.AmountPartner, 0)
                                                                   )
                                   WHEN TRUE THEN 0
                             ...
                             -- 1.2. Объем продаж 60 дней, кг (итого)
                     ...
                     UNION
                      SELECT -- 2.1. Объем Акционных продаж в аналогичный период, кг
                             CASE WHEN inUserId = zc_Enum_Process_Auto_PrimeCost() AND 1=1 THEN 0
                             ELSE
                             CASE lpInsertUpdate_MovementItemFloat (inDescId        := zc_MIFloat_AmountRealPromo()
                                                                  , inMovementItemId:= tmpMI_promo_all.MovementItemId
                                                                  , inValueData     := COALESCE (tmpMI_sale.AmountPartnerPromo, 0)
                                                                   )
                                   WHEN TRUE THEN 0
                             ...
                             -- 2.2. Объем Акционных продаж 60 дней, кг
                     ...
                     UNION
                      SELECT -- 3.1. Кол-во возврат в аналогичный период, кг
                             CASE lpInsertUpdate_MovementItemFloat (inDescId        := zc_MIFloat_AmountRetIn()
                                                                  , inMovementItemId:= tmpMI_promo_all.MovementItemId
                                                                  , inValueData     := COALESCE (tmpMI_ReturnIn.AmountPartner, 0)
                                                                   )
                                   WHEN TRUE THEN 0
