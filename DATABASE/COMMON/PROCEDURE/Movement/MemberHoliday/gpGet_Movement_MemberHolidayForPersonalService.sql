...
RETURNS TABLE (MemberId Integer, MemberName TVarChar
             , Day_holiday TFloat 
             , Day_holiday1 TFloat
             , Day_holiday2 TFloat
    ...
    END  IF;
     ...
     RETURN QUERY 
       WITH 
        ...
        , tmpData AS (SELECT
                           ...
                           , CASE WHEN DATE_TRUNC ('Month', MovementDate_BeginDateEnd.ValueData) <> DATE_TRUNC ('Month', MovementDate_BeginDateStart.ValueData)
                                  THEN (DATE_PART ('DAY', DATE_TRUNC ('Month',MovementDate_BeginDateEnd.ValueData - INTERVAL '1 DAY') :: TIMESTAMP - MovementDate_BeginDateStart.ValueData :: TIMESTAMP)) 
                                  ELSE (DATE_PART ('DAY', MovementDate_BeginDateEnd.ValueData :: TIMESTAMP - MovementDate_BeginDateStart.ValueData :: TIMESTAMP) + 1)
                             END AS Day_holiday1

                           , CASE WHEN DATE_TRUNC ('Month', MovementDate_BeginDateEnd.ValueData) <> DATE_TRUNC ('Month', MovementDate_BeginDateStart.ValueData) 
                                  THEN (DATE_PART ('DAY', MovementDate_BeginDateEnd.ValueData :: TIMESTAMP - DATE_TRUNC ('Month',MovementDate_BeginDateEnd.ValueData:: TIMESTAMP) ) + 1) 
                                  ELSE 0
                             END AS Day_holiday2  
       ...
       , tmpMovementPersonalService AS (SELECT Movement.Id
                                        ...
                                        WHERE MovementDate.ValueData BETWEEN (SELECT DISTINCT tmpData.ServiceDateStart FROM tmpData) AND (SELECT DISTINCT tmpData.ServiceDateEnd FROM tmpData)
       ...
       SELECT  tmpData.MemberId
             ...
             /*tmpData.Id
             , tmpData.OperDateStart     --период начисления
             , tmpData.OperDateEnd

             , tmpData.BeginDateStart    -- период отпуска
             , tmpData.BeginDateEnd
             ...
             , tmpData.MovementId_PersonalService1
             , tmpData.MovementId_PersonalService2
             , tmpData.SummHoliday1
             , tmpData.SummHoliday2
             , tmpData.TotalSummHoliday  
             , tmpData.SummHoliday_calc
             , tmpData.Amount  --Ср.ЗП за день
       ...
       FROM tmpData 
            ...
            LEFT JOIN tmpPersonalService AS tmpPersonalService2 
                                         ON tmpPersonalService2.ServiceDate = tmpData.ServiceDateEnd
