...
DROP FUNCTION IF EXISTS gpSelect_Movement_ReestrReturnOut (TDateTime, TDateTime, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_ReestrReturnOut(
...
BEGIN
     ...
     WITH tmpStatus AS (SELECT zc_Enum_Status_Complete()   AS StatusId
        ...
        , tmpMovement AS (SELECT Movement.*
                          FROM tmpStatus
                               JOIN Movement ON Movement.OperDate BETWEEN inStartDate AND inEndDate  AND Movement.DescId = zc_Movement_ReestrReturnOut() AND Movement.StatusId = tmpStatus.StatusId
       ...
       FROM tmpMovement AS Movement
            ...
            LEFT JOIN Movement AS Movement_ReturnOut ON Movement_ReturnOut.Id = MovementFloat_MovementItemId.MovementId
...
-- тест
-- SELECT * FROM gpSelect_Movement_ReestrReturnOut (inStartDate:= '20.10.2016', inEndDate:= '25.10.2016', inIsErased:= FALSE, inSession:= zfCalc_UserAdmin())
