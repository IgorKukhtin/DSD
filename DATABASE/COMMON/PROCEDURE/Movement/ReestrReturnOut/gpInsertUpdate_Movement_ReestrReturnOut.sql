-- Function: gpInsertUpdate_Movement_ReestrReturnOut()

DROP FUNCTION IF EXISTS gpInsertUpdate_Movement_ReestrReturnOut (Integer, TVarChar, TDateTime, TVarChar);

CREATE OR REPLACE FUNCTION gpInsertUpdate_Movement_ReestrReturnOut(
...
BEGIN
     ...
     IF NOT EXISTS (SELECT 1 FROM Movement WHERE Movement.Id = ioId AND Movement.InvNumber = inInvNumber AND Movement.OperDate = inOperDate AND  Movement.DescId = zc_Movement_ReestrReturnOut())
     ...
     -- сохранили <Документ>,
     ioId:= lpInsertUpdate_Movement_ReestrReturnOut (ioId               := ioId
