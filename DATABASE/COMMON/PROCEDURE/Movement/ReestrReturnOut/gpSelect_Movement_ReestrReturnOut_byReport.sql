-- Function: gpSelect_Movement_ReestrReturnOut_byReport()

DROP FUNCTION IF EXISTS gpSelect_Movement_ReestrReturnOut_byReport (Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_ReestrReturnOut_byReport(
...
RETURNS TABLE (Id Integer, InvNumber TVarChar, OperDate TDateTime
             ...
             , InvNumber_ReturnOut TVarChar, OperDate_ReturnOut TDateTime
             , StatusCode_ReturnOut Integer, StatusName_ReturnOut TVarChar
...
BEGIN
     ...
     -- Результат
     RETURN QUERY 
     WITH tmpMovement AS (SELECT DISTINCT Movement_ReestrReturnOut.*
                          FROM MovementLinkMovement AS MovementLinkMovement_Transport           -- привязка документа реестра
                               INNER JOIN Movement AS Movement_ReestrReturnOut 
                                                   ON Movement_ReestrReturnOut.Id = MovementLinkMovement_Transport.MovementId
                                                  AND Movement_ReestrReturnOut.DescId = zc_Movement_ReestrReturnOut()
                                                  AND Movement_ReestrReturnOut.StatusId <> zc_Enum_Status_Erased()
       ...
       FROM tmpMovement AS Movement
            ...
            /*LEFT JOIN MovementString AS MovementString_InvNumberOrder
                                     ON MovementString_InvNumberOrder.MovementId = Movement_ReturnOut.Id
...
-- тест
--  SELECT * FROM gpSelect_Movement_ReestrReturnOut_byReport (inMovementId:= 12595207, inSession:= '2') 
