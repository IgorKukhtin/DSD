-- Function: gpSelect_Movement_CurrencyList()

DROP FUNCTION IF EXISTS gpSelect_Movement_CurrencyList (TDateTime, TDateTime, Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_CurrencyList(
...
BEGIN
     ...
     -- vbUserId:= lpCheckRight (inSession, zc_Enum_Process_Select_Movement_CurrencyList());
     ...
     RETURN QUERY 
       WITH tmpStatus AS (SELECT zc_Enum_Status_Complete() AS StatusId
          ...
          , tmpMovement AS (SELECT Movement.*
                            FROM tmpStatus
                                 JOIN Movement ON Movement.DescId = zc_Movement_CurrencyList()
...
-- тест
--  SELECT * FROM gpSelect_Movement_CurrencyList (inStartDate:= '01.09.2018', inEndDate:= '30.09.2018', inJuridicalBasisId:= 0, inIsErased:= false, inSession:= zfCalc_UserAdmin())
