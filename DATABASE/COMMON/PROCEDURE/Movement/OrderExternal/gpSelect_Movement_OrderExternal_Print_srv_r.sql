-- Function: gpSelect_Movement_OrderExternal_Print_srv_r()

DROP FUNCTION IF EXISTS gpSelect_Movement_OrderExternal_Print_srv_r (Integer, TVarChar);
DROP FUNCTION IF EXISTS gpSelect_Movement_OrderExternal_Print_srv_r (Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_OrderExternal_Print_srv_r(
...
-- end if;
     ...
     END IF;
      ...
      --
     OPEN Cursor1 FOR
       WITH tmpMovement AS (SELECT inMovementId AS MovementId
                            WHERE inisJuridical = FALSE AND vbRetailId = 0
                           )
     ...
     , tmpObject_GoodsPropertyValue_basis AS
       (SELECT ObjectLink_GoodsPropertyValue_Goods.ChildObjectId AS GoodsId
        ...
        FROM (SELECT vbGoodsPropertyId_basis AS GoodsPropertyId
          ...
          , tmpMovement AS (SELECT inMovementId AS MovementId
                            WHERE inisJuridical = FALSE AND vbRetailId = 0
                           )
...
--ALTER FUNCTION gpSelect_Movement_OrderExternal_Print_srv_r (Integer,TVarChar) OWNER TO postgres;
...
-- тест
-- SELECT * FROM gpSelect_Movement_OrderExternal_Print_srv_r (inMovementId := 31229340, inIsJuridical:=False, inSession:= zfCalc_UserAdmin())
