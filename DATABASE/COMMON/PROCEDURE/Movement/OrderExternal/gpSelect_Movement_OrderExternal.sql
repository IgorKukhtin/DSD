...
BEGIN
     ...
     WITH tmpStatus AS (SELECT zc_Enum_Status_Complete()   AS StatusId
        ...
        , tmpAccessKey_IsDocumentAll AS (SELECT 1 AS Id FROM ObjectLink_UserRole_View WHERE RoleId = zc_Enum_Role_Admin() AND UserId = vbUserId
                                   -- Заявки-Днепр
                                   UNION SELECT 1 AS Id FROM ObjectLink_UserRole_View WHERE RoleId = 402720 AND UserId = vbUserId 
                                   -- Заявки ВСЕ - Филиалы (просмотр)
                                   UNION SELECT 1 AS Id FROM ObjectLink_UserRole_View WHERE RoleId = 13788411 AND UserId = vbUserId
                                   --
