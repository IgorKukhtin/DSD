...
DROP FUNCTION IF EXISTS gpSelect_Movement_ReestrIncomeStartPeriod_Print (TDateTime, TDateTime, Boolean, TVarChar);
...
CREATE OR REPLACE FUNCTION gpSelect_Movement_ReestrIncomeStartPeriod_Print(
    ...
    IN inReestrIncomeKindId        Integer   ,
    IN inIsReestrIncomeKind        Boolean   ,
...
BEGIN
     ...
     IF COALESCE (inReestrIncomeKindId, 0 ) = 0
        THEN inReestrIncomeKindId := zc_Enum_ReestrIncomeKind_PartnerOut();
     END IF;
     ...
     OPEN Cursor1 FOR
       ...
       SELECT inStartDate AS StartDate
            ...
            , Object_ReestrIncomeKind.ValueData AS ReestrIncomeKindName
            , Object_User.ValueData       AS UserName
       FROM Object AS Object_ReestrIncomeKind
            LEFT JOIN Object AS Object_User ON Object_User.Id = vbUserId
       WHERE Object_ReestrIncomeKind.Id = zc_Enum_ReestrIncomeKind_PartnerOut();
    ...
    RETURN NEXT Cursor1;
     ...
     OPEN Cursor2 FOR
       WITH 
               ...
               tmpMI AS (SELECT MovementItem.Id            AS MovementItemId
                         ...
                         FROM Movement
                           ...
                           INNER JOIN MovementLinkObject AS MovementLinkObject_ReestrIncomeKind
                                                         ON MovementLinkObject_ReestrIncomeKind.MovementId = MovementFloat_MovementItemId.MovementId
                                                        AND MovementLinkObject_ReestrIncomeKind.DescId = zc_MovementLinkObject_ReestrIncomeKind()
                                                        AND MovementLinkObject_ReestrIncomeKind.ObjectId = zc_Enum_ReestrIncomeKind_PartnerOut()  -- ВЫвезено со склада
       ...
       FROM tmpMI
            ...
            LEFT JOIN MovementLinkObject AS MovementLinkObject_ReestrIncomeKind
                                         ON MovementLinkObject_ReestrIncomeKind.MovementId = Movement_Sale.Id
                                        AND MovementLinkObject_ReestrIncomeKind.DescId = zc_MovementLinkObject_ReestrIncomeKind()
            LEFT JOIN Object AS Object_ReestrIncomeKind ON Object_ReestrIncomeKind.Id = MovementLinkObject_ReestrIncomeKind.ObjectId
       ...
       WHERE ((inIsReestrIncomeKind = TRUE AND MovementLinkObject_ReestrIncomeKind.ObjectId = inReestrIncomeKindId) 
          OR inIsReestrIncomeKind = FALSE
             )
...
-- тест
-- SELECT * FROM gpSelect_Movement_ReestrIncomeStartPeriod_Print(inStartDate := ('26.12.2016')::TDateTime , inEndDate := ('31.12.2016')::TDateTime , inPersonalId := 0 ,  inPersonalTradeId := 0, inReestrIncomeKindId := 640043  , inIsReestrIncomeKind := 'False' , inIsShowAll := 'True' ,  inSession := '5'); --FETCH ALL "<unnamed portal 37>";
