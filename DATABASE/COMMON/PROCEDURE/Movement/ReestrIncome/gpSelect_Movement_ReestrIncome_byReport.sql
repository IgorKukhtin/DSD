-- Function: gpSelect_Movement_ReestrIncome_byReport()

DROP FUNCTION IF EXISTS gpSelect_Movement_ReestrIncome_byReport (Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Movement_ReestrIncome_byReport(
...
RETURNS TABLE (Id Integer, InvNumber TVarChar, OperDate TDateTime
             ...
             , InvNumber_Income TVarChar, OperDate_Income TDateTime
             , StatusCode_Income Integer, StatusName_Income TVarChar
...
BEGIN
     ...
     -- Результат
     RETURN QUERY 
     WITH tmpMovement AS (SELECT DISTINCT Movement_ReestrIncome.*
                          FROM MovementLinkMovement AS MovementLinkMovement_Transport           -- привязка документа реестра
                               INNER JOIN Movement AS Movement_ReestrIncome 
                                                   ON Movement_ReestrIncome.Id = MovementLinkMovement_Transport.MovementId
                                                  AND Movement_ReestrIncome.DescId = zc_Movement_ReestrIncome()
                                                  AND Movement_ReestrIncome.StatusId <> zc_Enum_Status_Erased()
       ...
       FROM tmpMovement AS Movement
            ...
            LEFT JOIN MovementItemLinkObject AS MILinkObject_inBuh
                                            ...
                                            AND MILinkObject_inBuh.DescId = zc_MILinkObject_Buh()
...
-- тест
--  SELECT * FROM gpSelect_Movement_ReestrIncome_byReport (inMovementId:= 12595207, inSession:= '2') 
