...
--     vbUserId:= lpCheckRight (inSession, zc_Enum_Process_InsertUpdate_Movement_TaxCorrective());
     ...
     ioId := lpInsertUpdate_Movement_TaxCorrective(ioId, inInvNumber, inInvNumberPartner, inOperDate, inDateRegistered,
                                         inChecked, inRegistered, inDocument, inPriceWithVAT, inVATPercent,
                                         inFromId, inToId, inContractId, inDocumentTaxKind,
                                         inDocumentMasterId, inDocumentChildId, vbUserId);
...
-- тест
-- SELECT * FROM gpInsertUpdate_Movement_TaxCorrective (ioId:= 0, inInvNumber:= '-1',inInvNumberPartner:= '-1', inOperDate:= '01.01.2013', inDateRegistered:= '01.01.2013', inChecked:= FALSE, inRegistered:= FALSE, inDocument:=FALSE, inPriceWithVAT:= true, inVATPercent:= 20, inFromId:= 1, inToId:= 2, inContractId:= 0, inDocumentTaxKind:= 0, inDocumentMasterId:=0, inDocumentChildId:=0, inSession:= '2')