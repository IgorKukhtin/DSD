...
DROP FUNCTION IF EXISTS gpSelect_ReplMovementBoolean (TVarChar, Integer, Integer, Integer, TVarChar, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_ReplMovementBoolean(
...
BEGIN
     ...
     SELECT
        ...
        , MovementBoolean.DescId                       AS DescId
        , MovementBooleanDesc.Code    :: VarChar (100) AS DescName
        , MovementBooleanDesc.ItemName:: VarChar (100) AS ItemName

        , ''                        :: VarChar (1)     AS ValueDataS
        , 0                         :: TFloat          AS ValueDataF
        , NULL                      :: TDateTime       AS ValueDataD
        , MovementBoolean.ValueData                    AS ValueDataB
        , FALSE                     :: Boolean         AS isValuDNull
        , CASE WHEN MovementBoolean.ValueData IS NULL THEN TRUE ELSE FALSE END :: Boolean AS isValuBNull
     ...
     FROM ReplMovement
          ...
          INNER JOIN MovementBoolean     ON MovementBoolean.MovementId = ReplMovement.MovementId
          LEFT JOIN  MovementBooleanDesc ON MovementBooleanDesc.Id   = MovementBoolean.DescId
     ...
     ORDER BY ReplMovement.MovementId, MovementBoolean.DescId
    ;
...
-- тест
-- SELECT * FROM gpSelect_ReplMovementBoolean  (inSessionGUID:= CURRENT_TIMESTAMP :: TVarChar, inStartId:= 0, inEndId:= 0, inDataBaseId:= 0, gConnectHost:= '', inSession:= zfCalc_UserAdmin()) -- ORDER BY 1
