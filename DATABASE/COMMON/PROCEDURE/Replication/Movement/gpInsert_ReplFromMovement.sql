...
DROP FUNCTION IF EXISTS gpInsert_ReplFromMovement (TVarChar, TDateTime, TVarChar, Integer, TVarChar, TVarChar);
     ...
     INSERT INTO ReplMovement (MovementId, DescId, UserId_last, OperDate_last, OperDate, SessionGUID)
        WITH tmpDesc AS (SELECT MovementDesc.Id AS DescId
                         ...
                         WHERE (MovementDesc.Id = vbDescId OR vbDescId = 0)
                           AND (MovementDesc.Id IN (zc_Movement_Sale()
     ...
     THEN
         ...
         INSERT INTO MovementString (MovementId, DescId, ValueData)
           SELECT ReplMovement.MovementId, zc_MovementString_GUID()
                  ...
                  -- !!!КЛЮЧ!!!
                , ReplMovement.MovementId :: TVarChar || ' - M-ch - ' || inDataBaseId :: TVarChar AS ValueData
         ...
         INSERT INTO MovementItemString (MovementItemId, DescId, ValueData)
           SELECT MovementItem.Id, zc_MIString_GUID()
                  ...
                  -- !!!КЛЮЧ!!!
                , MovementItem.Id :: TVarChar || ' - MI-ch - ' || inDataBaseId :: TVarChar AS ValueData
...
-- SELECT * FROM gpInsert_ReplFromMovement  (inSessionGUID:= CURRENT_TIMESTAMP :: TVarChar, inStartDate:= CURRENT_TIMESTAMP - INTERVAL '1 DAY', inDescCode:= '', inDataBaseId:= 0, gConnectHost:= '', inSession:= zfCalc_UserAdmin())
-- SELECT * FROM gpInsert_ReplFromMovement  (inSessionGUID:= CURRENT_TIMESTAMP :: TVarChar, inStartDate:= CURRENT_TIMESTAMP - INTERVAL '1 DAY', inDescCode:= '', inDataBaseId:= 0, gConnectHost:= '', inSession:= zfCalc_UserAdmin())
