-- View: DiscountProgramsTax

DROP VIEW IF EXISTS DiscountProgramsTax;

CREATE OR REPLACE VIEW DiscountProgramsTax
AS 
  WITH _tmpresult AS (SELECT extId
                      ...
                      FROM dblink ('host=192.168.0.219 dbname=project port=5432 user=project password=sqoII5szOnrcZxJVF1BL'::text
                                 , ('SELECT extId
                                          ...
                                          , linkDiscounts_extId
                                          , linkDiscounts_discount
                                    FROM gpSelect_Movement_DiscountProgramsTax_effie (zfCalc_UserAdmin())'
                                    ) :: Text
...
ALTER TABLE DiscountProgramsTax  OWNER TO postgres;

GRANT ALL ON TABLE PUBLIC.DiscountProgramsTax TO admin;
GRANT ALL ON TABLE PUBLIC.DiscountProgramsTax TO project;
...
-- тест
-- SELECT * FROM DiscountProgramsTax ORDER BY 1
