-- Function: gpSelect_wms_message_all()

DROP FUNCTION IF EXISTS gpSelect_wms_message_all (TDateTime, TDateTime);

CREATE OR REPLACE FUNCTION gpSelect_wms_message_all (
...
BEGIN
    RETURN QUERY
        ...
        FROM  wms_message AS Msg 
            LEFT JOIN wms_from_host_error AS Err ON Msg.Id = Err.wms_message_id
...
-- тест
-- SELECT * FROM gpSelect_wms_message_all (inStartDate:= current_date, inEndDate:= current_timestamp)
