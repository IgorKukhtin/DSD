          ...
          IF vbUserId_save = 5 THEN
           ...
           RETURN QUERY
                ...
                , tmpPriceList AS (SELECT COALESCE(ObjectLink_Partner_PriceList.ChildObjectId
                                  ...
                                  UNION                                    
                                   ...
                                   FROM tmpPartner
                                        ...
                                        INNER JOIN Object AS Object_PriceList ON Object_PriceList.Id       = OL_ContractPriceList_PriceList.ChildObjectId
                                                                           --AND Object_PriceList.isErased = OL_ContractPriceList_PriceList.ChildObjectId
                                   WHERE CURRENT_DATE + INTERVAL '1 DAY' BETWEEN ObjectDate_StartDate.ValueData AND ObjectDate_EndDate.ValueData
                                  )
                , tmpGoods AS (SELECT DISTINCT tmp.GoodsId_key FROM gpSelectMobile_Object_GoodsByGoodsKind (inSyncDateIn, vbSession_save) AS tmp)
            ...
            , tmpRes_all AS (SELECT (Object_PriceListItem.Id)                                     :: Integer   AS Id
                                  ...
                                  , ObjectLink_PriceListItem_GoodsKind.ChildObjectId              :: Integer   AS GoodsKindId
                                  ...
                                  , TRUE                                                          :: Boolean   AS isSync
                                    -- № п/п
                                  , ROW_NUMBER() OVER (PARTITION BY ObjectLink_PriceListItem_Goods.ChildObjectId ORDER BY Object_PriceListItem.Id ASC) AS Ord
                             FROM Object AS Object_PriceListItem
                                  ...
                                  JOIN tmpGoods ON tmpGoods.GoodsId_key = ObjectLink_PriceListItem_Goods.ChildObjectId
                             ...
                             GROUP BY Object_PriceListItem.Id
                                    ...
                                    , ObjectLink_PriceListItem_GoodsKind.ChildObjectId
                                    , ObjectLink_PriceListItem_PriceList.ChildObjectId
                            )
      , tmpGoodsByGoodsKind AS (SELECT gpSelect.Id
                                     , gpSelect.GoodsId
                                     , gpSelect.GoodsId_key
                                     , gpSelect.GoodsKindId
                                FROM gpSelectMobile_Object_GoodsByGoodsKind (inSyncDateIn:= zc_DateStart(), inSession:= vbSession_save) AS gpSelect
                                WHERE gpSelect.isErased = FALSE
                               )
          , tmpPriceListItem AS (SELECT ObjectLink_PriceListItem_Goods.ObjectId                                           AS ObjectId
                ...
                , tmpRes AS (SELECT COALESCE (tmpPriceListItem.ObjectId, tmpRes_all.Id, tmpRes_all_1.Id, tmpRes_all_2.Id) AS Id
                                  , tmpGoodsByGoodsKind.GoodsId
                                  , tmpGoodsByGoodsKind.GoodsId_key
                                  , tmpGoodsByGoodsKind.GoodsKindId
                                  , tmpPriceList.PriceListId
                                  , COALESCE (tmpRes_all.OrderStartDate, tmpRes_all_1.OrderStartDate, tmpRes_all_2.OrderStartDate)    AS OrderStartDate
                                  , COALESCE (tmpRes_all.OrderEndDate, tmpRes_all_1.OrderEndDate, tmpRes_all_2.OrderEndDate)          AS OrderEndDate
                                  , COALESCE (tmpRes_all.OrderPrice, tmpRes_all_1.OrderPrice, tmpRes_all_2.OrderPrice)                AS OrderPrice
                                  , COALESCE (tmpRes_all.SaleStartDate, tmpRes_all_1.SaleStartDate, tmpRes_all_2.SaleStartDate)       AS SaleStartDate
                                  , COALESCE (tmpRes_all.SaleEndDate, tmpRes_all_1.SaleEndDate, tmpRes_all_2.SaleEndDate)             AS SaleEndDate
                                  , COALESCE (tmpRes_all.SalePrice, tmpRes_all_1.SalePrice, tmpRes_all_2.SalePrice)                   AS SalePrice
                                  , COALESCE (tmpRes_all.ReturnStartDate, tmpRes_all_1.ReturnStartDate, tmpRes_all_2.ReturnStartDate) AS ReturnStartDate
                                  , COALESCE (tmpRes_all.ReturnEndDate, tmpRes_all_1.ReturnEndDate, tmpRes_all_2.ReturnEndDate)       AS ReturnEndDate
                                  , COALESCE (tmpRes_all.ReturnPrice, tmpRes_all_1.ReturnPrice, tmpRes_all_2.ReturnPrice)             AS ReturnPrice
                                  , TRUE                                                                                              AS isSync
                                    -- № п/п
                                  , ROW_NUMBER() OVER (PARTITION BY tmpGoodsByGoodsKind.GoodsId, tmpGoodsByGoodsKind.GoodsKindId, tmpPriceList.PriceListId ORDER BY tmpRes_all.Id ASC) AS Ord
                             FROM tmpGoodsByGoodsKind
                                  JOIN tmpPriceList ON tmpPriceList.PriceListId > 0
                                  LEFT JOIN tmpPriceListItem ON tmpPriceListItem.GoodsId     = tmpGoodsByGoodsKind.GoodsId_key
                                                            AND tmpPriceListItem.GoodsKindId = tmpGoodsByGoodsKind.GoodsKindId
                                                            AND tmpPriceListItem.PriceListId = tmpPriceList.PriceListId
                                  LEFT JOIN tmpRes_all ON tmpRes_all.GoodsId     = tmpGoodsByGoodsKind.GoodsId_key
                                                      AND tmpRes_all.GoodsKindId = tmpGoodsByGoodsKind.GoodsKindId
                                                      AND tmpRes_all.PriceListId = tmpPriceList.PriceListId
                                  LEFT JOIN tmpRes_all AS tmpRes_all_1
                                                       ON tmpRes_all_1.GoodsId                                      = tmpGoodsByGoodsKind.GoodsId_key
                                                      AND COALESCE (tmpRes_all_1.GoodsKindId, zc_GoodsKind_Basis()) = zc_GoodsKind_Basis()
                                                      AND tmpRes_all_1.PriceListId = tmpPriceList.PriceListId
                                  LEFT JOIN tmpRes_all AS tmpRes_all_2
                                                       ON tmpRes_all_2.GoodsId   = tmpGoodsByGoodsKind.GoodsId_key
                                                      AND tmpRes_all_2.Ord       = 1
                                                      AND tmpRes_all_2.PriceListId = tmpPriceList.PriceListId

                             WHERE COALESCE (tmpPriceListItem.ObjectId, tmpRes_all.Id, tmpRes_all_1.Id, tmpRes_all_2.Id) > 0
                            -- and tmpRes_all.GoodsId = 2755 
                            )
             -- Результат
             SELECT tmpRes.Id                :: Integer AS Id
                  , tmpRes.GoodsId           :: Integer AS GoodsId
                  , tmpRes.GoodsId_key       :: Integer AS GoodsId_key
                  , tmpRes.GoodsKindId       :: Integer AS GoodsKindId
                  , tmpRes.PriceListId       :: Integer AS PriceListId
                  , tmpRes.OrderStartDate    :: TDateTime AS OrderStartDate
                  , tmpRes.OrderEndDate      :: TDateTime AS OrderEndDate
                  , tmpRes.OrderPrice        :: TFloat AS OrderPrice
                  , tmpRes.SaleStartDate     :: TDateTime AS SaleStartDate
                  , tmpRes.SaleEndDate       :: TDateTime AS SaleEndDate
                  , tmpRes.SalePrice         :: TFloat AS SalePrice
                  , tmpRes.ReturnStartDate   :: TDateTime AS ReturnStartDate
                  , tmpRes.ReturnEndDate     :: TDateTime AS ReturnEndDate 
                  , tmpRes.ReturnPrice       :: TFloat AS ReturnPrice
                  , tmpRes.isSync
             FROM tmpRes
             WHERE tmpRes.Ord = 1
