...
BEGIN
      ...
      , tmpGoodsByGoodsKind AS (SELECT gpSelect.Id
                                ...
                                FROM gpSelectMobile_Object_GoodsByGoodsKind (inSyncDateIn:= zc_DateStart(), inSession:= inSession) AS gpSelect
                                WHERE gpSelect.isErased = FALSE
                               )
   , tmpGoods_old AS (WITH tmpProtocol AS (SELECT ObjectProtocol.ObjectId AS GoodsId, MAX(ObjectProtocol.OperDate) AS MaxOperDate
                                           FROM ObjectProtocol
                                                JOIN Object AS Object_Goods
                                                            ON Object_Goods.Id = ObjectProtocol.ObjectId
                                                           AND Object_Goods.DescId = zc_Object_Goods() 
             ...
             FROM Object AS Object_Goods
                  JOIN tmpFilter ON tmpFilter.GoodsId = Object_Goods.Id
                  JOIN tmpGoodsByGoodsKind ON tmpGoodsByGoodsKind.GoodsId_key = Object_Goods.Id
                  LEFT JOIN tmpGoods ON tmpGoods.GoodsId = Object_Goods.Id 
                  LEFT JOIN ObjectFloat AS ObjectFloat_Goods_Weight
                                        ON ObjectFloat_Goods_Weight.ObjectId = Object_Goods.Id
                                       AND ObjectFloat_Goods_Weight.DescId = zc_ObjectFloat_Goods_Weight() 
           ...
           --LIMIT CASE WHEN vbUserId = 1072129 THEN 0 ELSE 500000 END
            ...
            UNION ALL
             ...
             SELECT Object_Goods.Id
                  ...
                  , Object_Goods.ValueData
                  , Object_Goods.Weight
                  , Object_Goods.GoodsGroupId
                  , Object_Goods.MeasureId
                  , Object_Goods.TradeMarkId
                  , TRUE  AS isErased
                  , FALSE AS isSync
             FROM tmpGoods_old AS Object_Goods
