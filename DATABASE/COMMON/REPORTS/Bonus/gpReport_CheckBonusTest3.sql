...
DROP FUNCTION IF EXISTS gpReport_CheckBonusTest3 (TDateTime, TDateTime, Integer, Integer, Integer, TVarChar);
...
*/
    ...
    , tmpContainer1 AS (SELECT  DISTINCT
     ...
     , tmpCLO_Partner AS (SELECT ContainerLinkObject.*
                          ...
                          WHERE ContainerLinkObject.ContainerId IN (SELECT DISTINCT tmpContainer1.ContainerId FROM tmpContainer1 WHERE tmpContainer1.PaidKindId_byBase <> zc_Enum_PaidKind_SecondForm())
...
-- select * from gpReport_CheckBonusTest2(inStartDate := ('28.05.2020')::TDateTime , inEndDate := ('28.05.2020')::TDateTime , inPaidKindId := 4 , inJuridicalId := 344240 , inBranchId := 0 ,  inSession := '5');--
 ...
 select * from gpReport_CheckBonusTest3(inStartDate := ('01.07.2020')::TDateTime , inEndDate := ('03.07.2020')::TDateTime , inPaidKindId := 4 , inJuridicalId := 0 , inBranchId := 0 ,  inSession := '5');
 
 
 /*
 
 
 --select * from Object where Object.descId = zc_Object_PaidKind()    

WITH 
           tmpContract_full AS (SELECT View_Contract.*
                                FROM Object_Contract_View AS View_Contract
                                WHERE (View_Contract.JuridicalId = 0 OR COALESCE (0, 0) = 0)
                                )
         , tmpContract_all AS (SELECT *
                               FROM tmpContract_full
       ...
       , tmpContractConditionKind AS (SELECT -- условие договора
                                      ...
                                      FROM ObjectLink AS ObjectLink_ContractConditionKind
                                           ...
                                           INNER JOIN ObjectLink AS ObjectLink_ContractCondition_Contract
                                                               ...
                                                               AND ObjectLink_ContractCondition_Contract.DescId = zc_ObjectLink_ContractCondition_Contract()
                                           INNER JOIN tmpContract_find AS View_Contract ON View_Contract.ContractId = ObjectLink_ContractCondition_Contract.ChildObjectId
                                                                                  --  AND (View_Contract.JuridicalId = 0 OR 0 = 0)
    ...
    ,tmpContractGroup AS (SELECT tmpContract.JuridicalId
                           ...
                           --  AND (tmpContract.JuridicalId = 0 OR 0 = 0)
                           GROUP BY tmpContract.JuridicalId
                                  , tmpContract.ContractId_child
                                  , tmpContract.InfoMoneyId_child
                                  , tmpContract.PaidKindId_byBase
    ...
    , tmpContainer1 AS (SELECT DISTINCT
                       ...
                       FROM tmpAccount
                            ...
                            /*LEFT JOIN ContainerLinkObject AS CLO_Partner
                                                     ON CLO_Partner.ContainerId = Container.Id
                                                    AND CLO_Partner.DescId = zc_ContainerLinkObject_Partner()
                                                    AND 4 <> zc_Enum_PaidKind_FirstForm()*/
                            -- ограничиваем контрагентами --
                            -- ограничиваем контрагентами --
                            --INNER JOIN tmpContractPartner ON (tmpContractPartner.PartnerId = CLO_Partner.ObjectId
                              --                               ) OR ContainerLO_PaidKind.ObjectId = zc_Enum_PaidKind_FirstForm()
                            ...
                            LEFT JOIN ContainerLinkObject AS ContainerLO_Branch
                                                          ON ContainerLO_Branch.ContainerId = Container.Id
                                                         AND ContainerLO_Branch.DescId = zc_ContainerLinkObject_Branch()
                       WHERE COALESCE (ContainerLO_Branch.ObjectId,0) = 8381 OR 8381 = 0
                       -- WHERE Container.ObjectId <> zc_Enum_Account_50401() -- "Маркетинг"
                       --   AND Container.ObjectId <> zc_Enum_AccountDirection_70300() нужно убрать проводки по оплате маркетинга
                       --  AND (4 <> zc_Enum_PaidKind_FirstForm() AND CLO_Partner.ObjectId IN (SELECT DISTINCT tmpContractPartner.PartnerId FROM tmpContractPartner) )
                       )
*/
 , tmpContainer2 AS (
SELECT Container.*
...
where Container.ObjectId IN (SELECT DISTINCT tmpAccount.AccountId FROM tmpAccount)
 ...
 , tmpContainer1 AS ( SELECT *
, COALESCE (ContainerLO_Branch.ObjectId,0) 
FROM tmpContainer2 
                            -- ограничение по 4-м ключам
                            JOIN tmpContractGroup ON tmpContractGroup.JuridicalId       = tmpContainer2.JuridicalId 
                                                 AND tmpContractGroup.ContractId_child  = tmpContainer2.ContractId
                                                 AND tmpContractGroup.InfoMoneyId_child = tmpContainer2.InfoMoneyId
                                                 AND tmpContractGroup.PaidKindId_byBase = tmpContainer2.PaidKindId
                            LEFT JOIN ContainerLinkObject AS ContainerLO_Branch
                                                          ON ContainerLO_Branch.ContainerId = tmpContainer2.Id
                                                         AND ContainerLO_Branch.DescId = zc_ContainerLinkObject_Branch()
  WHERE COALESCE (ContainerLO_Branch.ObjectId,0) = 8381 OR 8381 = 0 
)
...
/*
      , tmpMovementCont AS (SELECT tmpContainer.JuridicalId
                                 ...
                                 , SUM (CASE WHEN MIContainer.MovementDescId = zc_Movement_ReturnIn() THEN MIContainer.Amount ELSE 0 END) AS Sum_Return  -- возврат
                                 , CASE WHEN false = TRUE THEN MIContainer.MovementDescId ELSE 0 END  AS MovementDescId
                                 , CASE WHEN false = TRUE THEN MIContainer.MovementId ELSE 0 END      AS MovementId

                            FROM MovementItemContainer AS MIContainer
                                 JOIN tmpContainer ON tmpContainer.ContainerId = MIContainer.ContainerId
                            WHERE MIContainer.DescId = zc_MIContainer_Summ()
                              AND MIContainer.OperDate BETWEEN '30.06.2020' AND '30.06.2020'
                              AND MIContainer.MovementDescId IN (zc_Movement_Sale(), zc_Movement_ReturnIn(), zc_Movement_BankAccount(),zc_Movement_Cash(), zc_Movement_SendDebt())
                            GROUP BY tmpContainer.JuridicalId
                                   , tmpContainer.ContractId_child
                                   , tmpContainer.InfoMoneyId_child
                                   , tmpContainer.PaidKindId_byBase
                                   , CASE WHEN false = TRUE THEN MIContainer.MovementDescId ELSE 0 END
                                   , CASE WHEN false = TRUE THEN MIContainer.MovementId ELSE 0 END
                                   , tmpContainer.BranchId
                                   , tmpContainer.PartnerId
                            )
           ...
           , tmpAll as(SELECT tmpContract.InvNumber_master
                     ...
                     UNION ALL 
                       SELECT View_Contract_InvNumber_master.InvNumber AS InvNumber_master
                            ...
                            , CASE WHEN false = TRUE THEN Movement.Id ELSE 0 END      AS MovementId
                            , CASE WHEN false = TRUE THEN Movement.DescId ELSE 0 END  AS MovementDescId
                            , COALESCE (MILinkObject_Branch.ObjectId,0)      AS BranchId
                     FROM Movement 
                            LEFT JOIN MovementItem ON MovementItem.MovementId = Movement.Id AND MovementItem.DescId = zc_MI_Master()
                       ...
                       WHERE Movement.DescId = zc_Movement_ProfitLossService()
                         AND Movement.StatusId = zc_Enum_Status_Complete()
                         AND Movement.OperDate BETWEEN '30.06.2020' AND '30.06.2020'
                         AND MILinkObject_InfoMoney.ObjectId IN (zc_Enum_InfoMoney_21501() -- Маркетинг + Бонусы за продукцию
                                                               , zc_Enum_InfoMoney_21502()) -- Маркетинг + Бонусы за мясное сырье
                         AND (MovementItem.ObjectId = 0 OR 0 = 0)
                         -- AND MILinkObject_ContractConditionKind.ObjectId IN (zc_Enum_ContractConditionKind_BonusPercentAccount(), zc_Enum_ContractConditionKind_BonusPercentSaleReturn(), zc_Enum_ContractConditionKind_BonusPercentSale())
                      )
      , tmpData AS (SELECT tmpAll.ContractId_master
                         , tmpAll.ContractId_child
                    ...
                    WHERE (tmpAll.Sum_CheckBonus > 0
                       OR tmpAll.Sum_Bonus > 0
                       OR tmpAll.Sum_BonusFact <> 0)
                      AND (tmpAll.PaidKindId = 4 OR 4 = 0)
                      AND (tmpAll.JuridicalId = 0 OR 0 = 0)
                    GROUP BY  tmpAll.ContractId_master
                            , tmpAll.ContractId_child
                            , tmpAll.ContractId_find
                            , tmpAll.InvNumber_master
