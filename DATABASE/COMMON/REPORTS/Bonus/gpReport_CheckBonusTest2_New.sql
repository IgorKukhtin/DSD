-- FunctiON: gpReport_CheckBonusTest2 ()

-- gpReport_CheckBonusTest2_New.sql = gpReport_CheckBonusTest = gpReport_CheckBonusTest2

DROP FUNCTION IF EXISTS gpReport_CheckBonusTest2 (TDateTime, TDateTime, Integer, Integer, Integer, Integer, Boolean, Boolean, Boolean, TVarChar);
...
BEGIN
    ...
    --!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    --правильный расчет в процке gpReport_CheckBonusTest2Test3
    --!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
/*
    ...
    from gpReport_CheckBonusTest2 (inStartDate:= '01.10.2014', inEndDate:= '31.10.2014', inSession:= '5') as a
    where Sum_Bonus <> 0 -- and Sum_Bonus =30
*/
-- тест
-- select * from gpReport_CheckBonusTest2 (inStartDate:= '15.03.2016', inEndDate:= '15.03.2016', inPaidKindID:= zc_Enum_PaidKind_FirstForm(), inJuridicalId:= 0, inBranchId:= 0, inSession:= zfCalc_UserAdmin());
-- select * from gpReport_CheckBonusTest2(inStartDate := ('28.05.2020')::TDateTime , inEndDate := ('28.05.2020')::TDateTime , inPaidKindId := 3 , inJuridicalId := 344240 , inBranchId :=  8374, inMemberId:=0 ,  inIsMovement := FALSE, inSession := '5');--
-- select * from gpReport_CheckBonusTest2(inStartDate := ('28.05.2021')::TDateTime , inEndDate := ('28.05.2021')::TDateTime , inPaidKindId := 3 , inJuridicalId := 344240 , inBranchId :=  8374, inMemberId:=0 ,  inIsMovement := FALSE,inisDetail := TRUE, inisGoods:= false, inSession := '5');--
