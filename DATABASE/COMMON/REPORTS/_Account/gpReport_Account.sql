...
BEGIN
    ...
    FROM  
	   (SELECT  Container.Id as ContainerId  -- счет количественный
        ...
        FROM (SELECT inAccountId AS AccountId) AS tmpAccount -- счет --(select Id as AccountId from Object where descId = zc_Object_Account() and objectcode in (30505,30508) ) as tmpAccount  -- (SELECT inAcountId AS AccountId) AS tmpAccount -- счет 
        ...
        LEFT JOIN Object_Personal_View AS View_Personal ON View_Personal.PersonalId=ContainerLinkObject_Personal.ObjectId
        ...
        GROUP BY View_Personal.PersonalCode
               , View_Personal.PersonalName
