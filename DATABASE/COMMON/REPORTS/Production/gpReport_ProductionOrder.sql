...
BEGIN
    ...
    RETURN QUERY
     ...
     WITH tmpProductionRemains AS (SELECT ddd.GoodsId, ddd.goodskindid, ddd.partiongoodsid, ObjectDate.valuedata FROM 

(SELECT dd.GoodsId, dd.goodskindid, dd.partiongoodsid, SUM(StartAmount) AS Summ FROM (
SELECT Container.ObjectId          AS GoodsId
                  ...
                  LEFT JOIN MovementItemContainer AS MIContainer ON MIContainer.Containerid = Container.Id
                           ...
                           WHERE  CLO_Unit.ObjectId IN( 8447, 8448) AND 
...
WHERE abs(summ) > 0.01
     ...
     tmpStoreRemains AS (SELECT dd.GoodsId, dd.goodskindid, SUM(StartAmount) AS Remains 
                       ...
                       GROUP BY Container.ObjectId, GoodsKindId, Container.Amount 
                        HAVING (Container.Amount - COALESCE(SUM (MIContainer.Amount), 0)) <>0 ) AS dd
   ...
   GROUP BY MovementItem.objectid, MILinkObject_GoodsKind.objectid) AS DD
