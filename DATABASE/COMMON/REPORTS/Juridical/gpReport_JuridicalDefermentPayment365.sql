 -- Function: gpReport_JuridicalDefermentPayment365()

DROP FUNCTION IF EXISTS gpReport_JuridicalDefermentPayment365 (TDateTime, TDateTime, Integer, Integer, Integer, Integer, TVarChar);

CREATE OR REPLACE FUNCTION gpReport_JuridicalDefermentPayment365(
...
BEGIN
     ...
     tmpReport AS (SELECT tmpReport.*
                   FROM lpReport_JuridicalDefermentPayment365 (inOperDate         := inOperDate
...
-- тест
-- SELECT * FROM gpReport_JuridicalDefermentPayment_365 (inOperDate:= CURRENT_DATE, inEmptyParam:= NULL :: TDateTime, inAccountId:= 0, inPaidKindId:= zc_Enum_PaidKind_FirstForm(),  inBranchId:= 0, inJuridicalGroupId:= null, inSession:= zfCalc_UserAdmin());
-- SELECT * FROM gpReport_JuridicalDefermentPayment_365 (inOperDate:= CURRENT_DATE, inEmptyParam:= NULL :: TDateTime, inAccountId:= 0, inPaidKindId:= zc_Enum_PaidKind_SecondForm(), inBranchId:= 0, inJuridicalGroupId:= null, inSession:= zfCalc_UserAdmin());

--select * from gpReport_JuridicalDefermentPayment_365(inOperDate := ('19.12.2021')::TDateTime , inEmptyParam := ('01.05.2013')::TDateTime , inAccountId := 9128 , inPaidKindId := 3 , inBranchId := 0 , inJuridicalGroupId := 0 ,  inSession := '378f6845-ef70-4e5b-aeb9-45d91bd5e82e')
