 -- Function: gpReport_BalanceNo()

DROP FUNCTION IF EXISTS gpReport_BalanceNo (TDateTime, TDateTime, TVarChar);

CREATE OR REPLACE FUNCTION gpReport_BalanceNo(
           ...
           (SELECT tmpMIContainer_Remains.AccountId
            ...
            FROM
                (SELECT Container.ObjectId AS AccountId
                 ...
                 WHERE Container.DescId = zc_Container_SummAsset()
                                  ...
                                  AND (tmpReportOperation.PaidKindId = Object_Account_View.PaidKindId
                                       OR (tmpReportOperation.PaidKindId = 0
                                           AND ((Object_Account_View.PaidKindId = zc_Enum_PaidKind_FirstForm()     AND Object_Account_View.isFirstForm = TRUE)
                                             OR (Object_Account_View.PaidKindId = zc_Enum_PaidKind_FirstForm_pav() AND Object_Account_View.isFirstForm = FALSE)
...
-- тест
-- SELECT * FROM gpReport_BalanceNo (inStartDate:= '01.08.2024', inEndDate:= '31.08.2024', inSession:= zfCalc_UserAdmin())
