...
DROP FUNCTION IF EXISTS gpReport_GoodsBalance_Server (TDateTime, TDateTime, Integer, Integer, Integer, Integer, Integer, Boolean, Boolean, Boolean, Boolean, Boolean, TVarChar);
...
BEGIN
    ...
    THEN
        WITH tmpObjectGoods AS (SELECT lfObject_Goods_byGoodsGroup.GoodsId FROM lfSelect_Object_Goods_byGoodsGroup (inGoodsGroupId) AS lfObject_Goods_byGoodsGroup
                               )
    ...
    ELSE IF inGoodsId <> 0
         THEN
        WITH tmpGoods AS (SELECT inGoodsId AS GoodsId
                         UNION ALL
                          SELECT DISTINCT ObjectLink.ChildObjectId AS GoodsId FROM ObjectLink WHERE ObjectLink.DescId = zc_ObjectLink_Goods_Fuel() AND ObjectLink.ChildObjectId > 0 AND ObjectLink.ObjectId = inGoodsId
                         )
                , tmpContainer AS (SELECT CLO_Goods.ContainerId FROM ContainerLinkObject AS CLO_Goods WHERE CLO_Goods.ObjectId IN (SELECT tmpGoods.GoodsId FROM tmpGoods) AND CLO_Goods.DescId = zc_ContainerLinkObject_Goods()
                                  UNION
                                   SELECT Container.Id FROM Container WHERE Container.ObjectId IN (SELECT tmpGoods.GoodsId FROM tmpGoods) AND Container.DescId = zc_Container_Count()
                                  )
                , tmpAccount AS (SELECT View_Account.AccountGroupId, View_Account.AccountId
                                 FROM (SELECT inAccountGroupId              AS AccountGroupId WHERE inAccountGroupId <> 0
                                 UNION SELECT zc_Enum_AccountGroup_10000()  AS AccountGroupId WHERE inAccountGroupId = 0
   ...
   ;
    ...
    RETURN QUERY
   WITH 
       tmpObject_GoodsPropertyValue_basis AS (SELECT ObjectLink_GoodsPropertyValue_Goods.ChildObjectId AS GoodsId
...
--                               , MAX (_tmpContainer.ContainerId_begin)  AS ContainerId_begin
                          ...
                          FROM _tmpContainer
                               ...
                               --цены привязываем 2 раза по виду и без
...
--                              , MIN (tmpAll.PartionGoodsDate) AS 
  ...
  -- , (select count(*) from tmpMIContainer_GP_all) :: Integer AS GoodsCode
          ...
          , (tmpResult.CountStart * COALESCE (tmpPriceStart_kind.Price, tmpPriceStart.Price) ) :: TFloat AS SummPriceListStart
          , (tmpResult.CountEnd   * COALESCE (tmpPriceEnd_kind.Price, tmpPriceEnd.Price) )     :: TFloat AS SummPriceListEnd
...
*/
        ...
        LEFT JOIN tmpPriceEnd AS tmpPriceEnd_kind 
                             ...
                             AND COALESCE (tmpPriceEnd_kind.GoodsId,0) = COALESCE (tmpResult.GoodsKindId,0)
...
-- select * from gpReport_GoodsBalance_Server(inStartDate := ('09.02.2024')::TDateTime , inEndDate := ('29.02.2024')::TDateTime , inAccountGroupId := 0 , inUnitGroupId := 0 , inLocationId := 8448 , inGoodsGroupId := 0 , inGoodsId := 867706 , inIsInfoMoney := 'False' , inIsAllMO := 'False' , inIsAllAuto := 'False' , inIsOperDate_Partion:= TRUE, inIsPartionCell := TRUE, inSession := '5');
