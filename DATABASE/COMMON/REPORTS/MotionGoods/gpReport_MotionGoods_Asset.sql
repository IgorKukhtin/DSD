       ...
       */
       LEFT JOIN Object AS Object_Location ON Object_Location.Id = tmpMIContainer_group.LocationId
        ...
        LEFT JOIN ObjectString AS ObjectString_Asset_InvNumber
                               ON ObjectString_Asset_InvNumber.ObjectId = Object_Goods.Id
...
ALTER FUNCTION gpReport_MotionGoods_Asset (TDateTime, TDateTime, Integer, Integer, Integer, Integer, Integer, Integer, Integer, Boolean, TVarChar) OWNER TO postgres;
...
-- тест
-- SELECT * FROM gpReport_MotionGoods_Asset (inStartDate:= '01.01.2015', inEndDate:= '01.01.2015', inAccountGroupId:= 0, inUnitGroupId:= 0, inLocationId:= 0, inGoodsGroupId:= 0, inGoodsId:= 0, inUnitGroupId_by:=0, inLocationId_by:= 0, inIsInfoMoney:= FALSE, inSession:= zfCalc_UserAdmin())
-- SELECT * from gpReport_MotionGoods_Asset (inStartDate:= '01.08.2015', inEndDate:= '01.08.2015', inAccountGroupId:= 0, inUnitGroupId:= 8459, inLocationId:= 0, inGoodsGroupId:= 1860, inGoodsId:= 1, inUnitGroupId_by:= 0, inLocationId_by:= 0, inIsInfoMoney:= TRUE, inSession := zfCalc_UserAdmin());
