    ...
    END IF;
     ...
     RETURN QUERY 
      ...
      SELECT _tmp_All.GoodsId, Object_Goods.ObjectCode AS GoodsCode, Object_Goods.ValueData AS GoodsName  
           ...
           --
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.StartCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.StartCount) AS TFloat) END AS StartWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.StartCount) AS TFloat) else 0 END AS StartCount   
           , CAST (SUM (_tmp_All.StartSumm) AS TFloat)     AS StartSumm

           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.IncomeCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.IncomeCount) AS TFloat) END AS IncomeWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.IncomeCount) AS TFloat) else 0 END AS IncomeCount   
           , CAST (SUM (_tmp_All.IncomeSumm) AS TFloat)    AS IncomeSumm

           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.SendInCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.SendInCount) AS TFloat) END AS SendInWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.SendInCount) AS TFloat) else 0 END AS SendInCount   
           , CAST (SUM (_tmp_All.SendInSumm) AS TFloat)    AS SendInSumm

           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.SendOutCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.SendOutCount) AS TFloat) END AS SendOutWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.SendOutCount) AS TFloat) else 0 END AS SendOutCount           
           , CAST (SUM (_tmp_All.SendOutSumm) AS TFloat)   AS SendOutSumm

           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.SaleCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.SaleCount) AS TFloat) END AS SaleWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.SaleCount) AS TFloat) else 0 END AS SaleCount           
           , CAST (SUM (_tmp_All.SaleCount) AS TFloat)      AS SaleCount       
           , CAST (SUM (_tmp_All.SaleSumm) AS TFloat)      AS SaleSumm

           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.ReturnOutCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.ReturnOutCount) AS TFloat) END AS ReturnOutWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.ReturnOutCount) AS TFloat) else 0 END AS ReturnOutCount           
           , CAST (SUM (_tmp_All.ReturnOutSumm) AS TFloat) AS ReturnOutSumm

           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.ReturnInCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.ReturnInCount) AS TFloat) END AS ReturnInWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.ReturnInCount) AS TFloat) else 0 END AS ReturnInCount           
           , CAST (SUM (_tmp_All.ReturnInSumm) AS TFloat)  AS ReturnInSumm
        
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.LossCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.LossCount) AS TFloat) END AS LossWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.LossCount) AS TFloat) else 0 END AS LossCount           
           , CAST (SUM (_tmp_All.LossSumm) AS TFloat)      AS LossSumm
        
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.InventoryCount)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.InventoryCount) AS TFloat) END AS InventoryWeight   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.InventoryCount) AS TFloat) else 0 END AS InventoryCount           
           , CAST (SUM (_tmp_All.InventorySumm) AS TFloat) AS InventorySumm
           
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST ((SUM (_tmp_All.EndCount_calc)*ObjectFloat_Weight.ValueData) AS TFloat) else  CAST (SUM (_tmp_All.EndCount_calc) AS TFloat) END AS EndWeight_calc   
           , case Object_Measure.Id = 1 /*шт*/ THEN CAST  (SUM (_tmp_All.EndCount_calc) AS TFloat) else 0 END AS EndCount_calc           
        ...
        UNION ALL
               ...
               FROM (
                 ...
                 LEFT JOIN  ContainerLinkObject AS ContainerLinkObject_Unit
                                                ON ContainerLinkObject_Unit.Containerid = _tmpContainerAll.ContainerId
                                               AND ContainerLinkObject_Unit.DescId = zc_ContainerLinkObject_Unit()
