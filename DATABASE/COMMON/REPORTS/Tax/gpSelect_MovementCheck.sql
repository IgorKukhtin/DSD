...
RETURNS TABLE (ParamText TVarChar, Param_Sale TVarChar, Param_Tax TVarChar, Param_TaxConv TVarChar
...
BEGIN
	...
	ELSE IF vbDescId = zc_Movement_TaxCorrective()
	     THEN
	         vbTaxCorrectiveId := inDocumentId;
   ...
   SELECT 'Дата' ::TVarChar
        , CAST (MAX (CASE WHEN Movement.DescId = zc_Movement_Sale() THEN MovementDate_OperDatePartner.ValueData  END) AS TVarChar )
   ...
   SELECT 'Договор' ::TVarChar
        , CAST (MAX (CASE WHEN Movement.DescId = zc_Movement_Sale() THEN View_Contract_InvNumber.InvNumber  END)  AS TVarChar )
   ...
   SELECT 'Юр.лицо' ::TVarChar
        , CAST (MAX (CASE WHEN Movement.DescId = zc_Movement_Sale() THEN Object_JuridicalTo.ValueData END)  AS TVarChar )
