-- Function: gpReport_Branch_App1_Month()

DROP FUNCTION IF EXISTS gpReport_Branch_App1_Month (TDateTime, TDateTime,  Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpReport_Branch_App1_Month(
...
BEGIN
   ...
   WITH 
    ...
    , tmpGoodsSumm AS (SELECT tmpContainerListSum.BranchId
                            ...
                            , DATE_TRUNC ('Month', MIContainer.OperDate) AS Month
                            ...
                            , SUM (CASE WHEN MIContainer.MovementDescId = zc_Movement_SendOnPrice()
                                         ...
                                         AND tmpContainerListSum.isUnit_Vz = FALSE 
                                        THEN MIContainer.Amount                              ---перемещение по цене
                                      ELSE 0
                       ...
                       GROUP BY tmpContainerListSum.ContainerId
                              , tmpContainerListSum.Amount
                              , tmpContainerListSum.BranchId,tmpContainerListSum.isUnit_Vz
                       ...
                       FROM tmpUnitList 
                          ...
                          GROUP BY CLO_Juridical.ContainerId
                                 , tmpUnitList.BranchId
                                 , CASE WHEN inIsUnit = TRUE THEN tmpUnitList.Id ELSE 0 END 
                                 , DATE_TRUNC ('Month', MIContainer.OperDate)
                      )

    , tmpGoodsCount AS  (SELECT DATE_TRUNC ('Month', MIContainer.OperDate) AS Month
                              , tmpContainerList.BranchId
                              ...
                              , SUM (CASE WHEN MIContainer.MovementDescId = zc_Movement_Sale() THEN -1 * MIContainer.Amount                                        --Sale
                         ...
                         GROUP BY tmpContainerList.Amount
                                , tmpContainerList.BranchId
                                , tmpContainerList.ObjectId
                                , tmpContainerList.UnitId
                                , tmpContainerList.ContainerId
                                , tmpContainerList.isUnit_Vz
                                , CASE WHEN inIsUnit = TRUE THEN tmpContainerList.UnitId ELSE 0 END
                                , DATE_TRUNC ('Month', MIContainer.OperDate)
                        )
                    
    , tmpGoodsWeight AS (SELECT tmpGoodsCount.Month
                              , tmpGoodsCount.BranchId 
   ...
   SELECT tmpAll.Month             ::TDateTime  AS Month
   ...
   FROM 
        (SELECT tmpGoodsSumm.Month
              , tmpGoodsSumm.BranchId
              , tmpGoodsSumm.UnitId 
         ...
         GROUP BY tmpGoodsSumm.BranchId
                ...
                , tmpGoodsSumm.Month
     UNION ALL
         SELECT tmpGoodsWeight.Month
              , tmpGoodsWeight.BranchId 
...
-- тест
--  SELECT * FROM gpReport_Branch_App1_Month (inStartDate:= '01.02.2018'::TDateTime, inEndDate:= '08.02.2018'::TDateTime, inBranchId:= 8374, inIsUnit := FALSE, inSession:= zfCalc_UserAdmin()) -- филиал Одесса
--  SELECT * FROM gpReport_Branch_App1_Month (inStartDate:= '01.02.2018'::TDateTime, inEndDate:= '08.02.2018'::TDateTime, inBranchId:= 8374, inIsUnit := TRUE, inSession:= zfCalc_UserAdmin())  -- филиал Одесса
-- SELECT * FROM gpReport_Branch_App1_Month (inStartDate:= '01.02.2018'::TDateTime, inEndDate:= '08.02.2018'::TDateTime, inBranchId:= 8374, inIsUnit := TRUE, inSession:= zfCalc_UserAdmin())  -- филиал Одесса
