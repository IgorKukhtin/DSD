                ...
                UNION ALL    
                 SELECT Container.ObjectId AS GoodsId       
                      ...
                      , SUM (CASE WHEN MovementBoolean_PriceWithVAT.ValueData = TRUE OR COALESCE (MovementFloat_VATPercent.ValueData, 0) = 0
                                       ...
                                       THEN CASE WHEN MovementFloat_ChangePercent.ValueData < 0 THEN CAST ( (1 + MovementFloat_ChangePercent.ValueData / 100) * CAST (COALESCE (MIFloat_Price.ValueData, 0) * COALESCE (MIFloat_AmountChangePercent.ValueData, 0) AS NUMERIC (16, 2)) AS NUMERIC (16, 2))
                                                 WHEN MovementFloat_ChangePercent.ValueData > 0 THEN CAST ( (1 + MovementFloat_ChangePercent.ValueData / 100) * CAST (COALESCE (MIFloat_Price.ValueData, 0) * COALESCE (MIFloat_AmountChangePercent.ValueData, 0) AS NUMERIC (16, 2)) AS NUMERIC (16, 2))
                                                 ELSE CAST (COALESCE (MIFloat_Price.ValueData, 0) * COALESCE (MIFloat_AmountChangePercent.ValueData, 0) AS NUMERIC (16, 2))
                                            END
                                       -- если цены без НДС, тогда учитываем или % Скидки или % Наценки для суммы с НДС (этот вариант будет и для НАЛ и для БН)
                                  ELSE CASE WHEN MovementFloat_ChangePercent.ValueData < 0 THEN CAST ( (1 + MovementFloat_ChangePercent.ValueData / 100) * CAST ((1 + COALESCE (MovementFloat_VATPercent.ValueData, 0) / 100) * CAST (COALESCE (MIFloat_Price.ValueData, 0) * COALESCE (MIFloat_AmountChangePercent.ValueData, 0) AS NUMERIC (16, 2)) AS NUMERIC (16, 2)) AS NUMERIC (16, 2))
                                            WHEN MovementFloat_ChangePercent.ValueData > 0 THEN CAST ( (1 + MovementFloat_ChangePercent.ValueData / 100) * CAST ((1 + COALESCE (MovementFloat_VATPercent.ValueData, 0) / 100) * CAST (COALESCE (MIFloat_Price.ValueData, 0) * COALESCE (MIFloat_AmountChangePercent.ValueData, 0) AS NUMERIC (16, 2)) AS NUMERIC (16, 2)) AS NUMERIC (16, 2))
