   ...
   -- Результат
    ...
    -- ограничиваем по виду документа 
     ...
     FROM (SELECT  tmpMI.InvNumber
                      ...
                      , tmpMI.GoodsId       
                      , SUM(tmpMI.Summ) as Summ
                      , SUM(tmpMI.Amount) as Amount

                      , tmpMI.ChildGoodsId     
                      , SUM(tmpMI.ChildSumm) as ChildSumm
                      , SUM(tmpMI.ChildAmount) as ChildAmount
     FROM(
		...
		Group by tmpMI.InvNumber
          ...
          LEFT JOIN ObjectLink AS ObjectLink_GoodsChild_Measure ON ObjectLink_GoodsChild_Measure.ObjectId = Object_GoodsChild.Id 
