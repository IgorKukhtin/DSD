           ...
           FROM (SELECT CASE WHEN inIsPartner = TRUE THEN tmpMovement.JuridicalId ELSE 0 END AS JuridicalId
                ...
                UNION ALL
                 SELECT CASE WHEN inIsPartner = TRUE THEN tmpMovement.JuridicalId ELSE 0 END AS JuridicalId
                      ...
                      , SUM (CASE WHEN tmpMovement.MovementDescId = zc_Movement_ReturnIn() AND MIContainer.AnalyzerId = zc_Enum_ProfitLossDirection_10800() AND ObjectLink_Goods_Measure.ChildObjectId = zc_Measure_Sh() THEN COALESCE (MIContainer.Amount, 0) ELSE 0 END) AS Return_AmountPartner_Sh
