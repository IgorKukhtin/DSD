...
BEGIN
   ...
   , tmpMIContainer AS (SELECT MIContainer.*
                        ...
                        WHERE MIContainer.MovementDescId NOT IN (zc_Movement_Cash(), zc_Movement_BankAccount())  
                       )
   ...
   , tmpMovement AS (SELECT MIContainer.MovementId
                          ...
                          , SUM (CASE WHEN MIContainer.OperDate <= inEndDate AND COALESCE (MIContainer.AnalyzerId, 0) NOT IN (zc_Enum_AnalyzerId_PersonalService_Nalog(), zc_Enum_AnalyzerId_PersonalService_NalogRet()) THEN -1 * MIContainer.Amount ELSE 0 END) AS ServiceSumm
                     FROM tmpContainer
                     ...
                     GROUP BY MIContainer.MovementId 
                            ...
                            , tmpContainer.BranchId
                            , tmpContainer.ServiceDate
                     HAVING 0 <> SUM (CASE WHEN MIContainer.OperDate <= inEndDate AND COALESCE (MIContainer.AnalyzerId, 0) NOT IN (zc_Enum_AnalyzerId_PersonalService_Nalog(), zc_Enum_AnalyzerId_PersonalService_NalogRet()) THEN -1 * MIContainer.Amount ELSE 0 END)
                    )
