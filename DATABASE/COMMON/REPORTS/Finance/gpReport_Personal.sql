...
RETURNS TABLE (MemberCode Integer, MemberName TVarChar
...
BEGIN
  ...
  RETURN QUERY
     ...
     FROM
         (SELECT Operation_all.ObjectId,  Operation_all.MemberId, Operation_all.InfoMoneyId,
          ...
          FROM
          (SELECT Container.Id AS ContainerId, Container.ObjectId, CLO_Member.ObjectId AS MemberId, CLO_InfoMoney.ObjectId AS InfoMoneyId,
            ...
            GROUP BY Container.Id, MIContainer.Containerid, Container.ObjectId, CLO_Member.ObjectId, CLO_InfoMoney.ObjectId
           ) AS Operation_all
          ...
          GROUP BY Operation_all.ObjectId, Operation_all.MemberId, Operation_all.InfoMoneyId, CLO_Car.ObjectId
           ) AS Operation
...
ALTER FUNCTION gpReport_Personal (TDateTime, TDateTime, Integer, Integer, Integer, Integer, TVarChar) OWNER TO postgres;
...
-- тест
 SELECT * FROM gpReport_Personal (inStartDate:= '01.08.2014', inEndDate:= '05.08.2014', inAccountId:= 0, inInfoMoneyId:= 0, inInfoMoneyGroupId:= 0, inInfoMoneyDestinationId:= 0, inSession:= '2');
