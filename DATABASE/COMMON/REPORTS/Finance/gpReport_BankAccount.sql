...
CREATE OR REPLACE FUNCTION gpReport_BankAccount(
    ...
    IN inIsDetail         Boolean   , -- Валюта
    IN inSession          TVarChar    -- сессия пользователя
)
RETURNS TABLE (ContainerId Integer, BankName TVarChar, BankAccountName TVarChar, CurrencyName TVarChar
...
-- тест
-- SELECT * FROM gpReport_BankAccount (inStartDate:= '01.08.2014', inEndDate:= '31.08.2014', inAccountId:= 0, inBankAccountId:=0, inCurrencyId:= 0, inIsDetail:= TRUE, inSession:= zfCalc_UserAdmin());
