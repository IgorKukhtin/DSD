-- Function: gpGet_Movement_BankAccount (Integer, TDateTime, TVarChar)

DROP FUNCTION IF EXISTS gpGet_Movement_BankAccount (Integer, TDateTime, Integer, Integer, TVarChar);
...
BEGIN
     ...
     -- vbUserId := lpCheckRight (inSession, zc_Enum_Process_Get_Movement_BankAccount());
     ...
     THEN
         ...
         SELECT
               0 AS Id
             , CAST (lfGet_InvNumber (0, zc_Movement_BankAccount()) AS TVarChar) AS InvNumber
          ...
          FROM lfGet_Object_Status (zc_Enum_Status_UnComplete()) AS lfGet
            ...
            LEFT JOIN Object AS Object_BankAccount ON Object_BankAccount.Id = inBankAccountId
...
-- тест
-- select * from gpGet_Movement_BankAccount(inMovementId := 0 , inOperDate := ('31.08.2018')::TDateTime , inBankAccountId := 0 , inCurrencyId := 0 ,  inSession := '8');