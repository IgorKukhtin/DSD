...
--    ;
     ...
     -- Дополнительная оплата гривна из за невыданной сдачи
    ...
    -- vbAmountDiscount := vbAmountDiscount - vbAmountDiffLeft_GRN;
     ...
     --raise notice 'D: % %  %  %', vbAmountDiffLeft_GRN, (vbAmountEUR_Over_GRN + vbAmountUSD_Over_GRN + vbAmountCARD_Over + vbAmountGRN_Over), vbAmountDiff, Round(zfCalc_CurrencyTo (vbAmountDiffLeft_GRN, inCurrencyValueEUR, 1), 2);
     ...
     THEN
       ...
       UPDATE _tmpResult SET Amount_GRN = vbAmountEUR_Pay_GRN - COALESCE(tmpData.Amount_GRN, 0) 
...
FROM lpSelect_MI_Child_calc(
    ...
    , inAmountGRN := 0, inAmountUSD := 300 , inAmountEUR := 176 , inAmountCARD := 0, inAmountDiscount_EUR := 0, inAmountDiff :=  40, inAmountRemains_EUR := 0
...
FROM lpSelect_MI_Child_calc(
      inMovementId            := 23589        
    , inUnitId                := (SELECT MLO.ObjectId FROM MovementLinkObject AS MLO WHERE MLO.MovementId = 23589 AND MLO.DescId = zc_MovementLinkObject_From())
    , inAmountGRN := 2743, inAmountUSD := 200 , inAmountEUR := 200 , inAmountCARD := 0, inAmountDiscount_EUR := 0, inAmountDiff :=  0, inAmountRemains_EUR := 0
    , inisDiscount := 'False', inisChangeEUR := 'False'
    , inCurrencyValueUSD := 37.68 , inCurrencyValueInUSD := 37.31 , inParValueUSD := 1
    , inCurrencyValueEUR := 40.95 , inCurrencyValueInEUR := 40.54 , inParValueEUR := 1
    , inCurrencyValueCross := 1.0868 , inParValueCross := 1
