...
RETURNS TABLE (Id Integer, PartionId Integer
             ...
             , PartionMI_Id Integer, SaleMI_Id Integer
...
BEGIN
     ...
     WITH 
         tmpMI_Sale AS (-- продажа покупателю
                        SELECT Movement.Id                                        AS MovementId
                             ...
                             , CAST (zfCalc_SummPriceList (MI_Master.Amount, MIFloat_OperPriceList.ValueData) - COALESCE (MIFloat_TotalChangePercent.ValueData, 0)  AS TFloat) AS TotalSummToPay
   ...
   , tmpMI_Master AS (SELECT MovementItem.Id
          ...
          , tmpMI AS (SELECT COALESCE (tmpMI_Master.Id, 0)                           AS Id
                           ...
                           , COALESCE (tmpMI_Sale.TotalSumm, 0)                      AS TotalSumm
                           , COALESCE (tmpMI_Sale.TotalSummPriceList, 0)             AS TotalSummPriceList
                           ...
                           , COALESCE (tmpMI_Sale.TotalSummToPay, 0)                 AS TotalSummToPay
    ...
    , tmpMI_Child AS (SELECT COALESCE (MovementItem.ParentId, 0)                                                                                                         AS ParentId
       ...
       SELECT
           ...
           , zfCalc_SummIn (MI_Sale.Amount, MIFloat_OperPrice.ValueData, MIFloat_CountForPrice.ValueData) AS TotalSumm
           ...
           , CAST ((zfCalc_SummPriceList (MI_Sale.Amount, MIFloat_OperPriceList.ValueData)
                    - COALESCE (MIFloat_TotalChangePercent.ValueData, 0)
                    - COALESCE (MIFloat_TotalPay.ValueData, 0)
                    - COALESCE (MIFloat_TotalReturn.ValueData, 0))
           ...
           , CAST (zfCalc_SummPriceList (MI_Sale.Amount, MIFloat_OperPriceList.ValueData) - COALESCE (MIFloat_TotalChangePercent.ValueData, 0) AS TFloat) AS TotalSummToPay
