...
DROP FUNCTION IF EXISTS gpSelect_Object_Goods_Choice (Integer, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Object_Goods_Choice(
    IN inUnitId      Integer,       --  подразделение для остатка
...
BEGIN
     ...
     tmpContainer AS (SELECT Container.ObjectId
       ...
       SELECT 
           ...
           , COALESCE (tmpContainer.Amount,0)  ::TFloat AS Remains
           
       FROM Object AS Object_Goods
            LEFT JOIN tmpContainer ON tmpContainer.ObjectId = Object_Goods.Id
     ...
     WHERE Object_Goods.DescId = zc_Object_Goods()
       ...
       AND (COALESCE (tmpContainer.Amount,0) <> 0 OR inIsShowAll = TRUE)
    ;
...
-- тест
-- SELECT * FROM gpSelect_Object_Goods_Choice (0, TRUE, zfCalc_UserAdmin())