...
BEGIN
       ...
       WHERE Object_PartionGoods.GoodsId          = inGoodsId
         ...
         AND Object_PartionGoods.PriceSale        <> COALESCE (OHF_PriceListItem_Value.ValueData, 0) -- меняем - только если значение отличается
      ;
...
/*
       ...
       WHERE OL_PriceListItem_Goods.ChildObjectId = Object_PartionGoods.GoodsId
         AND OL_PriceListItem_Goods.DescId        = zc_ObjectLink_PriceListItem_Goods()
         AND COALESCE (Object_PartionGoods.PriceSale, 0)        <> COALESCE (OHF_PriceListItem_Value.ValueData, 0) -- меняем - только если значение отличается
      ;
*/
-- тест
