...
BEGIN
   ...
   THEN
       RAISE EXCEPTION 'Ошибка.Не возможно определить Пользователя : <%>, <%>, <%>, <%>', inAccountGroupId, inAccountDirectionId, inInfoMoneyDestinationId, inInfoMoneyId;
   END IF;

   IF COALESCE (inAccountGroupId, 0) = 0
   THEN
       RAISE EXCEPTION 'Ошибка.Не возможно определить Счет т.к. не установлено <Группа счета> : <%>, <%>, <%>, <%>', inAccountGroupId, inAccountDirectionId, inInfoMoneyDestinationId, inInfoMoneyId;
   END IF;

   IF COALESCE (inAccountDirectionId, 0) = 0
   THEN
       RAISE EXCEPTION 'Ошибка.Не возможно определить Счет т.к. не установлено <Аналитика счета - направление> : <%>, <%>, <%>, <%>', inAccountGroupId, inAccountDirectionId, inInfoMoneyDestinationId, inInfoMoneyId;
   END IF;

   IF COALESCE (inInfoMoneyDestinationId, 0) = 0 AND COALESCE (inInfoMoneyId, 0) = 0
   THEN
       RAISE EXCEPTION 'Ошибка.Не возможно определить Счет т.к. не установлено <Управленческое назначение> : <%>, <%>, <%>, <%>', inAccountGroupId, inAccountDirectionId, inInfoMoneyDestinationId, inInfoMoneyId;
   END IF;
   ...
   THEN
       RAISE EXCEPTION 'Ошибка.Не возможно использовать удаленный Счет: <%>, <%>, <%>, <%>', lfGet_Object_ValueData (inAccountGroupId), lfGet_Object_ValueData (inAccountDirectionId), lfGet_Object_ValueData (inInfoMoneyDestinationId), lfGet_Object_ValueData (inInfoMoneyId);
   END IF;
   ...
   THEN
       ...
       IF inInsert = FALSE
       THEN
           RAISE EXCEPTION 'Ошибка.В данном документе не возможно создать новый Счет с параметрами: <%>, <%>, <%>, <%>', lfGet_Object_ValueData (inAccountGroupId), lfGet_Object_ValueData (inAccountDirectionId), lfGet_Object_ValueData (inInfoMoneyDestinationId), lfGet_Object_ValueData (inInfoMoneyId);
       END IF;
