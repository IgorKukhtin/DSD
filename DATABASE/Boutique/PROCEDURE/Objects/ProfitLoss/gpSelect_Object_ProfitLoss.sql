...
DROP FUNCTION IF EXISTS gpSelect_Object_ProfitLoss (Boolean,TVarChar);
   ...
   -- PERFORM lpCheckRight(inSession, zc_Enum_Process_Select_Object_ProfitLoss());
     ...
     RETURN QUERY 
       ...
       WHERE Object_ProfitLoss.DescId = zc_Object_ProfitLoss()
         AND (Object_ProfitLoss.isErased = FALSE OR inIsShowAll = TRUE) ;
...
-- тест
-- SELECT * FROM gpSelect_Object_ProfitLoss(True,'2')
