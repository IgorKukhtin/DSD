...
CREATE OR REPLACE FUNCTION zc_Enum_Process_Select_Movement_ReturnOut() RETURNS Integer AS $BODY$BEGIN RETURN (SELECT ObjectId AS Id FROM ObjectString WHERE ValueData = 'zc_Enum_Process_Select_Movement_ReturnOut' AND DescId = zc_ObjectString_Enum()); END; $BODY$ LANGUAGE plpgsql IMMUTABLE;
-- строки
...
CREATE OR REPLACE FUNCTION zc_Enum_Process_Select_MI_ReturnOut() RETURNS Integer AS $BODY$BEGIN RETURN (SELECT ObjectId AS Id FROM ObjectString WHERE ValueData = 'zc_Enum_Process_Select_MI_ReturnOut' AND DescId = zc_ObjectString_Enum()); END; $BODY$ LANGUAGE plpgsql IMMUTABLE;
...
PERFORM lpInsertUpdate_Object_Enum (inId:= zc_Enum_Process_Select_Movement_ReturnOut()
                                  , inDescId:= zc_Object_Process()
                                  , inCode:= 2
                                  , inName:= 'Документ <'||(SELECT ItemName FROM MovementDesc WHERE Id = zc_Movement_ReturnOut())||'> - получение данных.'
                                  , inEnumName:= 'zc_Enum_Process_Select_Movement_ReturnOut');
...
PERFORM lpInsertUpdate_Object_Enum (inId:= zc_Enum_Process_Select_MI_ReturnOut()
                                  , inDescId:= zc_Object_Process()
                                  , inCode:= 7
                                  , inName:= 'Строки документа <'||(SELECT ItemName FROM MovementDesc WHERE Id = zc_Movement_ReturnOut())||'> - получение данных.'
                                  , inEnumName:= 'zc_Enum_Process_Select_MI_ReturnOut');
