-- Function: gpReport_Sale_AnalysisAll()

DROP FUNCTION IF EXISTS gpReport_Sale_AnalysisAll (TDateTime, TDateTime, Integer, Integer, Integer, Integer, Integer, Integer, Integer, TFloat, TFloat, TFloat, TFloat, TFloat, TFloat, Boolean, Boolean, Boolean, Boolean, Boolean, Boolean, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpReport_Sale_AnalysisAll (
...
-- where MIContainer.MovementId =  266623 
 ...
 , _tmpData AS (
     ...
     SELECT _tmpData.*
          , CASE WHEN _tmpData.Tax_Amount    >= inPresent1      THEN 16744448 WHEN _tmpData.Tax_Amount    >= inPresent2      AND _tmpData.Tax_Amount    < inPresent1      THEN zc_Color_Yelow() WHEN _tmpData.Tax_Amount    < inPresent2      THEN zc_Color_Red() END AS Color_Amount
          , CASE WHEN _tmpData.Tax_Summ_curr >= inPresent1_Summ THEN 16744448 WHEN _tmpData.Tax_Summ_curr >= inPresent2_Summ AND _tmpData.Tax_Summ_curr < inPresent1_Summ THEN zc_Color_Yelow() WHEN _tmpData.Tax_Summ_curr < inPresent2_Summ THEN zc_Color_Red() END AS Color_Sum
          , CASE WHEN _tmpData.Tax_Summ_prof >= inPresent1_Prof THEN 16744448 WHEN _tmpData.Tax_Summ_prof >= inPresent2_Prof AND _tmpData.Tax_Summ_prof < inPresent1_Prof THEN zc_Color_Yelow() WHEN _tmpData.Tax_Summ_prof < inPresent2_Prof THEN zc_Color_Red() END AS Color_Prof
     FROM _tmpData
     ;
 END;
$BODY$
