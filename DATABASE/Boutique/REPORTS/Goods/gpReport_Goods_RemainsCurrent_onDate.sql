-- Function:  gpReport_Goods_RemainsCurrent_onDate()

DROP FUNCTION IF EXISTS gpReport_Goods_RemainsCurrent_onDate (TDateTime, Integer, Integer, Integer, Integer, Integer, Integer, Integer, Integer, Boolean, Boolean, Boolean, Boolean, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpReport_Goods_RemainsCurrent_onDate(
...
BEGIN
   ...
   , tmpContainer AS (SELECT Container.WhereObjectId                                   AS UnitId
                           ...
                           , CASE WHEN CLO_Client.ContainerId IS NULL THEN COALESCE (Container.Amount, 0) - SUM (COALESCE (MIContainer.Amount,0) ) ELSE 0 END AS Remains
                           , CASE WHEN CLO_Client.ContainerId > 0     THEN COALESCE (Container.Amount, 0) - SUM (COALESCE (MIContainer.Amount,0) ) ELSE 0 END AS RemainsDebt
                           , COALESCE (Container.Amount, 0) - SUM (COALESCE (MIContainer.Amount,0) )   AS RemainsAll
                      ...
                      /*  AND (Container.Amount <> 0 OR inPartnerId <> 0 OR inIsRemains = TRUE)*/
                      ...
                      GROUP BY Container.WhereObjectId
                           ...
                           , ObjectLink_Partner_Period.ChildObjectId
                           , Object_PartionGoods.BrandId
                           , Object_PartionGoods.PartnerId
                           , Object_PartionGoods.PeriodYear
                           ...
                           , Object_PartionGoods.MovementId
                             -- Если есть права видеть Цену вх.
                           , CASE WHEN vbIsOperPrice = TRUE THEN Object_PartionGoods.OperPrice ELSE 0 END
                           , Object_PartionGoods.CountForPrice

                           , COALESCE (tmpPriceList.OperPriceList, Object_PartionGoods.OperPriceList)
                           , COALESCE (tmpPriceList.CurrencyId_pl, zc_Currency_Basis())
       ...
       , tmpData_All AS (SELECT tmpContainer.UnitId
                              ...
                              , SUM (zfCalc_SummPriceList (tmpContainer.Remains, tmpContainer.OperPriceList))            AS SummDebt
                              , SUM (zfCalc_SummPriceList (tmpContainer.RemainsDebt, tmpContainer.OperPriceList))        AS SummDebt_profit
                         ...
                         WHERE COALESCE (tmpContainer.RemainsAll,0) <> 0
                         -- OR inPartnerId <> 0
                          OR inIsRemains = TRUE
 ...
 , tmpGoodsPrint AS (SELECT Object_GoodsPrint.UnitId
       ...
       , tmpPriceInfo AS (SELECT ObjectLink_PriceListItem_Goods.ChildObjectId     AS GoodsId
        ...
        SELECT
           ...
           , CAST (tmpData.TotalSummPriceList / CASE WHEN tmpData.CurrencyId = zc_Currency_Basis() THEN 1 WHEN tmpData.CurrencyValue_doc <> 0 THEN tmpData.CurrencyValue_doc ELSE 1 END
                                              * CASE WHEN tmpData.CurrencyId = zc_Currency_Basis() THEN 1 WHEN tmpData.ParValue_doc <> 0      THEN tmpData.ParValue_doc      ELSE 1 END
...
-- тест
-- SELECT * FROM gpReport_Goods_RemainsCurrent_onDate (inOperDate := '01.05.2020':: TDateTime, inUnitId:= 0, inBrandId:= 0, inPartnerId:= 0, inPeriodId:= 0, inStartYear:= 0, inEndYear:= 0, inUserId:= 0, inGoodsPrintId:= 0, inisPartion:= true, inisPartner:= FALSE, inisSize:= TRUE, inIsYear:= FALSE, inIsRemains:= FALSE, inSession:= zfCalc_UserAdmin())
