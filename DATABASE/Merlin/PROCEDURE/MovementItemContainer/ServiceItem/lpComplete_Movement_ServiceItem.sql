...
BEGIN
    ...
    -- Проводим документы начисления
    /*PERFORM lpComplete_Movement_Service (inMovementId:= tmpMovement.MovementId
                                       , inUserId    := inUserId
                                        )
    FROM -- Заливаем данные в начисления
        (*/
         PERFORM gpInsertUpdate_Movement_Service (ioId                := tmpMovement.MovementId_service
          ...
          FROM (WITH tmpMI AS (SELECT DATE_TRUNC ('MONTH', Movement.OperDate) AS EndDate
