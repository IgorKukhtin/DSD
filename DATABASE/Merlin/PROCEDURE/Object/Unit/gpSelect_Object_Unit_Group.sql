...
DROP FUNCTION IF EXISTS gpSelect_Object_Unit_Group (Boolean, Boolean, TVarChar);

CREATE OR REPLACE FUNCTION gpSelect_Object_Unit_Group(
...
BEGIN
     ...
     RETURN QUERY
       ...
       WHERE (Object_Unit.isLeaf = inisLeaf OR inisLeaf = TRUE)   --только группы   или все
    ;
...
-- тест
-- SELECT * FROM gpSelect_Object_Unit (TRUE, false, zfCalc_UserAdmin())