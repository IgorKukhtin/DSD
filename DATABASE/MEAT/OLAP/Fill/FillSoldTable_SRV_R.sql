...
, ('WITH tmpPartnerAddress AS (SELECT * FROM Object_Partner_Address_View)
   ...
   ;
    ...
    THEN
        ...
        -- 2. ***если запустили для предыдущего месяца - НУЖНО залить Текущий месяц
        ...
        THEN
            ...
            -- 2.2.Branch
            PERFORM gpInsert_bi_Table_OrderBranch (DATE_TRUNC ('MONTH', CURRENT_DATE), CURRENT_DATE - INTERVAL '1 DAY', zfCalc_UserAdmin());
            PERFORM gpInsert_bi_Table_SendBranch (DATE_TRUNC ('MONTH', CURRENT_DATE), CURRENT_DATE - INTERVAL '1 DAY', zfCalc_UserAdmin());
...
-- SELECT * FROM FillSoldTable ('28.07.2026', '28.07.2026', zfCalc_UserAdmin())
