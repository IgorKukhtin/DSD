...
;
  ...
  --
  UPDATE SoldTable SET SaleBonus = COALESCE(Sale_Summ, 0) - COALESCE(Bonus, 0)                               -- Реализация "минус" Бонус (для % рент. с учетом бонуса)
                   , Sale_Profit = COALESCE(Sale_Summ, 0) - COALESCE(Sale_SummCost, 0)                       -- Прибыль (только реализ)
             , SaleReturn_Profit = COALESCE(Sale_Summ, 0) - COALESCE(Return_Summ, 0)                         -- Прибыль с учетом !!!возврата!!!
               , SaleBonusProfit = COALESCE(Sale_Summ, 0) - COALESCE(Bonus, 0) - COALESCE(Return_Summ, 0)    -- Прибыль с учетом !!!возврата!!! и бонусов
  ;
