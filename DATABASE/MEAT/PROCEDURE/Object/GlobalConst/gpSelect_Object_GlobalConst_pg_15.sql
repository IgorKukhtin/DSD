...
-- FROM pg_stat_activity as a where a.state <> 'active' AND a.query_start < CURRENT_TIMESTAMP - INTERVAL '24 HOURS';
     ...
     -- Результат
     RETURN QUERY
       WITH tmpProcess AS (SELECT * FROM pg_stat_activity WHERE state ILIKE 'active' /*and UseName = 'postgres'*/)
        ...
        --, tmpProcess_Waiting AS (SELECT COUNT (*) :: TVarChar AS Res FROM tmpProcess WHERE waiting = TRUE)
          , tmpProcess_Waiting AS (SELECT COUNT (*) :: TVarChar AS Res FROM tmpProcess WHERE wait_event_Type ILIKE 'Lock')
