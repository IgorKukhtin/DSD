     ...
     THEN
         ...
         THEN
           ...
           FROM dblink('host='||vbHost||' dbname='||vbDBName||' port='||vbPort::Text||' user='||vbUserName||' password='||vbPassword,
                       'SELECT lpInsertFind_Container ('||
                       '  inContainerDescId   := '||CASE WHEN inContainerDescId IS NULL THEN 'NULL' ELSE inContainerDescId::TEXT END||
                       ', inParentId          := '||CASE WHEN inParentId IS NULL THEN 'NULL' ELSE inParentId::TEXT END||
                       ', inObjectId          := '||CASE WHEN NULLIF(inObjectId, 0) IS NULL THEN 'NULL' ELSE inObjectId::TEXT END||
                       ', inJuridicalId_basis := '||CASE WHEN NULLIF(inJuridicalId_basis, 0) IS NULL THEN 'NULL' ELSE inJuridicalId_basis::TEXT END||
                       ', inBusinessId        := '||CASE WHEN NULLIF(inBusinessId, 0) IS NULL THEN 'NULL' ELSE inBusinessId::TEXT END||
                       ', inObjectCostDescId  := '||CASE WHEN inObjectCostDescId IS NULL THEN 'NULL' ELSE inObjectCostDescId::TEXT END||
                       ', inObjectCostId      := '||CASE WHEN inObjectCostId IS NULL THEN 'NULL' ELSE inObjectCostId::TEXT END||
                       ', inDescId_1          := '||CASE WHEN inDescId_1 IS NULL THEN 'NULL' ELSE inDescId_1::TEXT END||
                       ', inObjectId_1        := '||CASE WHEN NULLIF(inObjectId_1, 0) IS NULL THEN 'NULL' ELSE inObjectId_1::TEXT END||
                       ', inDescId_2          := '||CASE WHEN inDescId_2 IS NULL THEN 'NULL' ELSE inDescId_2::TEXT END||
                       ', inObjectId_2        := '||CASE WHEN NULLIF(inObjectId_2, 0) IS NULL THEN 'NULL' ELSE inObjectId_2::TEXT END||
                       ', inDescId_3          := '||CASE WHEN inDescId_3 IS NULL THEN 'NULL' ELSE inDescId_3::TEXT END||
                       ', inObjectId_3        := '||CASE WHEN NULLIF(inObjectId_3, 0) IS NULL THEN 'NULL' ELSE inObjectId_3::TEXT END||
                       ', inDescId_4          := '||CASE WHEN inDescId_4 IS NULL THEN 'NULL' ELSE inDescId_4::TEXT END||
                       ', inObjectId_4        := '||CASE WHEN NULLIF(inObjectId_4, 0) IS NULL THEN 'NULL' ELSE inObjectId_4::TEXT END||
                       ', inDescId_5          := '||CASE WHEN inDescId_5 IS NULL THEN 'NULL' ELSE inDescId_5::TEXT END||
                       ', inObjectId_5        := '||CASE WHEN NULLIF(inObjectId_5, 0) IS NULL THEN 'NULL' ELSE inObjectId_5::TEXT END||
                       ', inDescId_6          := '||CASE WHEN inDescId_6 IS NULL THEN 'NULL' ELSE inDescId_6::TEXT END||
                       ', inObjectId_6        := '||CASE WHEN NULLIF(inObjectId_6, 0) IS NULL THEN 'NULL' ELSE inObjectId_6::TEXT END||
                       ', inDescId_7          := '||CASE WHEN inDescId_7 IS NULL THEN 'NULL' ELSE inDescId_7::TEXT END||
                       ', inObjectId_7        := '||CASE WHEN NULLIF(inObjectId_7, 0) IS NULL THEN 'NULL' ELSE inObjectId_7::TEXT END||
                       ', inDescId_8          := '||CASE WHEN inDescId_8 IS NULL THEN 'NULL' ELSE inDescId_8::TEXT END||
                       ', inObjectId_8        := '||CASE WHEN NULLIF(inObjectId_8, 0) IS NULL THEN 'NULL' ELSE inObjectId_8::TEXT END||
                       ', inDescId_9          := '||CASE WHEN inDescId_9 IS NULL THEN 'NULL' ELSE inDescId_9::TEXT END||
                       ', inObjectId_9        := '||CASE WHEN NULLIF(inObjectId_9, 0) IS NULL THEN 'NULL' ELSE inObjectId_9::TEXT END||
                       ', inDescId_10         := '||CASE WHEN inDescId_10 IS NULL THEN 'NULL' ELSE inDescId_10::TEXT END||
                       ', inObjectId_10       := '||CASE WHEN NULLIF(inObjectId_10, 0) IS NULL THEN 'NULL' ELSE inObjectId_10::TEXT END||
                       ') AS ID') AS 
                         q(Id Integer);  
           ...
           IF COALESCE (vbContainerId, 0) = 0
           THEN
             RAISE EXCEPTION 'Ошибка получение контейнера с основного сервера';
           END IF;
         
           -- добавили Остаток
           INSERT INTO Container (Id, DescId, ObjectId, ParentId, Amount, KeyValue, MasterKeyValue, ChildKeyValue, WhereObjectId)
                          VALUES (vbContainerId, inContainerDescId, inObjectId, CASE WHEN inParentId = 0 THEN NULL ELSE inParentId END, 0, vbKeyValue, vbMasterKeyValue, vbChildKeyValue, vbWhereObjectId)
           ;
         ELSE
         
           RAISE EXCEPTION 'Создавать контейнера на резервном сервере нельзя.'; 
