...
BEGIN
     ...
     raise notice 'Start 01: %', CLOCK_TIMESTAMP();
...
-- !!!ВРЕМЕННО!!!
     ...
     -- таблица - филиал Одесса + филиал Запорожье сохраняем для последующего удаления на основном
     ...
     ANALYSE _tmpUnit_branch;

     raise notice '_tmpUnit_branch: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM _tmpUnit_branch);
     ...
     ANALYSE _tmpContainer_branch;

     raise notice '_tmpContainer_branch: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM _tmpContainer_branch);

     CREATE TEMP TABLE tmpMIContainer ON COMMIT DROP AS
     SELECT MIContainer.*
     FROM MovementItemContainer AS MIContainer
     WHERE MIContainer.OperDate >= CASE WHEN vbStartDate_zavod < inStartDate THEN vbStartDate_zavod ELSE inStartDate END;
     
     ANALYSE tmpMIContainer;

     raise notice 'tmpMIContainer: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpMIContainer);

     CREATE TEMP TABLE tmpContainerS_zavod ON COMMIT DROP AS
      SELECT Container_Summ.*
     ...
     ANALYSE tmpContainerS_zavod;

     raise notice 'tmpContainerS_zavod: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpContainerS_zavod);

     CREATE TEMP TABLE tmpContainerS_branch ON COMMIT DROP AS
      SELECT Container_Summ.*
      FROM Container AS Container_Summ
     ...
     ANALYSE tmpContainerS_branch;

     raise notice 'tmpContainerS_branch: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpContainerS_branch);
         
     CREATE TEMP TABLE tmpContainer_zavod ON COMMIT DROP AS
      SELECT Container.*, COALESCE (Object_Unit.Id, 0) AS UnitId
     ...
     ANALYSE tmpContainer_zavod;

     raise notice 'tmpContainer_zavod: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpContainer_zavod);

     CREATE TEMP TABLE tmpContainer_branch ON COMMIT DROP AS
      SELECT Container.*, COALESCE (Object_Unit.Id, 0) AS UnitId
     ...
     ANALYSE tmpContainer_branch;

     raise notice 'tmpContainer_branch: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpContainer_branch);
     
     CREATE TEMP TABLE tmpMIFloat_Summ ON COMMIT DROP AS
      SELECT MIFloat_Summ.MovementItemId, MIFloat_Summ.DescId, MIFloat_Summ.ValueData
      FROM MovementItem
     ...
     ANALYSE tmpMIFloat_Summ;

     raise notice 'tmpMIFloat_Summ: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpMIFloat_Summ);
     ...
     raise notice 'tmpContainer: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpContainer);
     ...
     raise notice '_tmpMaster: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM _tmpMaster);
     ...
     THEN
         ...
         CREATE TEMP TABLE tmpMIContainer_Summ_Out ON COMMIT DROP
           AS (SELECT DISTINCT MIContainer_Summ_Out.Id, MIContainer_Summ_Out.MovementId, MIContainer_Summ_Out.MovementItemId
               ...
               FROM _tmpMaster
                    INNER JOIN tmpMIContainer AS MIContainer_Summ_Out
         ...
         raise notice 'tmpMIContainer_Summ_Out: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpMIContainer_Summ_Out);
         ...
         raise notice 'tmpMIContainer_Summ_In: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpMIContainer_Summ_In);
         ...
         raise notice 'tmpContainer_count: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM tmpContainer_count);
         ...
         CREATE TEMP TABLE MIContainer_Count_In  ON COMMIT DROP
           AS (SELECT MIContainer_Count_In.Id, MIContainer_Count_In.MovementItemId, MIContainer_Count_In.MovementId, MIContainer_Count_In.ContainerId, SUM (MIContainer_Count_In.Amount) AS Amount
               FROM tmpContainer_count
                    INNER JOIN tmpMIContainer AS MIContainer_Count_In
         ...
         raise notice 'MIContainer_Count_In: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM MIContainer_Count_In);
         ...
         INSERT INTO _tmpChild (MasterContainerId, ContainerId, MasterContainerId_Count, ContainerId_Count, OperCount, isExternal, DescId)
            WITH MIContainer_Count_Out AS (SELECT MIContainer_Count_Out.Id, MIContainer_Count_Out.ParentId, MIContainer_Count_Out.MovementId, MIContainer_Count_Out.MovementDescId, MIContainer_Count_Out.OperDate, MIContainer_Count_Out.MovementItemId, MIContainer_Count_Out.ContainerId, MIContainer_Count_Out.WhereObjectId_Analyzer, SUM (MIContainer_Count_Out.Amount) AS Amount
                                           FROM tmpContainer_count
                                                INNER JOIN tmpMIContainer AS MIContainer_Count_Out
         ...
         raise notice '_tmpChild: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM _tmpChild);
...
*/
     ...
     -- !!! 1-ая итерация для всех !!!
         ...
         raise notice 'UPDATE _tmpMaster: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM _tmpMaster);
...
*/
     ...
     */
     ...
     THEN
         ...
         INSERT INTO _replica.HistoryCost_Rewiring (RewiringUUId, ContainerId, StartDate, EndDate, Price, Price_external, StartCount, StartSumm, IncomeCount, IncomeSumm, CalcCount, CalcSumm, CalcCount_external, CalcSumm_external, OutCount, OutSumm, MovementItemId_diff, Summ_diff)
            ...
            -- Результат
            SELECT vbRewiringUUId, _tmpMaster.ContainerId, inStartDate AS StartDate, inEndDate AS EndDate
            ...
            FROM _tmpMaster
                 ...
                 INNER JOIN _tmpContainer_branch ON _tmpContainer_branch.ContainerId = _tmpMaster.ContainerId;
     ...
     ELSE
         ...
         INSERT INTO _replica.HistoryCost_Rewiring (RewiringUUId, ContainerId, StartDate, EndDate, Price, Price_external, StartCount, StartSumm, IncomeCount, IncomeSumm, CalcCount, CalcSumm, CalcCount_external, CalcSumm_external, OutCount, OutSumm, MovementItemId_diff, Summ_diff)
            SELECT vbRewiringUUId, _tmpMaster.ContainerId, inStartDate AS StartDate
            ...
            FROM _tmpMaster
                 ...
                 -- LEFT JOIN _tmpDiff ON _tmpDiff.ContainerId = _tmpMaster.ContainerId
            WHERE COALESCE (_tmpContainer_branch.ContainerId, 0) = 0;
     ...
     raise notice '_replica.HistoryCost_Rewiring: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM _replica.HistoryCost_Rewiring);
     ...
     vbOperDate_StartBegin:= CLOCK_TIMESTAMP();
        ...
        -- !!!ВРЕМЕННО!!!
        UPDATE _replica.HistoryCost_Rewiring SET Price          = 1.1234 * CASE WHEN HistoryCost_Rewiring.Price < 0 THEN -1 ELSE 1 END
                                               , Price_external = 1.1234 * CASE WHEN HistoryCost_Rewiring.Price < 0 THEN -1 ELSE 1 END
        FROM (SELECT Container.ContainerId
              FROM (SELECT DISTINCT HistoryCost_Rewiring.ContainerId FROM _replica.HistoryCost_Rewiring) AS Container
                   INNER JOIN ContainerLinkObject AS ContainerLO_Goods
                                                  ON ContainerLO_Goods.ContainerId = Container.ContainerId
        ...
        WHERE HistoryCost_Rewiring.StartDate = inStartDate
          AND ABS (HistoryCost_Rewiring.Price) >  10800
          AND HistoryCost_Rewiring.ContainerId = Container.ContainerId
          AND HistoryCost_Rewiring.RewiringUUId = vbRewiringUUId
       ;

     raise notice 'UPDATE _replica.HistoryCost_Rewiring: % <%>', CLOCK_TIMESTAMP(), (SELECT count(*) FROM _replica.HistoryCost_Rewiring);
    ...
    THEN
        RAISE EXCEPTION 'Тест прошел успешно для <%> <%>', inSession, (SELECT count(*) FROM _replica.HistoryCost_Rewiring);
    END IF;*/
...
-- SELECT * FROM _replica.gpInsertUpdate_HistoryCost_Rewiring (inStartDate:= '01.09.2023', inEndDate:= '30.09.2023', inBranchId:= 8379, inItearationCount:= 50, inDiffSumm:= 1, inSession:= zfCalc_UserAdmin());