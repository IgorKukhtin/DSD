...
BEGIN
  ...
  THEN
    EXECUTE 'select last_value from '||pg_get_serial_sequence('Movement', 'id')
    INTO vbLastId;
    
    IF vbLastId = current_row.Id
    THEN
      RAISE EXCEPTION 'Ошибка. Запрещено создание нового документа <%> <%> <%>', 
        (SELECT MovementDesc.ItemName FROM MovementDesc WHERE MovementDesc.Id = current_row.DescId), 
        current_row.InvNumber, current_row.OperDate;   
    END IF;
  END IF;
