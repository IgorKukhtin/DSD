...
BEGIN
  ...
  THEN
    ...
    BEGIN
      vbSequence := pg_get_serial_sequence(TG_TABLE_NAME, 'id');        
    EXCEPTION
      WHEN others THEN vbSequence := '';
    END;
    
    IF COALESCE (vbSequence, '') <> ''
    THEN
      EXECUTE 'select last_value from '||vbSequence
      INTO vbLastId;
      
      IF vbLastId = current_row.Id
      THEN
        RAISE EXCEPTION 'Ошибка. Вставка звписей в таблицу <%> запрещено.', TG_TABLE_NAME;   
      END IF;
    END IF;
  END IF;
