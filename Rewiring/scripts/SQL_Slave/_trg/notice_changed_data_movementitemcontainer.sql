...
BEGIN
  ...
  -- Если не роль для перепроведения выходим
  IF _replica.zc_isUserRewiring() = FALSE
  THEN
    RETURN current_row;
  END IF;
  
  -- Провкерякм создания с использованием Id тестового сервера
  IF (TG_OP ILIKE 'INSERT') 
  THEN
    EXECUTE 'select last_value from '||pg_get_serial_sequence('movementitemcontainer', 'id')
    INTO vbLastId;
    
    IF vbLastId = current_row.Id
    THEN
      RAISE EXCEPTION 'Ошибка. Запрещено создание движения по документу';   
    END IF;
  END IF;
  
  RAISE EXCEPTION 'Ошибка. Запрещено изменение движения по документу';  
