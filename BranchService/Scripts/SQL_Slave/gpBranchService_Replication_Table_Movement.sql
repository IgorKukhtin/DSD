-- Function: _replica.gpBranchService_Replication_Table_Movement (TVarChar, TVarChar)

DROP FUNCTION IF EXISTS _replica.gpBranchService_Replication_Table_Movement (TVarChar, Text, Text, Text, Text, Integer, Integer, TVarChar);

CREATE OR REPLACE FUNCTION _replica.gpBranchService_Replication_Table_Movement(
     ...
     FROM _replica.gpBranchService_Select_MasterConnectParams(inSession);
       ...
       vbQueryText := 'WITH ins AS ('||
                      ...
                      'FROM dblink(''host='||vbHost||
                                   ...
                                   'SELECT '||vbColumn_Name_Master||' FROM '||inTableName||' '||
                                   inLike||' WHERE '||inTableName||'.DescId = '||inDescId||'() AND '||inWhere||' '||
