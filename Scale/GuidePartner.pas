...
type
  TGuidePartnerForm = class(TForm)
    ...
    procedure EditPartnerNameExit(Sender: TObject);
...
begin
    if Key=13 then
      ...
      then ButtonChoiceItemClick(Self)
        ...
        if (ActiveControl=EditPartnerCode)
        then if trim(EditPartnerCode.Text)='' then ActiveControl:=EditPartnerName else EditPartnerCodeExit(EditPartnerCode)
        else if (ActiveControl=EditPartnerName)
             then if trim(EditPartnerName.Text)='' then ActiveControl:=EditPartnerCode else EditPartnerNameExit(EditPartnerName);
...
begin
     if fEnterPartnerCode then
       with CDS do begin
           Filtered:=false;
           if trim(EditPartnerCode.Text)<>'' then Filtered:=true;
       end;
end;
{------------------------------------------------------------------------------}
...
{------------------------------------------------------------------------------}
procedure TGuidePartnerForm.EditPartnerCodeExit(Sender: TObject);
begin
     if fStartWrite=true then exit;

     if (CDS.Filtered=false)and(Length(trim(EditPartnerCode.Text))>0)
     then begin fEnterPartnerCode:=true;{CDS.Filtered:=true;}end;
end;
{------------------------------------------------------------------------------}
...
begin
     if fEnterPartnerName then
       with CDS do begin
           Filtered:=false;
           if trim(EditPartnerName.Text)<>'' then Filtered:=true;
       end;
end;
{------------------------------------------------------------------------------}
...
{------------------------------------------------------------------------------}
procedure TGuidePartnerForm.EditPartnerNameExit(Sender: TObject);
var Code_begin:Integer;
begin
     if fStartWrite=true then exit;

     if (CDS.Filtered=false)and(Length(trim(EditPartnerName.Text))>0)
     then begin fEnterPartnerName:=true;{CDS.Filtered:=true;}end;
end;
{------------------------------------------------------------------------------}
