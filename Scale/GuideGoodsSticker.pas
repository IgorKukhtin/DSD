    ...
    if (Key=27) then
      ...
      then CancelCxFilter
           ...
           then actExitExecute(Self)
                ...
                begin
                     if not Execute (false, true, false, false) then begin ShowMessage ('Для отмены ПЕЧАТИ ЭТИКЕТОК необходимо ввести пароль.'); exit; end;
                     //
...
begin
     ...
     begin
          ...
          // после сохранения - еще ПЕЧАТЬ ИТОГОВОЙ
          try
            frxDBDHeaderForm.UserName:= 'frxDBDHeader_';
            //
...
begin
     CanClose:=false;
           ...
           then CanClose:=true
                ...
                begin
                     if not Execute (false, true, false, false) then begin ShowMessage ('Для отмены взвешивания необходимо ввести пароль.'); exit; end;
                     //
                     if DMMainScaleForm.gpGet_Scale_PSW_delete (StringValueEdit.Text) <> ''
