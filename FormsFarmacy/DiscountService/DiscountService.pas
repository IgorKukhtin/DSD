            ...
            try
              ...
              RESTRequest.AddParameter('medicard',
                EncodeString('<?xml version="1.0" encoding="UTF-8"?>'#13#10 +
                             ...
                             '  <discont_value>' + CurrToStrXML(FDiscontАbsolute) + '</discont_value>'#13#10 +
...
begin
  ...
  try
    ...
    begin
      ...
      //
      if (lDiscountExternalId > 0) and ((gCode = 1) or (gCode = 2) and (gUserName <> '') or (gCode in [3, 4, 5, 6])) and
      ...
      else  if (BarCode_find <> '') and (gCode in [3, 5, 6]) then
      begin
...
begin
  if (gCode in [3, 5, 6]) and (FIdCasual <> '') then
  begin
    Result := True;
