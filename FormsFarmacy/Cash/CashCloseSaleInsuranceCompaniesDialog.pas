unit CashCloseSaleInsuranceCompaniesDialog;
...
type
  TCashCloseSaleInsuranceCompaniesDialogForm = class(TAncestorDialogForm)
...
var
  CashCloseSaleInsuranceCompaniesDialogForm: TCashCloseSaleInsuranceCompaniesDialogForm;
...
procedure TCashCloseSaleInsuranceCompaniesDialogForm.edSalerCashPropertiesChange(Sender: TObject);
var
  tmpVal: Currency;
begin
...
procedure TCashCloseSaleInsuranceCompaniesDialogForm.ParentFormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  try
    spSelect.ParamByName('inMovementId').Value := FormParams.ParamByName('Id').Value;
    spSelect.Execute;
  ...
  try
    spUpdate_CashRegister.ParamByName('inMovementId').Value := FormParams.ParamByName('Id').Value;
...
procedure TCashCloseSaleInsuranceCompaniesDialogForm.actShowExecute(Sender: TObject);
begin
  inherited;
    try
      FSummaTotal := spGet_Movement.ParamByName('TotalSumm').AsFloat;
      FPaidTypeTemp := -1;
...
procedure TCashCloseSaleInsuranceCompaniesDialogForm.Add_Log_XML(AMessage: String);
var
  F: TextFile;
begin
  try
...
begin
  ...
  try
    try
      ...
      if (nSumAll + Disc) <> FSummaTotal then
      begin
        ShowMessage ('Проверьте сумму продажи.');
        exit;
      end;
...
initialization
  RegisterClass(TCashCloseSaleInsuranceCompaniesDialogForm);
