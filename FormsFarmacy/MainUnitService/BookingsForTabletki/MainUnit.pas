...
begin
  ...
  try
    ...
    if MinutesBetween(FDataСancelled, Now()) > 50 then
    begin
...
begin
  ...
  begin
     if TabletkiAPI.ErrorsText = '' then Add_Log('Нет новых отказов.')
     else Add_Log(TabletkiAPI.ErrorsText);
  end;

  if TabletkiAPI.BookingsHeadCDS.IsEmpty then Exit;
  Add_Log('Аптека: ' + qryUnit.FieldByName('Name').AsString);
  Add_Log('   отказов: ' + IntToStr(TabletkiAPI.BookingsHeadCDS.RecordCount));
  ...
  try
    ...
    begin
      ...
      if spSetErasetBooking.Params.ParamByName('outisOk').AsBoolean then
        Add_Log('   отказ: ' + TabletkiAPI.BookingsHeadCDS.FieldByName('bookingId').AsString);
...
begin
  ...
  try
    ...
    FDataСancelled := Ini.ReadDateTime('Common', 'DataСancelled', Date);
    Ini.WriteDateTime('Common', 'DataСancelled', FDataСancelled);
...
procedure TMainForm.SetDataСancelled;
var
  Ini: TIniFile;
begin
  Ini := TIniFile.Create(ExtractFilePath(Application.ExeName) + 'BookingsForTabletki.ini');
  ...
  try
    ...
    Ini.WriteDateTime('Common', 'DataСancelled', Now());
