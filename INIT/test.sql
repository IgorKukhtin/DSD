    WITH tmpContainer AS (SELECT Container.Id AS ContainerId, Container.ObjectId AS AccountId, Container.Amount
                 ...
                 --          FROM (SELECT AccountId FROM Object_Account_View WHERE  AccountCode in ( 30505, 20503, 70105, 100301, 110101 ) ) AS tmpAccount -- счет 
                           FROM (SELECT Id AS AccountId FROM Object WHERE DescId = zc_Object_Account() and ObjectCode in ( 30505, 30508, 20503, 70105, 100301, 110101 )  /*WHERE Object_Account_View.AccountDirectionCode IN (20500, 30500, 70100, 100300, 110000)*/) AS tmpAccount -- счет 
                               JOIN Container ON Container.ObjectId = tmpAccount.AccountId
                                             AND Container.DescId = zc_Container_Summ()
                         )
    SELECT  (tmpReport.InvNumber) AS InvNumber
         , tmpReport.OperDate
       ...
       (SELECT ContainerLO_InfoMoney.ObjectId AS InfoMoneyId
        ...
        FROM
            (SELECT tmpContainer.ContainerId
                  ...
                  , tmpContainer.Amount - COALESCE (SUM (CASE WHEN MIContainer.OperDate > '31.10.2013' THEN  MIContainer.Amount ELSE 0 END), 0) AS SummEnd
                  , 0 AS SummIn
                  , 0 AS SummOut
                  , 0 AS MovementDescId
                  , 0 AS MovementId
             ...
             FROM tmpContainer
                  LEFT JOIN MovementItemContainer AS MIContainer ON MIContainer.Containerid = tmpContainer.ContainerId
                                                                AND MIContainer.OperDate >= '01.10.2013'
             ...
             HAVING (tmpContainer.Amount - COALESCE (SUM (MIContainer.Amount), 0) <> 0)
                 OR (tmpContainer.Amount - COALESCE (SUM (CASE WHEN MIContainer.OperDate > '31.10.2013' THEN  MIContainer.Amount ELSE 0 END), 0) <> 0)
            UNION ALL
             SELECT tmpMIReport.ContainerId
                  ...
                  , Movement.InvNumber
                  , COALESCE (ContainerLO_Personal.ObjectId, 0 /*COALESCE (MI_Personal.ObjectId, MovementLO_PersonalDriver.ObjectId)*/) AS PersonalId_inf
                  , COALESCE (ContainerLO_Car.ObjectId, 0 /*MILinkObject_Car.ObjectId*/) AS CarId_inf
                  , tmpMIReport.AccountId_inf
                  , COALESCE (/*MI_Route.ObjectId*/0, tmpMIReport.RouteId_inf) AS RouteId_inf
               ...
               FROM (SELECT tmpContainer.ContainerId
                     ...
                     FROM tmpContainer
                          ...
                          JOIN MovementItemReport AS MIReport ON MIReport.ReportContainerId = ReportContainerLink.ReportContainerId
                                                             AND MIReport.OperDate BETWEEN '01.10.2013' AND '31.10.2013'
...
--                    , MI_Route.ObjectId
       ...
       LEFT JOIN ObjectLink AS ObjectLink_InfoMoney_InfoMoneyDestination
                            ON ObjectLink_InfoMoney_InfoMoneyDestination.ObjectId = View_InfoMoney.Id
       ...
       LEFT JOIN ObjectLink AS ObjectLink_InfoMoney_InfoMoneyGroup
                            ON ObjectLink_InfoMoney_InfoMoneyGroup.ObjectId = View_InfoMoney.Id
       ...
       LEFT JOIN ObjectLink AS ObjectLink_Account_AccountGroup
                            ON ObjectLink_Account_AccountGroup.ObjectId = View_Account.Id
       ...
       LEFT JOIN ObjectLink AS ObjectLink_Account_AccountDirection
                            ON ObjectLink_Account_AccountDirection.ObjectId = View_Account.Id
       ...
       LEFT JOIN ObjectLink AS ObjectLink_Account_AccountGroup_inf
                            ON ObjectLink_Account_AccountGroup_inf.ObjectId = View_Account_inf.Id
       ...
       LEFT JOIN ObjectLink AS ObjectLink_Account_AccountDirection_inf
                            ON ObjectLink_Account_AccountDirection_inf.ObjectId = View_Account_inf.Id
       ...
       LEFT JOIN ObjectLink AS ObjectLink_ProfitLoss_ProfitLossGroup_inf
                            ON ObjectLink_ProfitLoss_ProfitLossGroup_inf.ObjectId = View_ProfitLoss_inf.Id
       ...
       LEFT JOIN ObjectLink AS ObjectLink_ProfitLoss_ProfitLossDirection_inf
                            ON ObjectLink_ProfitLoss_ProfitLossDirection_inf.ObjectId = View_ProfitLoss_inf.Id
