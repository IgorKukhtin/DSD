...
//    property RectangleColorColumn: TcxGridColumn read FRectangleColorColumn write FRectangleColorColumn;
  ...
  TСhangeCellData = class(TPersistent)
  private
  ...
  private
    ...
    FСhangeCellAction: TCustomAction;
    FСhangeCellData: TСhangeCellData;
  ...
  published
    ...
    property СhangeCellAction: TCustomAction  read FСhangeCellAction write FСhangeCellAction;
    // Новые координаты для сохранения в базу
    property СhangeCellData: TСhangeCellData  read FСhangeCellData write FСhangeCellData;
    // Служебные поля
...
constructor TСhangeCellData.Create;
begin
...
destructor TСhangeCellData.Destroy;
begin
  FLeftParam.Free;
  FTopParam.Free;
  FWidthParam.Free;
...
begin
  ...
  FСhangeCellData := TСhangeCellData.Create;
...
begin
  ...
  FFocusSetID := FFocusedIdParam.Value;
...
begin
  ...
  begin
    ...
    begin
      FСhangeCellData.LeftParam.Value := FControlDown.Left;
      FСhangeCellData.TopParam.Value := FControlDown.Top;
      FСhangeCellData.WidthParam.Value := FControlDown.Width;
      FСhangeCellData.HeightParam.Value := FControlDown.Height;

      if Assigned(FСhangeCellAction) then //FСhangeCellAction.Execute;
      begin
        FTimerProcess := FTimerProcess or 1;
        FСhangeCellAction.Execute;
      end;
    end;
  end;
end;
...
begin
  ...
  try
    ...
    if (FTimerProcess and 1) > 0 then if Assigned(FСhangeCellAction) then FСhangeCellAction.Execute;
