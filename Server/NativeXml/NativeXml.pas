      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              // test!
              DoDebugOut(Self, wsInfo, Format('SubType=%s, SubName=%s, SubValueChars=%d',
                [CN.Nodes[i].ElementTypeName, CN.Nodes[i].Name, Length(CN.Nodes[i].Value)]));
...
begin
  ...
  while i <= L do
  begin
// bugfix 13jul2012(thx Christian)
//    if ((EolStyle = esCR) and (S[i] = #$0D)) or
//       ((EolStyle in [esLF, esCRLF]) and (S[i] = #$0A)) then
    if ((EolStyle in [esCR, esCRLF]) and (S[i] = #$0D)) then
      inc(IntervalCount);
    inc(i);
  end;
