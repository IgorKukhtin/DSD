...
begin
  result := InsertUpdateMovementProductionSeparate(0, 'Ќомер 1' ,Date, '12.05.13', 0, 0);
end;
...
begin
  if InsertedIdMovementList.Find(IntToStr(Id), Index) then begin
     // здесь мы разрешаем удал€ть “ќЋ№ ќ вставленные в момент теста данные
     DeleteRecord(Id);
     InsertedIdMovementList.Delete(Index);
  end
  else
     raise Exception.Create('ѕопытка удалить запись, вставленную вне теста!!!');
end;
