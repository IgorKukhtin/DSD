...
type
  ...
  public
    function InsertUpdateUnit(Id, Code: Integer; Name: String;
                              ParentId, JuridicalId: integer): integer;
...
begin
 ...
 // добавляем группу 1
  ...
  try
    ...
    try
      ObjectTest.InsertUpdateUnit(Id, -1, 'Группа 1 - тест', Id, 0);
    ...
    Id2 := ObjectTest.InsertUpdateUnit(0, -2, 'Группа 2 - тест', Id, 0);
    try
      with ObjectTest.GetRecord(Id) do begin
      ...
      try
        ObjectTest.InsertUpdateUnit(Id, -1, 'Группа 1 - тест', Id2, 0);
        Check(false, 'Нет сообщение об ошибке');
      except
      ...
      Id3 := ObjectTest.InsertUpdateUnit(0, -3, 'Группа 3 - тест', Id2, 0);
      try
        with ObjectTest.GetRecord(Id2) do begin
        ...
        try
          ObjectTest.InsertUpdateUnit(Id, -1, 'Группа 1 - тест', Id3, 0);
          Check(false, 'Нет сообщение об ошибке');
        except

        end;
        Check((ObjectTest.GetDataSet.RecordCount = RecordCount + 3), 'Количество записей не изменилось');
        ObjectTest.InsertUpdateUnit(Id3, -3, 'Группа 3 - тест', 0, 0);
        with ObjectTest.GetRecord(Id2) do begin
           Check(FieldByName('isLeaf').AsBoolean, 'Не правильно установлено свойство isLeaf Id = ' + IntToStr(Id2));
        end;
      finally
      ...
      ObjectTest.InsertUpdateUnit(Id2, -3, 'Группа 3 - тест', 0, 0);
      with ObjectTest.GetRecord(Id) do begin
         Check(FieldByName('isLeaf').AsBoolean, 'Не правильно установлено свойство isLeaf Id = ' + IntToStr(Id));
      end;
    finally
...
begin
  result := InsertUpdateUnit(0, -11100, 'Test - Подразделение', 0, 0);
  inherited;
end;
