...
begin
     ...
     with fromQuery,Sql do begin
        ...
        Clear;
        Add('select Bill.Id as ObjectId');
        //Add('     , case when FromId_Postgres is null then '+FormatToVarCharServer_notNULL('ошибка склад-')+'+UnitFrom.UnitName +'+FormatToVarCharServer_notNULL('-')+' else '+FormatToVarCharServer_notNULL('')+' end + cast (Bill.BillNumber as TVarCharMedium) as InvNumber');
        Add('     , case when FromId_Postgres is null then '+FormatToVarCharServer_notNULL('ошибка склад-')+'+UnitFrom.UnitName +'+FormatToVarCharServer_notNULL('-')+' else '+FormatToVarCharServer_notNULL('')+' end as InvNumber');
        Add('     , Bill.BillDate as OperDate');
...
// +' and Bill.Id = 1260716'
        ...
        Add('union all');
        Add('select Bill.Id as ObjectId');
        //Add('     , '+FormatToVarCharServer_notNULL('f-')+'+case when FromId_Postgres is null then '+FormatToVarCharServer_notNULL('ошибка склад')+'+ fUnit.UnitName + '+FormatToVarCharServer_notNULL('-')+' else '+FormatToVarCharServer_notNULL('')+' end + cast (Bill.BillNumber as TVarCharMedium) as InvNumber');
        Add('     , '+FormatToVarCharServer_notNULL('f-')+'+case when FromId_Postgres is null then '+FormatToVarCharServer_notNULL('ошибка склад')+'+ fUnit.UnitName + '+FormatToVarCharServer_notNULL('-')+' else '+FormatToVarCharServer_notNULL('')+' end as InvNumber');
        ...
        Add('     left outer join dba.fUnit_byLoad AS fUnitTo on fUnitTo.UnitId = Bill.ToId and fUnitTo.Id_byLoad <> 0');

        Add('     left outer join dba.fUnit on fUnit.Id = Bill.FromId');

        Add('     left outer join dba.Unit AS UnitFrom on UnitFrom.Id = fUnitFrom.Id_byLoad');
        Add('     left outer join dba.Unit AS UnitTo on UnitTo.Id = fUnitTo.Id_byLoad');
        ...
        Add('     left outer join dba._pgPersonal as pgPersonalFrom on pgPersonalFrom.Id=UnitFrom.PersonalId_Postgres'
           +'                                                      and pgPersonalFrom.Id2_Postgres>0');
        Add('     left outer join dba.MoneyKind on MoneyKind.Id = Bill.MoneyKindId');
        Add('     left outer join dba.Unit as Unit_RouteSorting on Unit_RouteSorting.Id = Bill.RouteUnitId and 1=0');
        Add('     left outer join dba._pgPersonal on _pgPersonal.Id = Unit_RouteSorting.PersonalId_Postgres');
        Add('     left outer join dba._toolsView_Client_isChangeDate on _toolsView_Client_isChangeDate.ClientId = UnitTo.ID');
        Add('where Bill.BillDate between '+FormatToDateServer_notNULL(StrToDate(StartDateEdit.Text))+' and '+FormatToDateServer_notNULL(StrToDate(EndDateEdit.Text))
...
begin
     ...
     with fromQuery,Sql do begin
        ...
        Add('     , case when fBillItems.BillItemsId_byLoad is not null and BillItems.OperPrice<>fBillItems.OperPrice then '+FormatToVarCharServer_notNULL('f-ошибка цена')
           +'            when fBillItems.BillItemsId_byLoad is not null and GoodsProperty.Id is null then '+FormatToVarCharServer_notNULL('f-ошибка товар')
           +'            when fBillItems.BillItemsId_byLoad is not null and isnull(KindPackage.Id,0)<>isnull(GoodsProperty_Detail.KindPackageId,-1) then '+FormatToVarCharServer_notNULL('f-ошибка вид')
        ...
        Add('     , case when isError=zc_rvYes() then '+FormatToVarCharServer_notNULL('f-ошибка товар или вид')+'+case when isnull(pgPersonalFrom.Id2_Postgres, pgUnitFrom.Id_Postgres) is null then fUnit.UnitName + '+FormatToVarCharServer_notNULL('-')+' else '+FormatToVarCharServer_notNULL('')+' end + cast (Bill.BillNumber as TVarCharMedium)'
...
//        Add('                                    and Goods.ParentId not in(686,1670,2387,2849,5874)'); // Тара + СЫР + ХЛЕБ + С-ПЕРЕРАБОТКА + ТУШЕНКА
        ...
        begin
             ...
             //
             if (FieldByName('isError').AsInteger=FieldByName('zc_rvYes').AsInteger)
             then begin
