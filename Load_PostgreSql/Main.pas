...
<<<<<<< f085f035c512b56e324ebc0ef140b0d9af39a2f5
    ...
    function pLoadDocumentItem_ProductionUnionMaster
      (SaveCount: Integer): Integer;
    procedure pLoadDocumentItem_ProductionUnionChild(SaveCount1,
      SaveCount2: Integer);
    function pLoadDocument_ProductionSeparate: Integer;
    function pLoadDocumentItem_ProductionSeparateMaster
      (SaveCount: Integer): Integer;
    procedure pLoadDocumentItem_ProductionSeparateChild(SaveCount1,
      SaveCount2: Integer);
...
begin
  if MessageDlg('Действительно остановить загрузку?', mtConfirmation,
...
begin
  if not fStop then
    if MessageDlg('Действительно остановить загрузку и выйти?', mtConfirmation,
      [mbYes, mbNo], 0) = mrYes then
      fStop := true;
  //
  if fStop then
...
begin
  ...
  begin
    ...
    try
      ...
      ShowMessage('fExecSqFromQuery' + #10 + #13 + mySql);
      Result := false;
      exit;
    end;
  end;
...
begin
  if trim(_Value) = '' then
    ...
    Result := chr(39) + trim(_Value) + chr(39);
end;
...
begin
  ...
  Result := chr(39) + IntToStr(Year) + '-' + IntToStr(Month) + '-' +
    IntToStr(Day) + chr(39);
end;
...
begin
  Result := false;
  ShowMessage
    ('ERROR - toStoredProc_ZConnection on myExecToStoredProc_ZConnection')
  ...
  for i := 0 to ComponentCount - 1 do
    if (Components[i] is TCheckBox) then
      if Components[i].Tag = 10 then
        TCheckBox(Components[i]).Checked := cbAllGuide.Checked;
end;
  ...
  for i := 0 to ComponentCount - 1 do
    if (Components[i] is TCheckBox) then
      if Components[i].Tag = 30 then
        TCheckBox(Components[i]).Checked := cbAllCompleteDocument.Checked;
end;
  ...
  for i := 0 to ComponentCount - 1 do
    if (Components[i] is TCheckBox) then
      if Components[i].Tag = 20 then
        TCheckBox(Components[i]).Checked := cbAllDocument.Checked;
end;
...
begin
  if (not cbComplete.Checked) and (not cbUnComplete.Checked) then
...
begin
  ...
  StartDateEdit.Text :=
    DateToStr(StrToDate('01.' + IntToStr(Month) + '.' + IntToStr(Year)));
  ...
  begin
    ...
    Month := Month + 1;
  EndDateEdit.Text := DateToStr(StrToDate('01.' + IntToStr(Month) + '.' +
    IntToStr(Year)) - 1);
  EndDateCompleteEdit.Text := EndDateEdit.Text;
  // коннектимся под админом
...
var
  ...
  StrTime: String;
begin
  if MessageDlg('Действительно загрузить выбранные справочники?',
  ...
  Gauge.Visible := true;
  //
  if cbSetNull_Id_Postgres.Checked then
  begin
    if MessageDlg
      ('Действительно set СПРАВОЧНИКИ+ДОКУМЕНТЫ.Sybase.ВСЕМ.Id_Postgres = null?',
      mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
      exit;
  ...
  if not fStop then
    pLoadGuide_GoodsKind;
  if not fStop then
    pLoadGuide_PaidKind;
  if not fStop then
    pLoadGuide_ContractKind;
  if not fStop then
    pLoadGuide_JuridicalGroup;
  if not fStop then
    pLoadGuide_Juridical(false);
  if not fStop then
    pLoadGuide_Partner(false);
  ...
  if not fStop then
    pLoadGuide_Unit;
  if not fStop then
    pLoadGuide_Member_andPersonal;
  if not fStop then
    pLoadGuide_RouteSorting;

  if not fStop then
    pLoadGuide_PriceList;
  if not fStop then
    pLoadGuide_PriceListItems;
  if not fStop then
    pLoadGuide_GoodsProperty;
  if not fStop then
    pLoadGuide_GoodsPropertyValue;

  if not fStop then
    pLoadGuide_InfoMoneyGroup;
  if not fStop then
    pLoadGuide_InfoMoneyDestination;
  if not fStop then
    pLoadGuide_InfoMoney;
  if not fStop then
    pLoadGuide_AccountGroup;
  if not fStop then
    pLoadGuide_AccountDirection;
  if not fStop then
    pLoadGuide_Account;
  if not fStop then
    pLoadGuide_ProfitLossGroup;
  if not fStop then
    pLoadGuide_ProfitLossDirection;
  if not fStop then
    pLoadGuide_ProfitLoss;
  //
  Gauge.Visible := false;
  DBGrid.Enabled := true;
  OKGuideButton.Enabled := true;
  ...
  //
  tmpDate2 := Now;
  if (tmpDate2 - tmpDate1) >= 1 then
    StrTime := DateTimeToStr(tmpDate2 - tmpDate1)
  else
  begin
  ...
  if fStop then
    ShowMessage('Справочники НЕ загружены. Time=(' + StrTime + ').')
  else
    ShowMessage('Справочники загружены. Time=(' + StrTime + ').');
  //
  fStop := true;
end;
...
begin
  if MessageDlg('Действительно загрузить выбранные документы?', mtConfirmation,
    [mbYes, mbNo], 0) <> mrYes then
    exit;
  fStop := false;
  DBGrid.Enabled := false;
  ...
  if cbSetNull_Id_Postgres.Checked then
  begin
    if MessageDlg('Действительно set ДОКУМЕНТЫ.Sybase.ВСЕМ.Id_Postgres = null?',
      mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
      exit;
    pSetNullDocument_Id_Postgres;
  end;
  ...
  //
  if not fStop then
    pLoadGuide_Juridical(true);
  if not fStop then
    pLoadGuide_Partner(true);
  //
  if not fStop then
    myRecordCount1 := pLoadDocument_Income;
  if not fStop then
    pLoadDocumentItem_Income(myRecordCount1);

  if not fStop then
    myRecordCount1 := pLoadDocument_IncomePacker;
  if not fStop then
    pLoadDocumentItem_IncomePacker(myRecordCount1);

  if not fStop then
    myRecordCount1 := pLoadDocument_SendUnit;
  if not fStop then
    pLoadDocumentItem_SendUnit(myRecordCount1);
  if not fStop then
    myRecordCount1 := pLoadDocument_SendPersonal;
  if not fStop then
    pLoadDocumentItem_SendPersonal(myRecordCount1);
  if not fStop then
    myRecordCount1 := pLoadDocument_SendUnitBranch;
  if not fStop then
    pLoadDocumentItem_SendUnitBranch(myRecordCount1);

  if not fStop then
    myRecordCount1 := pLoadDocument_Sale;
  if not fStop then
    pLoadDocumentItem_Sale(myRecordCount1);

  if not fStop then
    myRecordCount1 := pLoadDocument_ReturnOut;
  if not fStop then
    pLoadDocumentItem_ReturnOut(myRecordCount1);
  if not fStop then
    myRecordCount1 := pLoadDocument_ReturnIn;
  if not fStop then
    pLoadDocumentItem_ReturnIn(myRecordCount1);

  if not fStop then
    myRecordCount1 := pLoadDocument_ProductionUnion;
  if not fStop then
    myRecordCount2 := pLoadDocumentItem_ProductionUnionMaster(myRecordCount1);
  if not fStop then
    pLoadDocumentItem_ProductionUnionChild(myRecordCount1, myRecordCount2);
  if not fStop then
    myRecordCount1 := pLoadDocument_ProductionSeparate;
  if not fStop then
    myRecordCount2 := pLoadDocumentItem_ProductionSeparateMaster
      (myRecordCount1);
  if not fStop then
    pLoadDocumentItem_ProductionSeparateChild(myRecordCount1, myRecordCount2);

  if not fStop then
    myRecordCount1 := pLoadDocument_Loss;
  if not fStop then
    pLoadDocumentItem_Loss(myRecordCount1);

  if not fStop then
    myRecordCount1 := pLoadDocument_Inventory;
  if not fStop then
    pLoadDocumentItem_Inventory(myRecordCount1);

  if not fStop then
    myRecordCount1 := pLoadDocument_Zakaz;
  if not fStop then
    pLoadDocumentItem_Zakaz(myRecordCount1);
  //
  Gauge.Visible := false;
  DBGrid.Enabled := true;
  OKGuideButton.Enabled := true;
  ...
  //
  // toZConnection.Connected:=false;
  if not cbOnlyOpen.Checked then
    fromADOConnection.Connected := false;
  //
  tmpDate2 := Now;
  if (tmpDate2 - tmpDate1) >= 1 then
  ...
  if fStop then
    ShowMessage('Документы НЕ загружены. Time=(' + StrTime + ').')
  else
    ShowMessage('Документы загружены. Time=(' + StrTime + ').');
  //
  fStop := true;
end;
...
var
  ...
  StrTime: String;
begin
  if (cbInsertHistoryCost.Checked) then
    if MessageDlg
      ('Действительно расчитать <СЕБЕСТОИМОСТЬ по МЕСЯЦАМ> за период с <' +
      DateToStr(StrToDate(StartDateCompleteEdit.Text)) + '> по <' +
      DateToStr(StrToDate(EndDateCompleteEdit.Text)) + '> ?', mtConfirmation,
      [mbYes, mbNo], 0) <> mrYes then
      exit
    else
  else if (cbComplete.Checked) and (cbUnComplete.Checked) then
    if MessageDlg('Действительно Распровести/Провести выбранные документы?',
      mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
      exit
    else
  else if cbUnComplete.Checked then
    if MessageDlg('Действительно только Распровести выбранные документы?',
      mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
      exit
    else
  else if cbComplete.Checked then
    if MessageDlg('Действительно только Провести выбранные документы?',
      mtConfirmation, [mbYes, mbNo], 0) <> mrYes then
      exit
    else
  else
  begin
    ShowMessage('Ошибка.Не выбрано Распровести или Провести.');
  end;
  //
  fStop := false;
  DBGrid.Enabled := false;
  ...
  begin
    if not fStop then
      pCompleteDocument_Income(false);
    if not fStop then
      pCompleteDocument_Send(false);
    if not fStop then
      pCompleteDocument_SendOnPrice(false);
    if not fStop then
      pCompleteDocument_ProductionUnion(false);
    if not fStop then
      pCompleteDocument_ProductionSeparate(false);
  end;
  ...
  //
  if (not fStop) and
    (not((cbInsertHistoryCost.Checked) and (cbInsertHistoryCost.Enabled))) then
    pCompleteDocument_Income(cbLastComplete.Checked);
  if not fStop then
    pCompleteDocument_Send(cbLastComplete.Checked);
  if not fStop then
    pCompleteDocument_SendOnPrice(cbLastComplete.Checked);
  if not fStop then
    pCompleteDocument_ProductionUnion(cbLastComplete.Checked);
  if not fStop then
    pCompleteDocument_ProductionSeparate(cbLastComplete.Checked);
  //
  Gauge.Visible := false;
  DBGrid.Enabled := true;
  OKGuideButton.Enabled := true;
  ...
  if (fStop) and (cbInsertHistoryCost.Checked) then
    ShowMessage('СЕБЕСТОИМОСТЬ по МЕСЯЦАМ расчитана НЕ полностью. Time=(' +
      StrTime + ').')
  else if fStop then
    ShowMessage('Документы НЕ Распроведены и(или) НЕ Проведены. Time=(' +
      StrTime + ').')
  else if cbInsertHistoryCost.Checked then
    ShowMessage('СЕБЕСТОИМОСТЬ по МЕСЯЦАМ расчитана полностью. Time=(' +
      StrTime + ').')
  else
    ShowMessage('Документы Распроведены и(или) Проведены. Time=(' +
      StrTime + ').');
  //
  fStop := true;
end;
...
begin
  ...
  fExecSqFromQuery
    ('update dba.Unit set Id_Postgres_RouteSorting=null,Id_Postgres_Business = null, Id1_Postgres = null, Id2_Postgres = null, Id3_Postgres = null');
  fExecSqFromQuery
    ('update dba._pgPersonal set Id1_Postgres = null, Id2_Postgres = null');
  fExecSqFromQuery('update dba.PriceList_byHistory set Id_Postgres = null');
  fExecSqFromQuery
    ('update dba.PriceListItems_byHistory set Id_Postgres = null');
  fExecSqFromQuery('update dba.GoodsProperty_Postgres set Id_Postgres = null');
  fExecSqFromQuery
    ('update dba.GoodsProperty_Detail set Id1_Postgres = null, Id2_Postgres = null, Id3_Postgres = null, Id4_Postgres = null, Id5_Postgres = null, Id6_Postgres = null, Id7_Postgres = null'
  ...
  fExecSqFromQuery
    ('update dba._pgInfoMoney set Id1_Postgres = null, Id2_Postgres = null, Id3_Postgres = null');
  fExecSqFromQuery
    ('update dba._pgAccount set Id1_Postgres = null, Id2_Postgres = null, Id3_Postgres = null');
  fExecSqFromQuery
    ('update dba._pgProfitLoss set Id1_Postgres = null, Id2_Postgres = null, Id3_Postgres = null');
  fExecSqFromQuery
    ('update dba._pgUnit set Id_Postgres = null, Id_Postgres_Branch = null');
end;
...
begin
  fExecSqFromQuery
    ('update dba.Bill set Id_Postgres = null where Id_Postgres is not null'); //
  fExecSqFromQuery
    ('update dba.BillItems set Id_Postgres = null where Id_Postgres is not null');
  fExecSqFromQuery
    ('update dba.BillItemsReceipt set Id_Postgres = null where Id_Postgres is not null');
end;

// ----------------------------------------------------------------------------------------------------------------------------------------------------
procedure TMainForm.pLoadGuide_Measure;
begin
  if (not cbMeasure.Checked) or (not cbMeasure.Enabled) then
    exit;
  //
  myEnabledCB(cbMeasure);
  //
  with fromQuery, Sql do
  begin
    Close;
    Clear;
    ...
    begin
      ...
      //
      toStoredProc.Params.ParamByName('ioId').Value :=
        FieldByName('Id_Postgres').AsString;
      toStoredProc.Params.ParamByName('inCode').Value :=
        FieldByName('ObjectCode').AsString;
      toStoredProc.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      // toStoredProc.Params.ParamByName('inSession').Value:=fGetSession;
      if not myExecToStoredProc then;
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Measure set Id_Postgres=' +
...
begin
  if (not cbGoodsGroup.Checked) or (not cbGoodsGroup.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      toStoredProc.Params.ParamByName('ioId').Value :=
        FieldByName('Id_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inCode').Value :=
        FieldByName('ObjectCode').AsInteger;
      toStoredProc.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      toStoredProc.Params.ParamByName('inParentId').Value :=
        FieldByName('ParentId_Postgres').AsInteger;
      // toStoredProc.Params.ParamByName('inSession').Value:=fGetSession;
      if not myExecToStoredProc then; // exit;
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Goods set Id_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  ...
  if (not cbGoods.Checked) or (not cbGoods.Enabled) then
    exit;
  //
  myEnabledCB(cbGoods);
  ...
  begin
    ...
    Add('     left outer join dba._pgInfoMoney on _pgInfoMoney.ObjectCode = case when fCheckGoodsParentID(1491,Goods.ParentId) =zc_rvYes() then 20701');
    // АГРОСЕЛЬПРОМ  - 20701	Общефирменные Товары	Прочие товары
    Add('                                                                        when fCheckGoodsParentID(5,   Goods.ParentId) =zc_rvYes()then 30101');
    // ГП            - 30101	Доходы	Продукция	Готовая продукция
    Add('                                                                        when fCheckGoodsParentID(5306,Goods.ParentId) =zc_rvYes() then 30101');
    // ПЕРЕПАК       - 30101	Доходы	Продукция	Готовая продукция
    Add('                                                                        when fCheckGoodsParentID(3482,Goods.ParentId) =zc_rvYes() then 30101');
    // эксперименты       - 30101	Доходы	Продукция	Готовая продукция
    Add('                                                                        when fCheckGoodsParentID(5874,Goods.ParentId) =zc_rvYes() then 30102');
    // ТУШЕНКА       - 30102	Доходы	Продукция	Тушенка
    Add('                                                                        when fCheckGoodsParentID(2387,Goods.ParentId) =zc_rvYes() then 30103');
    // ХЛЕБ          - 30103	Доходы  Продукция	Хлеб
    Add('                                                                        when fCheckGoodsParentID(2849,Goods.ParentId) =zc_rvYes() then 30301');
    // С-ПЕРЕРАБОТКА - 30301	Доходы  Переработка	Переработка

    Add('                                                                        when fCheckGoodsParentID(6682,Goods.ParentId) =zc_rvYes() then 20204');
    // КАНЦТОВАРЫ - 20204	Общефирменные  Прочие ТМЦ Канц товары
    Add('                                                                        when fCheckGoodsParentID(6677,Goods.ParentId) =zc_rvYes() then 20601');
    // КУЛЬКИ - 20601	Общефирменные  Прочие материалы	Прочие материалы
    Add('                                                                        when fCheckGoodsParentID(2954,Goods.ParentId) =zc_rvYes() then 20203');
    // МОЮЩЕЕ - 20203	Общефирменные  Прочие ТМЦ Моющие средства, кислоты
    Add('                                                                        when fCheckGoodsParentID(6678,Goods.ParentId) =zc_rvYes() then 20601');
    // ПЛЕНКА И СКОТЧ - 20601	Общефирменные  Прочие материалы	Прочие материалы
    Add('                                                                        when fCheckGoodsParentID(2949,Goods.ParentId) =zc_rvYes() then 20205');
    // РАЗНОЕ - 20205	Общефирменные  Прочие ТМЦ Прочие ТМЦ
    Add('                                                                        when fCheckGoodsParentID(2641,Goods.ParentId) =zc_rvYes() then 20202');
    // СПЕЦОДЕЖДА - 20202	Общефирменные  Прочие ТМЦ Спецодежда
    Add('                                                                        when fCheckGoodsParentID(6681,Goods.ParentId) =zc_rvYes() then 20601');
    // ЦЕННИКИ, ЯРЛЫКИ, ЭТ. ДАТА - 20601	Общефирменные  Прочие материалы	Прочие материалы
    Add('                                                                        when fCheckGoodsParentID(6676,Goods.ParentId) =zc_rvYes() then 20601');
    // ЩЕПА - 20601	Общефирменные  Прочие материалы	Прочие материалы
    Add('                                                                        when fCheckGoodsParentID(2787,Goods.ParentId) =zc_rvYes() then 20205');
    // СД-КУХНЯ - 20205	Общефирменные  Прочие ТМЦ Прочие ТМЦ

    Add('                                                                        when fCheckGoodsParentID(2642,Goods.ParentId) =zc_rvYes() then 20101');
    // СД-ЗАПЧАСТИ оборуд-е - 20101	Общефирменные  Запчасти и Ремонты	Запчасти и Ремонты

    Add('                                                                        when fCheckGoodsParentID(2647,Goods.ParentId) =zc_rvYes() then 10201');
    // СД-ПЕКАРНЯ - 10201		Основное сырье Прочее сырье	Специи
    Add('                                                                        when Goods.Id in (6041, 7013) then 10201');
    // СД-ТУШЕНКА - 10201		Основное сырье Прочее сырье	Специи
    Add('                                                                        when Goods.ParentId in (5857) then 10203');
    // СД-ТУШЕНКА - 10203		Основное сырье Прочее сырье	Упаковка

    Add('                                                                        when fCheckGoodsParentID(4213,Goods.ParentId) =zc_rvYes() then 20601');
    // ГОФРОТАРА - 20601	Общефирменные  Прочие материалы	Прочие материалы
    Add('                                                                        when fCheckGoodsParentID(3521,Goods.ParentId) =zc_rvYes() then 10201');
    // для проработок-новые специи - 10201		Основное сырье Прочее сырье	Специи
    Add('                                                                        when fCheckGoodsParentID(3221,Goods.ParentId) =zc_rvYes() then 10201');
    // ДОБАВКИ - 10201		Основное сырье Прочее сырье	Специи
    Add('                                                                        when fCheckGoodsParentID(2643,Goods.ParentId) =zc_rvYes() then 10201');
    // СПЕЦИИ - 10201		Основное сырье Прочее сырье	Специи
    Add('                                                                        when fCheckGoodsParentID(2644,Goods.ParentId) =zc_rvYes() then 10201');
    // СПЕЦИИ ДЕЛИКАТ. - 10201		Основное сырье Прочее сырье	Специи
    Add('                                                                        when fCheckGoodsParentID(2645,Goods.ParentId) =zc_rvYes() then 10202');
    // ОБОЛОЧКА - 10202		Основное сырье Прочее сырье	Оболочка
    Add('                                                                        when fCheckGoodsParentID(2631,Goods.ParentId) =zc_rvYes() then 10203');
    // !!!СД-СЫРЬЕ!!! - 10203		Основное сырье Прочее сырье	Упаковка

    Add('                                                                        when fCheckGoodsParentID(2648,Goods.ParentId) =zc_rvYes() then 10204');
    // СО-СЫРЬЕ СЫР - 10204		Основное сырье Прочее сырье Прочее сырье
    Add('                                                                        when Goods.Id in (2792, 7001, 6710) then 10103');
    // !!!СО- ГОВ. И СВ. Н\К + СЫР!!! - 10103		Основное сырье Мясное сырье Говядина
    Add('                                                                        when fCheckGoodsParentID(6435,Goods.ParentId) =zc_rvYes() then 10102');
    // !!!СО- ГОВ. И СВ. Н\К + СЫР!!! - 10102		Основное сырье Мясное сырье Свинина
    Add('                                                                        when fCheckGoodsParentID(3859,Goods.ParentId) =zc_rvYes() then 10105');
    // СО-БАРАНИНА - 10105		Основное сырье Мясное сырье Прочее мясное сырье
    Add('                                                                        when fCheckGoodsParentID(5676,Goods.ParentId) =zc_rvYes() then 10105');
    // СО-КАБАН и др. - 10105		Основное сырье Мясное сырье Прочее мясное сырье
    Add('                                                                        when fCheckGoodsParentID(5503,Goods.ParentId) =zc_rvYes() then 10105');
    // СО-ПТИЦА РАЗНАЯ - 10105		Основное сырье Мясное сырье Прочее мясное сырье

    Add('                                                                        when fCheckGoodsParentID(5489,Goods.ParentId) =zc_rvYes() then 10103');
    // СО-ГОВ.  ДЕЛ-СЫ* - 10103		Основное сырье Мясное сырье Говядина
    Add('                                                                        when fCheckGoodsParentID(5491,Goods.ParentId) =zc_rvYes() then 10103');
    // СО-ГОВ. ВЫС+ОДН.* - 10103		Основное сырье Мясное сырье Говядина
    Add('                                                                        when fCheckGoodsParentID(2633,Goods.ParentId) =zc_rvYes() then 10103');
    // СО-ГОВЯДИНА ПФ* - 10103		Основное сырье Мясное сырье Говядина
    Add('                                                                        when fCheckGoodsParentID(2662,Goods.ParentId) =zc_rvYes() then 10103');
    // СО-СЫРЬЕ УБОЙ ГОВ.* - 10103		Основное сырье Мясное сырье Говядина

    Add('                                                                        when fCheckGoodsParentID(2635,Goods.ParentId) =zc_rvYes() then 10104');
    // СО-КУРИЦА* - 10104		Основное сырье Мясное сырье Курица

    Add('                                                                        when fCheckGoodsParentID(2632,Goods.ParentId) =zc_rvYes() then 10102');
    // !!!СО!!! - 10102		Основное сырье Мясное сырье Свинина
    Add('                                                                        when fCheckGoodsParentID(2691,Goods.ParentId) =zc_rvYes() then 10103');
    // СО-ГОВЯДИНА ПРОДАЖА маг - 10103		Основное сырье Мясное сырье Говядина
    Add('                                                                        when Goods.Id in (2800) then 10104');
    // КУРЫ-ГРИЛЬ* - 10104		Основное сырье Мясное сырье Курица
    Add('                                                                        when Goods.Id in (3039) then 10103');
    // ДОНЕР - КЕБАБ - 10103		Основное сырье Мясное сырье Говядина
    Add('                                                                        when Goods.Id in (17) then 10103');
    // Гост СЫРЬЕ - К-3 - 10103		Основное сырье Мясное сырье Говядина
    Add('                                                                        when Goods.Id in (538) then 10102');
    // УДАЛЕННЫЕ П/К - 10102		Основное сырье Мясное сырье Свинина
    Add('                                                                        when fCheckGoodsParentID(3447,Goods.ParentId) =zc_rvYes() then 10102');
    // !!!СО-НА ПРОДАЖУ!!! - 10102		Основное сырье Мясное сырье Свинина

    Add('                                                                        when fCheckGoodsParentID(3217,Goods.ParentId) =zc_rvYes() then 21301');
    // СО-ЭМУЛЬСИИ - 10102		Общефирменные Незавершенное производство Незавершенное производство
    Add('                                                                        when fCheckGoodsParentID(1670,Goods.ParentId) =zc_rvYes() then 10201');
    // СЫР - 10201		Основное сырье Прочее сырье	Прочее сырье
    Add('                                                                        when fCheckGoodsParentID(686,Goods.ParentId) =zc_rvYes() then 20501');
    // Тара - 20501		Общефирменные Оборотная тара	Оборотная тара
    ...
    //
    toStoredProc_two.StoredProcName :=
      'gpInsertUpdate_ObjectBoolean_Goods_Partion';
    ...
    toStoredProc_two.Params.AddParam('inPartionCount', ftBoolean,
      ptInput, false);
    toStoredProc_two.Params.AddParam('inPartionSumm', ftBoolean,
      ptInput, false);
    //
    while not EOF do
    begin
      ...
      toStoredProc.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      toStoredProc.Params.ParamByName('inWeight').Value :=
        FieldByName('Ves_onMeasure').AsFloat;
      toStoredProc.Params.ParamByName('inGoodsGroupId').Value :=
        FieldByName('ParentId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inMeasureId').Value :=
        FieldByName('MeasureId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inInfoMoneyId').Value :=
        FieldByName('InfoMoneyId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inTradeMarkId').Value :=
        FieldByName('TradeMarkId_PG').AsInteger;

      if not myExecToStoredProc then; // exit;

      if (FieldByName('isPartionCount').AsInteger = FieldByName('zc_rvYes')
        .AsInteger) or (FieldByName('isPartionSumm').AsInteger = FieldByName
        ('zc_rvYes').AsInteger) then
      begin
        toStoredProc_two.Params.ParamByName('inId').Value :=
          toStoredProc.Params.ParamByName('ioId').Value;

        if FieldByName('isPartionCount').AsInteger = FieldByName('zc_rvYes').AsInteger
        then
          toStoredProc_two.Params.ParamByName('inPartionCount').Value := true
        else
          toStoredProc_two.Params.ParamByName('inPartionCount').Value := false;

        if FieldByName('isPartionSumm').AsInteger = FieldByName('zc_rvYes').AsInteger
        then
          toStoredProc_two.Params.ParamByName('inPartionSumm').Value := true
        else
          toStoredProc_two.Params.ParamByName('inPartionSumm').Value := false;
        if not myExecToStoredProc_two then;
      end;

      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.GoodsProperty set Id_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbGoodsKind.Checked) or (not cbGoodsKind.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.KindPackage set Id_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
<<<<<<< f085f035c512b56e324ebc0ef140b0d9af39a2f5
  ...
  begin
    ...
    begin
      ...
      if not myExecToStoredProc then; // exit;
      //
      if (1 = 1) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.MoneyKind set Id_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbContractKind.Checked) or (not cbContractKind.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.ContractKind set Id_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbJuridicalGroup.Checked) or (not cbJuridicalGroup.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Unit set Id1_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbJuridical.Checked) or (not cbJuridical.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      Add('     , isnull(isnull(zf_ChangeTVarCharMediumToNull(ClientInformation.GLNMain),ClientInformation.GLN),'
        + FormatToVarCharServer_notNULL('') + ') as GLNCode');
      Add('     , zc_rvYes() as zc_rvYes');
      Add('     , case when Unit.Id in (3, 165) then zc_rvYes() else zc_rvNo() end isCorporate');
      // АЛАН + ИРНА-1
      Add('     , case when Unit.Id = 3 then _pgInfoMoney_Alan.Id3_Postgres when Unit.Id = 165 then _pgInfoMoney_Irna.Id3_Postgres else null end InfoMoneyId_PG');
      // АЛАН + ИРНА-1
      ...
      Add('     left outer join dba.GoodsProperty_Postgres as GoodsProperty_PG on GoodsProperty_PG.Id= case when fIsClient_ATB(Unit.Id)=zc_rvYes() then 1'
        ...
        '    when fIsClient_GD(Unit.Id)=zc_rvYes() then 14' + '    else null'
        + ' end');
      ...
      Add('where (Unit.Id1_Postgres is null'
        ...
        '   and isnull(Unit_all.pgUnitId, 0)=0' + '      )' + '  or Unit.Id=3'
        // АЛАН
        );
      Add('group by ObjectId');
      Add('       , ObjectName');
    ...
    begin
      ShowMessage
        ('Так надо: !! DISABLE !!! pLoadGuide_Juridical where isBill=true');
      exit;
      { Add('select Unit.Id as ObjectId');
        Add('     , Unit.UnitCode as ObjectCode');
        Add('     , Unit.UnitName as ObjectName');
    ...
    Open;
    cbJuridical.Caption := '3.2. (' + IntToStr(RecordCount) +
      ') Юридические лица';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('inGLNCode').Value :=
        FieldByName('GLNCode').AsString;
      if FieldByName('isCorporate').AsInteger = FieldByName('zc_rvYes').AsInteger
      then
        toStoredProc.Params.ParamByName('inIsCorporate').Value := true
      else
        toStoredProc.Params.ParamByName('inIsCorporate').Value := false;
      toStoredProc.Params.ParamByName('inJuridicalGroupId').Value :=
        FieldByName('ParentId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inGoodsPropertyId').Value :=
        FieldByName('GoodsPropertyId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inInfoMoneyId').Value :=
        FieldByName('InfoMoneyId_PG').AsInteger;

      if not myExecToStoredProc then; // exit;
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Unit set Id2_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbPartner.Checked) or (not cbPartner.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ShowMessage
        ('Так надо: !!! DISABLE !!! pLoadGuide_Partner where isBill=true');
      exit;
      { Add('select Unit.Id as ObjectId');
        Add('     , Unit.UnitCode as ObjectCode');
        Add('     , Unit.UnitName as ObjectName');
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('inGLNCode').Value :=
        FieldByName('GLNCode').AsString;
      toStoredProc.Params.ParamByName('inPrepareDayCount').Value :=
        FieldByName('PrepareDayCount').AsFloat;
      toStoredProc.Params.ParamByName('inDocumentDayCount').Value :=
        FieldByName('DocumentDayCount').AsFloat;
      toStoredProc.Params.ParamByName('inJuridicalId').Value :=
        FieldByName('JuridicalId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inRouteId').Value :=
        FieldByName('RouteId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inRouteSortingId').Value :=
        FieldByName('RouteSortingId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inPersonalTakeId').Value :=
        FieldByName('PersonalTakeId_Postgres').AsInteger;
      if not myExecToStoredProc then; // exit;
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Unit set Id3_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbBusiness.Checked) or (not cbBusiness.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Unit set Id_Postgres_Business=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbBranch.Checked) or (not cbBranch.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgUnit set Id_Postgres_Branch=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  ...
  //
  //
  if (not cbUnitGroup.Checked) or (not cbUnitGroup.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('select Unit.Id as ObjectId');
    Add('     , 0 as ObjectCode');
    Add('     , ' + FormatToVarCharServer_notNULL('ВСЕ Экспедиторы и Филиалы') +
      ' as ObjectName');
    Add('     , Unit.Id3_Postgres as Id_Postgres');
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        if FieldByName('ObjectId').AsInteger = 151 then
          fExecSqFromQuery('update dba.Unit set Id3_Postgres=' +
            IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
            ' where Id = ' + FieldByName('ObjectId').AsString)
        else
...
begin
  if (not cbUnit.Checked) or (not cbUnit.Enabled) then
    exit;
  //
  myEnabledCB(cbUnit);
  //
  ...
  begin
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      toStoredProc.Params.ParamByName('inUnitGroupId').Value :=
        FieldByName('ParentId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inBranchId').Value :=
        FieldByName('BranchId_Postgres').AsInteger;
      // toStoredProc.Params.ParamByName('inSession').Value:=fGetSession;
      if not myExecToStoredProc then; // exit;
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
...
begin
  ...
  begin
    ...
    // Производство + Содержание транспорта
    ...
    toStoredProc.Params.AddParam('inProfitLossDirectionId', ftInteger,
      ptInput, 0);
    //
    while not EOF do
    begin
      ...
      then
        toStoredProc.Params.ParamByName('inPartionDate').Value := true
      else
        toStoredProc.Params.ParamByName('inPartionDate').Value := false;

      toStoredProc.Params.ParamByName('inParentId').Value :=
        FieldByName('ParentId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inBranchId').Value :=
        FieldByName('BranchId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inBusinessId').Value :=
        FieldByName('BusinessId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inJuridicalId').Value :=
        FieldByName('JuridicalId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inAccountDirectionId').Value :=
        FieldByName('AccountDirectionId').AsInteger;
      toStoredProc.Params.ParamByName('inProfitLossDirectionId').Value :=
        FieldByName('ProfitLossDirectionId').AsInteger;
      // toStoredProc.Params.ParamByName('inSession').Value:=fGetSession;
      if not myExecToStoredProc then; // exit;
      //
      if (1 = 1) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgUnit set Id_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ', Id_Postgres_Branch=zf_ChangeIntToNull(' +
          IntToStr(FieldByName('BranchId_Postgres').AsInteger) + ') where Id = '
          + FieldByName('ObjectId').AsString);
      //
      Next;
      Application.ProcessMessages;
      Gauge.Progress := Gauge.Progress + 1;
...
begin
  if (not cbMember_andPersonal.Checked) or (not cbMember_andPersonal.Enabled)
  then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      toStoredProc.Params.ParamByName('inINN').Value :=
        FieldByName('INN').AsString;
      if not myExecToStoredProc then; // exit;
      // Personal
      toStoredProc_two.Params.ParamByName('ioId').Value :=
        FieldByName('Id_Postgres_two').AsInteger;
      toStoredProc_two.Params.ParamByName('inCode').Value :=
        FieldByName('ObjectCode').AsInteger;
      toStoredProc_two.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      toStoredProc_two.Params.ParamByName('inMemberId').Value :=
        toStoredProc.Params.ParamByName('ioId').Value;
      toStoredProc_two.Params.ParamByName('inPositionId').Value :=
        FieldByName('PositionId_PG').AsInteger;
      toStoredProc_two.Params.ParamByName('inUnitId').Value :=
        FieldByName('UnitId_PG').AsInteger;
      toStoredProc_two.Params.ParamByName('inJuridicalId').Value :=
        FieldByName('JuridicalId_PG').AsInteger;
      toStoredProc_two.Params.ParamByName('inBusinessId').Value :=
        FieldByName('BusinessId_PG').AsInteger;
      ...
      //
      if (1 = 1) or (FieldByName('Id_Postgres').AsInteger = 0) or
        (FieldByName('Id_Postgres_two').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgPersonal set Id1_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ', Id2_Postgres=' + IntToStr(toStoredProc_two.Params.ParamByName
          ('ioId').Value) + ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
      Application.ProcessMessages;
      Gauge.Progress := Gauge.Progress + 1;
...
begin
  if (not cbRouteSorting.Checked) or (not cbRouteSorting.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Open;
    cbRouteSorting.Caption := '3.4. (' + IntToStr(RecordCount) +
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Unit set Id_Postgres_RouteSorting=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbPriceList.Checked) or (not cbPriceList.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.PriceList_byHistory set Id_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbPriceListItems.Checked) or (not cbPriceListItems.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('     , case when PriceListItems_byHistory.StartDate=zc_DateStart() then '
      + FormatToDateServer_notNULL(StrToDate('01.01.1990')) +
      ' else PriceListItems_byHistory.StartDate end as OperDate');
    ...
    Open;
    cbPriceListItems.Caption := '5.2. (' + IntToStr(RecordCount) +
      ') Прайс листы - цены';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('ioId').Value :=
        FieldByName('Id_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inPriceListId').Value :=
        FieldByName('PriceListId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inGoodsId').Value :=
        FieldByName('GoodsId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inOperDate').Value :=
        FieldByName('OperDate').AsDateTime;
      toStoredProc.Params.ParamByName('inValue').Value :=
        FieldByName('NewPrice').AsFloat;

      if not myExecToStoredProc then; // exit;
      //
      if (1 = 1) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.PriceListItems_byHistory set Id_Postgres='
          + IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbGoodsProperty.Checked) or (not cbGoodsProperty.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.GoodsProperty_Postgres set Id_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbGoodsPropertyValue.Checked) or (not cbGoodsPropertyValue.Enabled)
  then
    exit;
  //
  ...
  begin
    ...
    Add('     , zc_rvYes() as zc_rvYes');
    // ---------------------------1
    Add('     , case when trim (GoodsProperty_Detail.GoodsCodeScaner)<>' +
      FormatToVarCharServer_notNULL('') +
      ' then zc_rvYes() else zc_rvNo() end as is1');
    Add('     , null as ObjectName1');
    Add('     , case when is1=zc_rvNo() then cast (null as TSumm) when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,1,6)='
      + FormatToVarCharServer_notNULL('230365') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,13,2) when GoodsProperty.MeasureId = zc_measure_Sht() then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,15,2) else cast (null as TSumm) end as Amount1');
    Add('     , case when is1=zc_rvNo() then null when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,1,6)='
      + FormatToVarCharServer_notNULL('230365') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,1,12) when GoodsProperty.MeasureId = zc_measure_Sht() then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,1,13) else SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,1,6)+'
      + FormatToVarCharServer_notNULL('0000000') + ' end as BarCode1');
    Add('     , case when is1=zc_rvNo() then null when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,1,6)='
      + FormatToVarCharServer_notNULL('230365') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,17,5) when GoodsProperty.MeasureId = zc_measure_Sht() then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,18,5) else SUBSTR(GoodsProperty_Detail.GoodsCodeScaner,8,5) end as Article1');
    ...
    // ---------------------------2
    Add('     , case when trim (GoodsProperty_Detail.GoodsCodeScaner_byKievOK)<>'
      + FormatToVarCharServer_notNULL('') +
      ' then zc_rvYes() else zc_rvNo() end as is2');
    Add('     , null as ObjectName2');
    Add('     , cast (null as TSumm) as Amount2');
    Add('     , CASE WHEN LENGTH(GoodsProperty_Detail.GoodsCodeScaner_byKievOK)=6 THEN '
      + FormatToVarCharServer_notNULL('28') +
      '+GoodsProperty_Detail.GoodsCodeScaner_byKievOK ELSE ' +
      FormatToVarCharServer_notNULL('') + ' END as BarCode2');
    Add('     , CASE WHEN LENGTH(GoodsProperty_Detail.GoodsCodeScaner_byKievOK)=6 THEN '
      + FormatToVarCharServer_notNULL('') +
      ' ELSE trim (GoodsProperty_Detail.GoodsCodeScaner_byKievOK) END as Article2');
    ...
    Add('     , case when is3=zc_rvNo() then cast (null as TSumm) when GoodsProperty.MeasureId = zc_measure_Sht() and SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,20,2) <> '
      + FormatToVarCharServer_notNULL('') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,20,2) else cast (null as TSumm) end as Amount3');
    Add('     , SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,6,13) as BarCode3');
    Add('     , case when is3=zc_rvNo() then null when GoodsProperty.MeasureId = zc_measure_Sht() then case when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,23,1)='
      + FormatToVarCharServer_notNULL('0') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,24,5) else SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,23,6) end'
      + ' else case when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,20,1)='
      + FormatToVarCharServer_notNULL('0') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,21,5) else SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byMetro,20,6) end'
      + '       end as Article3');
    ...
    // ---------------------------4
    Add('     , case when trim (GoodsProperty_Detail.GoodsCodeScaner_byMain)<>'
      + FormatToVarCharServer_notNULL('') +
      ' or trim(GoodsProperty_Detail.GoodsName_Client)<>' +
      FormatToVarCharServer_notNULL('') +
    ...
    // ---------------------------5
    Add('     , case when trim (GoodsProperty_Detail.GoodsCodeScaner_byFozzi)<>'
      + FormatToVarCharServer_notNULL('') +
      '  then zc_rvYes() else zc_rvNo() end as is5');
    ...
    Add('     , case when is5=zc_rvNo() then null when GoodsProperty.MeasureId = zc_measure_Sht() then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,1,13) else SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,1,7)+'
      + FormatToVarCharServer_notNULL('000000') + ' end as BarCode5');
    Add('     , case when is5=zc_rvNo() then null when GoodsProperty.MeasureId = zc_measure_Sht() then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,18,6)'
      + ' when LENGTH (GoodsProperty_Detail.GoodsCodeScaner_byFozzi) = 24 then case when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,19,1)='
      + FormatToVarCharServer_notNULL('0') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,20,5) else SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,19,6) end'
      + ' when LENGTH (GoodsProperty_Detail.GoodsCodeScaner_byFozzi) = 23 then case when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,18,1)='
      + FormatToVarCharServer_notNULL('0') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,19,5) else SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,18,6) end'
      + ' when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,9,1)=' +
      FormatToVarCharServer_notNULL('0') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byFozzi,10,5)' +
    ...
    // ---------------------------6
    Add('     , case when trim (GoodsProperty_Detail.GoodsCodeScaner_byKisheni)<>'
      + FormatToVarCharServer_notNULL('') +
      '  then zc_rvYes() else zc_rvNo() end as is6');
    ...
    Add('     , case when is6=zc_rvNo() then null when GoodsProperty.MeasureId = zc_measure_Sht() then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byKisheni,18,7)'
      + ' when LENGTH (GoodsProperty_Detail.GoodsCodeScaner_byKisheni) = 24 then case when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byKisheni,18,1)='
      + FormatToVarCharServer_notNULL('0') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byKisheni,19,6) else SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byKisheni,18,7) end'
      + ' when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byKisheni,15,2)=' +
      FormatToVarCharServer_notNULL('00') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byKisheni,17,5)' +
      ' when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byKisheni,15,1)=' +
      FormatToVarCharServer_notNULL('0') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byKisheni,16,6)' +
    ...
    // ---------------------------7
    Add('     , case when trim (GoodsProperty_Detail.GoodsCodeScaner_byVivat)<>'
      + FormatToVarCharServer_notNULL('') +
      '  then zc_rvYes() else zc_rvNo() end as is7');
    ...
    Add('     , case when is7=zc_rvNo() then null when GoodsProperty.MeasureId = zc_measure_Sht() and SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byVivat,18,7) <> '
      + FormatToVarCharServer_notNULL('0000000') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byVivat,18,7)' +
      ' when GoodsProperty.MeasureId = zc_measure_Sht() then ' +
      FormatToVarCharServer_notNULL('') +
      ' when SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byVivat,15,1)=' +
      FormatToVarCharServer_notNULL('0') +
      ' then SUBSTR(GoodsProperty_Detail.GoodsCodeScaner_byVivat,16,6)' +
    ...
    Add('where is1=zc_rvYes()' + ' or is2=zc_rvYes()' + ' or is3=zc_rvYes()' +
      ' or is4=zc_rvYes()' + ' or is5=zc_rvYes()' + ' or is6=zc_rvYes()' +
      ' or is7=zc_rvYes()');
    Add('order by is7, BarCode7, ObjectId');
    Open;
    cbGoodsPropertyValue.Caption := '6.2. (' + IntToStr(RecordCount) +
      ') Значения свойств товаров для классификатора';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
    ...
    begin
      ...
      //
      // 1
      if FieldByName('is1').AsInteger = FieldByName('zc_rvYes').AsInteger then
      begin
        toStoredProc.Params.ParamByName('ioId').Value :=
          FieldByName('Id_Postgres1').AsInteger;
        toStoredProc.Params.ParamByName('inName').Value :=
          FieldByName('ObjectName1').AsString;
        toStoredProc.Params.ParamByName('inAmount').Value :=
          FieldByName('Amount1').AsFloat;
        toStoredProc.Params.ParamByName('inBarCode').Value :=
          FieldByName('BarCode1').AsString;
        toStoredProc.Params.ParamByName('inArticle').Value :=
          FieldByName('Article1').AsString;
        toStoredProc.Params.ParamByName('inBarCodeGLN').Value :=
          FieldByName('BarCodeGLN1').AsString;
        toStoredProc.Params.ParamByName('inArticleGLN').Value :=
          FieldByName('ArticleGLN1').AsString;
        toStoredProc.Params.ParamByName('inGoodsPropertyId').Value :=
          FieldByName('GoodsPropertyId1').AsInteger;
        toStoredProc.Params.ParamByName('inGoodsId').Value :=
          FieldByName('GoodsId1').AsInteger;
        toStoredProc.Params.ParamByName('inGoodsKindId').Value :=
          FieldByName('GoodsKindId1').AsInteger;
        // toStoredProc.Params.ParamByName('inSession').Value:=fGetSession;
        if not myExecToStoredProc then; // exit;
        //
        if (1 = 0) or (FieldByName('Id_Postgres1').AsInteger = 0) then
          fExecSqFromQuery('update dba.GoodsProperty_Detail set Id1_Postgres=' +
            IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
            ' where Id = ' + FieldByName('ObjectId').AsString);
      end;
      // 2
      if FieldByName('is2').AsInteger = FieldByName('zc_rvYes').AsInteger then
      begin
        toStoredProc.Params.ParamByName('ioId').Value :=
          FieldByName('Id_Postgres2').AsInteger;
        toStoredProc.Params.ParamByName('inName').Value :=
          FieldByName('ObjectName2').AsString;
        toStoredProc.Params.ParamByName('inAmount').Value :=
          FieldByName('Amount2').AsFloat;
        toStoredProc.Params.ParamByName('inBarCode').Value :=
          FieldByName('BarCode2').AsString;
        toStoredProc.Params.ParamByName('inArticle').Value :=
          FieldByName('Article2').AsString;
        toStoredProc.Params.ParamByName('inBarCodeGLN').Value :=
          FieldByName('BarCodeGLN2').AsString;
        toStoredProc.Params.ParamByName('inArticleGLN').Value :=
          FieldByName('ArticleGLN2').AsString;
        toStoredProc.Params.ParamByName('inGoodsPropertyId').Value :=
          FieldByName('GoodsPropertyId2').AsInteger;
        toStoredProc.Params.ParamByName('inGoodsId').Value :=
        ...
        if not myExecToStoredProc then; // exit;
        //
        if (1 = 0) or (FieldByName('Id_Postgres2').AsInteger = 0) then
          fExecSqFromQuery('update dba.GoodsProperty_Detail set Id2_Postgres=' +
            IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
            ' where Id = ' + FieldByName('ObjectId').AsString);
      end;
      // 3
      if FieldByName('is3').AsInteger = FieldByName('zc_rvYes').AsInteger then
      begin
        toStoredProc.Params.ParamByName('ioId').Value :=
          FieldByName('Id_Postgres3').AsInteger;
        toStoredProc.Params.ParamByName('inName').Value :=
          FieldByName('ObjectName3').AsString;
        toStoredProc.Params.ParamByName('inAmount').Value :=
          FieldByName('Amount3').AsFloat;
        toStoredProc.Params.ParamByName('inBarCode').Value :=
          FieldByName('BarCode3').AsString;
        toStoredProc.Params.ParamByName('inArticle').Value :=
          FieldByName('Article3').AsString;
        toStoredProc.Params.ParamByName('inBarCodeGLN').Value :=
          FieldByName('BarCodeGLN3').AsString;
        toStoredProc.Params.ParamByName('inArticleGLN').Value :=
          FieldByName('ArticleGLN3').AsString;
        toStoredProc.Params.ParamByName('inGoodsPropertyId').Value :=
          FieldByName('GoodsPropertyId3').AsInteger;
        toStoredProc.Params.ParamByName('inGoodsId').Value :=
        ...
        if not myExecToStoredProc then; // exit;
        //
        if (1 = 0) or (FieldByName('Id_Postgres3').AsInteger = 0) then
          fExecSqFromQuery('update dba.GoodsProperty_Detail set Id3_Postgres=' +
            IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
            ' where Id = ' + FieldByName('ObjectId').AsString);
      end;
      // 4
      if FieldByName('is4').AsInteger = FieldByName('zc_rvYes').AsInteger then
      begin
        toStoredProc.Params.ParamByName('ioId').Value :=
          FieldByName('Id_Postgres4').AsInteger;
        toStoredProc.Params.ParamByName('inName').Value :=
          FieldByName('ObjectName4').AsString;
        toStoredProc.Params.ParamByName('inAmount').Value :=
          FieldByName('Amount4').AsFloat;
        toStoredProc.Params.ParamByName('inBarCode').Value :=
          FieldByName('BarCode4').AsString;
        toStoredProc.Params.ParamByName('inArticle').Value :=
          FieldByName('Article4').AsString;
        toStoredProc.Params.ParamByName('inBarCodeGLN').Value :=
          FieldByName('BarCodeGLN4').AsString;
        toStoredProc.Params.ParamByName('inArticleGLN').Value :=
          FieldByName('ArticleGLN4').AsString;
        toStoredProc.Params.ParamByName('inGoodsPropertyId').Value :=
          FieldByName('GoodsPropertyId4').AsInteger;
        toStoredProc.Params.ParamByName('inGoodsId').Value :=
        ...
        if not myExecToStoredProc then; // exit;
        //
        if (1 = 0) or (FieldByName('Id_Postgres4').AsInteger = 0) then
          fExecSqFromQuery('update dba.GoodsProperty_Detail set Id4_Postgres=' +
            IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
            ' where Id = ' + FieldByName('ObjectId').AsString);
      end;
      // 5
      if FieldByName('is5').AsInteger = FieldByName('zc_rvYes').AsInteger then
      begin
        toStoredProc.Params.ParamByName('ioId').Value :=
          FieldByName('Id_Postgres5').AsInteger;
        toStoredProc.Params.ParamByName('inName').Value :=
          FieldByName('ObjectName5').AsString;
        toStoredProc.Params.ParamByName('inAmount').Value :=
          FieldByName('Amount5').AsFloat;
        toStoredProc.Params.ParamByName('inBarCode').Value :=
          FieldByName('BarCode5').AsString;
        toStoredProc.Params.ParamByName('inArticle').Value :=
          FieldByName('Article5').AsString;
        toStoredProc.Params.ParamByName('inBarCodeGLN').Value :=
          FieldByName('BarCodeGLN5').AsString;
        toStoredProc.Params.ParamByName('inArticleGLN').Value :=
          FieldByName('ArticleGLN5').AsString;
        toStoredProc.Params.ParamByName('inGoodsPropertyId').Value :=
          FieldByName('GoodsPropertyId5').AsInteger;
        toStoredProc.Params.ParamByName('inGoodsId').Value :=
        ...
        if not myExecToStoredProc then; // exit;
        //
        if (1 = 0) or (FieldByName('Id_Postgres5').AsInteger = 0) then
          fExecSqFromQuery('update dba.GoodsProperty_Detail set Id5_Postgres=' +
            IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
            ' where Id = ' + FieldByName('ObjectId').AsString);
      end;
      // 6
      if FieldByName('is6').AsInteger = FieldByName('zc_rvYes').AsInteger then
      begin
        toStoredProc.Params.ParamByName('ioId').Value :=
          FieldByName('Id_Postgres6').AsInteger;
        toStoredProc.Params.ParamByName('inName').Value :=
          FieldByName('ObjectName6').AsString;
        toStoredProc.Params.ParamByName('inAmount').Value :=
          FieldByName('Amount6').AsFloat;
        toStoredProc.Params.ParamByName('inBarCode').Value :=
          FieldByName('BarCode6').AsString;
        toStoredProc.Params.ParamByName('inArticle').Value :=
          FieldByName('Article6').AsString;
        toStoredProc.Params.ParamByName('inBarCodeGLN').Value :=
          FieldByName('BarCodeGLN6').AsString;
        toStoredProc.Params.ParamByName('inArticleGLN').Value :=
          FieldByName('ArticleGLN6').AsString;
        toStoredProc.Params.ParamByName('inGoodsPropertyId').Value :=
          FieldByName('GoodsPropertyId6').AsInteger;
        toStoredProc.Params.ParamByName('inGoodsId').Value :=
        ...
        if not myExecToStoredProc then; // exit;
        //
        if (1 = 0) or (FieldByName('Id_Postgres6').AsInteger = 0) then
          fExecSqFromQuery('update dba.GoodsProperty_Detail set Id6_Postgres=' +
            IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
            ' where Id = ' + FieldByName('ObjectId').AsString);
      end;
      // 7
      if FieldByName('is7').AsInteger = FieldByName('zc_rvYes').AsInteger then
      begin
        toStoredProc.Params.ParamByName('ioId').Value :=
          FieldByName('Id_Postgres7').AsInteger;
        toStoredProc.Params.ParamByName('inName').Value :=
          FieldByName('ObjectName7').AsString;
        toStoredProc.Params.ParamByName('inAmount').Value :=
          FieldByName('Amount7').AsFloat;
        toStoredProc.Params.ParamByName('inBarCode').Value :=
          FieldByName('BarCode7').AsString;
        toStoredProc.Params.ParamByName('inArticle').Value :=
          FieldByName('Article7').AsString;
        toStoredProc.Params.ParamByName('inBarCodeGLN').Value :=
          FieldByName('BarCodeGLN7').AsString;
        toStoredProc.Params.ParamByName('inArticleGLN').Value :=
          FieldByName('ArticleGLN7').AsString;
        toStoredProc.Params.ParamByName('inGoodsPropertyId').Value :=
          FieldByName('GoodsPropertyId7').AsInteger;
        toStoredProc.Params.ParamByName('inGoodsId').Value :=
        ...
        if not myExecToStoredProc then; // exit;
        //
        if (1 = 0) or (FieldByName('Id_Postgres7').AsInteger = 0) then
          fExecSqFromQuery('update dba.GoodsProperty_Detail set Id7_Postgres=' +
            IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
            ' where Id = ' + FieldByName('ObjectId').AsString);
      end;
      //
...
begin
  if (not cbInfoMoneyGroup.Checked) or (not cbInfoMoneyGroup.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 1) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgInfoMoney set Id1_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Name1 in (select Name1 from dba._pgInfoMoney where Id = ' +
          FieldByName('ObjectId').AsString + ')');
      //
      Next;
      Application.ProcessMessages;
...
begin
  if (not cbInfoMoneyDestination.Checked) or (not cbInfoMoneyDestination.Enabled)
  then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 1) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgInfoMoney set Id2_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Name1 in (select Name1 from dba._pgInfoMoney where Id = ' +
          FieldByName('ObjectId').AsString + ')' +
          '   and Name2 in (select Name2 from dba._pgInfoMoney where Id = ' +
          FieldByName('ObjectId').AsString + ')');
      //
      Next;
      Application.ProcessMessages;
...
begin
  if (not cbInfoMoney.Checked) or (not cbInfoMoney.Enabled) then
    exit;
  //
  ...
  begin
    ...
    toStoredProc.Params.AddParam('inInfoMoneyDestinationId', ftInteger,
      ptInput, 0);
    //
    while not EOF do
    begin
      ...
      toStoredProc.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      toStoredProc.Params.ParamByName('inInfoMoneyGroupId').Value :=
        FieldByName('InfoMoneyGroupId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inInfoMoneyDestinationId').Value :=
        FieldByName('InfoMoneyDestinationId_PG').AsInteger;
      // toStoredProc.Params.ParamByName('inSession').Value:=fGetSession;
      if not myExecToStoredProc then; // exit;
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgInfoMoney set Id3_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbAccountGroup.Checked) or (not cbAccountGroup.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgAccount set Id1_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Name1 in (select Name1 from dba._pgAccount where Id = ' +
          FieldByName('ObjectId').AsString + ')');
      //
      Next;
      Application.ProcessMessages;
...
begin
  if (not cbAccountDirection.Checked) or (not cbAccountDirection.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgAccount set Id2_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Name1 in (select Name1 from dba._pgAccount where Id = ' +
          FieldByName('ObjectId').AsString + ')' +
          '   and Name2 in (select Name2 from dba._pgAccount where Id = ' +
          FieldByName('ObjectId').AsString + ')');
      //
      Next;
      Application.ProcessMessages;
...
begin
  if (not cbAccount.Checked) or (not cbAccount.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      toStoredProc.Params.ParamByName('inAccountGroupId').Value :=
        FieldByName('AccountGroupId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inAccountDirectionId').Value :=
        FieldByName('AccountDirectionId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inInfoMoneyDestinationId').Value :=
        FieldByName('InfoMoneyDestinationId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inInfoMoneyId').Value :=
        FieldByName('InfoMoneyId_PG').AsInteger;
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgAccount set Id3_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  if (not cbProfitLossGroup.Checked) or (not cbProfitLossGroup.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgProfitLoss set Id1_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Name1 in (select Name1 from dba._pgProfitLoss where Id = ' +
          FieldByName('ObjectId').AsString + ')');
      //
      Next;
      Application.ProcessMessages;
...
begin
  if (not cbProfitLossDirection.Checked) or (not cbProfitLossDirection.Enabled)
  then
    exit;
  //
  ...
  begin
    ...
    begin
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgProfitLoss set Id2_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Name1 in (select Name1 from dba._pgProfitLoss where Id = ' +
          FieldByName('ObjectId').AsString + ')' +
          '   and Name2 in (select Name2 from dba._pgProfitLoss where Id = ' +
          FieldByName('ObjectId').AsString + ')');
      //
      Next;
      Application.ProcessMessages;
...
begin
  if (not cbProfitLoss.Checked) or (not cbProfitLoss.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('     , case when _pgProfitLoss.ObjectCode < 20000 then null ' +
      ' else isnull(_pgInfoMoney_30201.Id2_Postgres, (select max (isnull (_pgInfoMoney.Id2_Postgres, 0)) from dba._pgInfoMoney where _pgInfoMoney.ObjectCode <> 30201 and _pgInfoMoney.Name2 = _pgProfitLoss.Name3)) end as InfoMoneyDestinationId_PG');
    Add('     , case when _pgProfitLoss.ObjectCode < 20000 then null ' +
      ' else case when InfoMoneyDestinationId_PG is not null then null else (select max (isnull (_pgInfoMoney.Id3_Postgres, 0)) from dba._pgInfoMoney where _pgInfoMoney.Name3 = _pgProfitLoss.Name3) end end as InfoMoneyId_PG');
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('inName').Value :=
        FieldByName('ObjectName').AsString;
      toStoredProc.Params.ParamByName('inProfitLossGroupId').Value :=
        FieldByName('ProfitLossGroupId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inProfitLossDirectionId').Value :=
        FieldByName('ProfitLossDirectionId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inInfoMoneyDestinationId').Value :=
        FieldByName('InfoMoneyDestinationId_PG').AsInteger;
      toStoredProc.Params.ParamByName('inInfoMoneyId').Value :=
        FieldByName('InfoMoneyId_PG').AsInteger;
      ...
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba._pgProfitLoss set Id3_Postgres=' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ' where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
var
  ...
  Year, Month, Day: Word;
begin
  if (not cbInsertHistoryCost.Checked) or (not cbInsertHistoryCost.Enabled) then
    exit;
  //
  ...
  begin
    ...
    begin
      if calcStartDate = StrToDate(StartDateCompleteEdit.Text) then
        Add('          select cast(' + FormatToDateServer_notNULL(calcStartDate)
          + ' as date) as StartDate, cast(' + FormatToDateServer_notNULL
          (calcEndDate) + ' as date) as EndDate')
      else
        Add('union all select cast(' + FormatToDateServer_notNULL(calcStartDate)
          + ' as date) as StartDate, cast(' + FormatToDateServer_notNULL
          (calcEndDate) + ' as date) as EndDate');
      //
    ...
    begin
      ...
      //
      toStoredProc.Params.ParamByName('inStartDate').Value :=
        FieldByName('StartDate').AsDateTime;
      toStoredProc.Params.ParamByName('inEndDate').Value :=
...
begin
  if (not cbCompleteIncome.Checked) or (not cbCompleteIncome.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('from dba.Bill');
    Add('where Bill.BillDate between ' + FormatToDateServer_notNULL
      (StrToDate(StartDateCompleteEdit.Text)) + ' and ' +
      FormatToDateServer_notNULL(StrToDate(EndDateCompleteEdit.Text)) +
      '  and Id_Postgres is not null' +
      '  and Bill.BillKind=zc_bkIncomeToUnit()');
    Add('order by OperDate,ObjectId');
    Open;
    cbCompleteIncome.Caption := '1. (' + IntToStr(RecordCount) +
    ...
    begin
      ...
      begin
        toStoredProc.Params.ParamByName('inMovementId').Value :=
          FieldByName('Id_Postgres').AsInteger;
        if not myExecToStoredProc then; // exit;
      end;
      if cbComplete.Checked then
      begin
        toStoredProc_two.Params.ParamByName('inMovementId').Value :=
          FieldByName('Id_Postgres').AsInteger;
        toStoredProc_two.Params.ParamByName('inIsLastComplete').Value :=
          isLastComplete;
        if not myExecToStoredProc_two then; // exit;
      end;
      //
...
begin
  Result := 0;
  if (not cbIncome.Checked) or (not cbIncome.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('where Bill.BillDate between ' + FormatToDateServer_notNULL
      (StrToDate(StartDateEdit.Text)) + ' and ' + FormatToDateServer_notNULL
      (StrToDate(EndDateEdit.Text)) + '  and Bill.BillKind=zc_bkIncomeToUnit()'
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('ioId').Value :=
        FieldByName('Id_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inInvNumber').Value :=
        FieldByName('InvNumber').AsString;
      toStoredProc.Params.ParamByName('inOperDate').Value :=
        FieldByName('OperDate').AsDateTime;

      toStoredProc.Params.ParamByName('inOperDatePartner').Value :=
        FieldByName('OperDatePartner').AsDateTime;
      toStoredProc.Params.ParamByName('inInvNumberPartner').Value :=
        FieldByName('InvNumberPartner').AsString;

      if FieldByName('PriceWithVAT').AsInteger = FieldByName('zc_rvYes').AsInteger
      then
        toStoredProc.Params.ParamByName('inPriceWithVAT').Value := true
      else
        toStoredProc.Params.ParamByName('inPriceWithVAT').Value := false;
      toStoredProc.Params.ParamByName('inVATPercent').Value :=
        FieldByName('VATPercent').AsFloat;
      toStoredProc.Params.ParamByName('inChangePercent').Value :=
        FieldByName('ChangePercent').AsFloat;

      toStoredProc.Params.ParamByName('inFromId').Value :=
        FieldByName('FromId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inToId').Value :=
        FieldByName('ToId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inPaidKindId').Value :=
        FieldByName('PaidKindId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inContractId').Value :=
        FieldByName('ContractId').AsInteger;
      toStoredProc.Params.ParamByName('inCarId').Value := FieldByName('CarId')
        .AsInteger;
      toStoredProc.Params.ParamByName('inPersonalDriverId').Value :=
        FieldByName('PersonalDriverId').AsInteger;
      toStoredProc.Params.ParamByName('inPersonalPackerId').Value :=
        FieldByName('PersonalPackerId').AsInteger;

      if not myExecToStoredProc then; // exit;
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Bill set Id_Postgres=zf_ChangeIntToNull(' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ') where Id = ' + FieldByName('ObjectId').AsString + ' and 0<>' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value));
      //
      Next;
      Application.ProcessMessages;
...
begin
  ...
  begin
    ...
    // Тара + СЫР + ХЛЕБ + С-ПЕРЕРАБОТКА + ТУШЕНКА
    Add('where Bill.BillDate between ' + FormatToDateServer_notNULL
      (StrToDate(StartDateEdit.Text)) + ' and ' + FormatToDateServer_notNULL
      (StrToDate(EndDateEdit.Text)) + '  and Bill.BillKind=zc_bkIncomeToUnit()'
      + '  and UnitFrom.PersonalId_Postgres is null' +
    ...
    Add('order by ObjectId');
    Open;
    cbIncome.Caption := '1.1. (' + IntToStr(SaveCount) + ')(' +
      IntToStr(RecordCount) + ') Приход от поставщика';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('ioId').Value :=
        FieldByName('Id_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inMovementId').Value :=
        FieldByName('MovementId_Postgres').AsString;
      toStoredProc.Params.ParamByName('inGoodsId').Value :=
        FieldByName('GoodsId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inAmount').Value :=
        FieldByName('Amount').AsFloat;
      toStoredProc.Params.ParamByName('inAmountPartner').Value :=
        FieldByName('AmountPartner').AsFloat;
      toStoredProc.Params.ParamByName('inAmountPacker').Value :=
        FieldByName('AmountPacker').AsFloat;
      toStoredProc.Params.ParamByName('inPrice').Value :=
        FieldByName('Price').AsFloat;
      toStoredProc.Params.ParamByName('inCountForPrice').Value :=
        FieldByName('CountForPrice').AsFloat;
      toStoredProc.Params.ParamByName('inLiveWeight').Value :=
        FieldByName('LiveWeight').AsFloat;
      toStoredProc.Params.ParamByName('inHeadCount').Value :=
        FieldByName('HeadCount').AsFloat;
      toStoredProc.Params.ParamByName('inPartionGoods').Value :=
        FieldByName('PartionGoods').AsString;
      toStoredProc.Params.ParamByName('inGoodsKindId').Value :=
        FieldByName('GoodsKindId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inAssetId').Value :=
        FieldByName('AssetId_Postgres').AsInteger;
      // toStoredProc.Params.ParamByName('inSession').Value:=fGetSession;
      if not myExecToStoredProc then; // exit;
      //
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery
          ('update dba.BillItems set Id_Postgres=zf_ChangeIntToNull(' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ') where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
begin
  Result := 0;
  if (not cbIncomePacker.Checked) or (not cbIncomePacker.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Open;
    Result := RecordCount;
    cbIncomePacker.Caption := '1.2. (' + IntToStr(RecordCount) +
      ') Приход от заготовителя';
    //
    fStop := (cbOnlyOpen.Checked) and (not cbOnlyOpenMI.Checked);
    if cbOnlyOpen.Checked then
      exit;
    ...
    begin
      ...
      if (1 = 1) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery('update dba.Bill set Id_Postgres=zf_ChangeIntToNull(' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ') where Id = ' + FieldByName('ObjectId').AsString +
          ' and isnull(Id_Postgres,0)<>' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) + ' and 0<>' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value));
      //
      Next;
      Application.ProcessMessages;
...
begin
  ...
  begin
    ...
    Add('order by ObjectId');
    Open;
    cbIncomePacker.Caption := '1.2. (' + IntToStr(SaveCount) + ')(' +
      IntToStr(RecordCount) + ') Приход от заготовителя';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
...
begin
  if (not cbCompleteSend.Checked) or (not cbCompleteSend.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('     left outer join dba._pgPersonal as pgPersonalTo on pgPersonalTo.Id=UnitTo.PersonalId_Postgres'
      ...
      + '                                                      and UnitTo.ParentId=4137');
    // МО ЛИЦА-ВСЕ
    Add('where Bill.BillDate between ' + FormatToDateServer_notNULL
      (StrToDate(StartDateCompleteEdit.Text)) + ' and ' +
      FormatToDateServer_notNULL(StrToDate(EndDateCompleteEdit.Text)) +
      '  and Bill.BillKind in (zc_bkSendUnitToUnit())' +
      '  and (UnitFrom.pgUnitId is not null or UnitFrom.ParentId=4137)'
      // МО ЛИЦА-ВСЕ
      + '  and (UnitTo.pgUnitId is not null or UnitTo.ParentId=4137)'
      // МО ЛИЦА-ВСЕ
      + '  and (pgUnitFrom.Id_Postgres_Branch is null or UnitFrom.ParentId=4137)'
      // МО ЛИЦА-ВСЕ
      + '  and (pgUnitTo.Id_Postgres_Branch is null or UnitTo.ParentId=4137)'
      // МО ЛИЦА-ВСЕ
      );
    ...
    Add('order by OperDate,ObjectId');
    Open;
    cbCompleteSend.Caption := '2.1. (' + IntToStr(RecordCount) +
      ') Перемещение';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
...
begin
  Result := 0;
  if (not cbSendUnit.Checked) or (not cbSendUnit.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Open;
    Result := RecordCount;
    cbSendUnit.Caption := '2.1. (' + IntToStr(RecordCount) +
      ') Перемещение с подразделениями';
    //
    fStop := (cbOnlyOpen.Checked) and (not cbOnlyOpenMI.Checked);
    if cbOnlyOpen.Checked then
      exit;
...
begin
  ...
  begin
    ...
    Add('order by ObjectId');
    Open;
    cbSendUnit.Caption := '2.1. (' + IntToStr(SaveCount) + ')(' +
      IntToStr(RecordCount) + ') Перемещение с подразделениями';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('inAmount').Value :=
        FieldByName('Amount').AsFloat;
      toStoredProc.Params.ParamByName('inCount').Value :=
        FieldByName('inCount').AsFloat;
      toStoredProc.Params.ParamByName('inHeadCount').Value :=
        FieldByName('HeadCount').AsFloat;
      toStoredProc.Params.ParamByName('inPartionGoods').Value :=
        FieldByName('PartionGoods').AsString;
...
begin
  if (not cbCompleteSendOnPrice.Checked) or (not cbCompleteSendOnPrice.Enabled)
  then
    exit;
  //
  ...
  begin
    ...
    Add('     left outer join dba._pgPersonal as pgPersonalTo on pgPersonalTo.Id=UnitTo.PersonalId_Postgres'
      ...
      + '                                                      and UnitTo.ParentId=4137');
    // МО ЛИЦА-ВСЕ
    Add('zzz where Bill.BillDate between ' + FormatToDateServer_notNULL
      (StrToDate(StartDateCompleteEdit.Text)) + ' and ' +
      FormatToDateServer_notNULL(StrToDate(EndDateCompleteEdit.Text)) +
      '  and Bill.BillKind in (zc_bkSendUnitToUnit())' + '  and  ((' +
      ...
      '  and pgUnitTo.Id_Postgres_Branch is null' + '          )' + '     or ('
      + '       ))');
    Add('order by OperDate,ObjectId');
    Open;
    cbCompleteSendOnPrice.Caption := '2.2. (' + IntToStr(RecordCount) +
      ') Перемещение по цене';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
...
begin
  Result := 0;
  if (not cbSendPersonal.Checked) or (not cbSendPersonal.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Open;
    Result := RecordCount;
    cbSendPersonal.Caption := '2.2. (' + IntToStr(RecordCount) +
      ') Перемещение с экспедиторами';
    //
    fStop := (cbOnlyOpen.Checked) and (not cbOnlyOpenMI.Checked);
    if cbOnlyOpen.Checked then
      exit;
...
begin
  ...
  begin
    ...
    Add('order by ObjectId');
    Open;
    cbSendPersonal.Caption := '2.2. (' + IntToStr(SaveCount) + ')(' +
      IntToStr(RecordCount) + ') Перемещение с экспедиторами';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
...
begin
  Result := 0;
  if (not cbSendUnitBranch.Checked) or (not cbSendUnitBranch.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('where Bill.BillDate between ' + FormatToDateServer_notNULL
      ...
      + '  and not (' +
      '      (UnitFrom.pgUnitId is not null or UnitFrom.ParentId=4137)'
      // МО ЛИЦА-ВСЕ
      + '  and (UnitTo.pgUnitId is not null or UnitTo.ParentId=4137)'
      // МО ЛИЦА-ВСЕ
      + '  and (pgUnitFrom.Id_Postgres_Branch is null or UnitFrom.ParentId=4137)'
      ...
      + '  and not (' + '      isnull(UnitFrom.ParentId,0)<>4137' // МО ЛИЦА-ВСЕ
      + '  and isnull(UnitTo.ParentId,0)<>4137' // МО ЛИЦА-ВСЕ
      ...
      '     and pgUnitTo.Id_Postgres_Branch is null))' + '          )' +
      '  and not (' + '       ((UnitTo.pgUnitId is not null' +
    ...
    Open;
    Result := RecordCount;
    cbSendUnitBranch.Caption := '2.3. (' + IntToStr(RecordCount) +
      ') Перемещение с филиалами';
    //
    fStop := (cbOnlyOpen.Checked) and (not cbOnlyOpenMI.Checked);
    if cbOnlyOpen.Checked then
      exit;
...
begin
  ...
  begin
    ...
    Add('order by ObjectId');
    Open;
    cbSendUnitBranch.Caption := '2.3. (' + IntToStr(SaveCount) + ')(' +
      IntToStr(RecordCount) + ') Перемещение с филиалами';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
...
begin
  Result := 0;
  if (not cbSale.Checked) or (not cbSale.Enabled) then
    exit;
  //
  myEnabledCB(cbSale);
  //
  ...
  begin
    Close;
    Clear;
    Add('call dba._pgSelect_Bill_Sale(' + FormatToDateServer_notNULL
      (StrToDate(StartDateEdit.Text)) + ',' + FormatToDateServer_notNULL
      (StrToDate(EndDateEdit.Text)) + ')');
    Open;
    Result := RecordCount;
    ...
    toStoredProc_two.OutputType := otResult;
    toStoredProc_two.Params.Clear;
    toStoredProc_two.Params.AddParam('inMovementId', ftInteger,
      ptInputOutput, 0);
    //
    while not EOF do
    begin
      ...
      toStoredProc.Params.ParamByName('inPersonalDriverId').Value :=
        FieldByName('PersonalDriverId').AsInteger;
      toStoredProc.Params.ParamByName('inPersonalId').Value :=
        FieldByName('PersonalId_Postgres').AsInteger;
      ...
      if not myExecToStoredProc then; // exit;
      //
      if ((1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0)) and
        (FieldByName('isFl').AsInteger <> FieldByName('zc_rvYes').AsInteger)
      then
        fExecSqFromQuery('update dba.Bill set Id_Postgres=zf_ChangeIntToNull(' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ') where Id = ' + FieldByName('ObjectId').AsString + ' and 0<>' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value));
      if ((1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0)) and
        (FieldByName('isFl').AsInteger = FieldByName('zc_rvYes').AsInteger) then
        fExecSqFromQuery('update dba.fBill set Id_Postgres=zf_ChangeIntToNull('
          + IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ') where Id = ' + FieldByName('ObjectId').AsString + ' and 0<>' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value));
      { }
...
begin
  ...
  begin
    ...
    Add('     , case when fBill.BillId_byLoad is not null or fBill_two.BillId_byLoad is not null or fBillItems.BillItemsId_byLoad is not null'
      ...
      + '                            then isnull(-fBillItems.OperCount,0)' +
      '                         else 0' + '                    end' +
      '            else Amount' + '       end as AmountPartner');
    Add('     , BillItems.OperPrice as Price');
    Add('     , 1 as CountForPrice');
    Add('     , BillItems.OperCount_sh as HeadCount');
    Add('     , BillItems.PartionStr_MB as PartionGoods');
    ...
    Add('     left outer join (select BillDate, BillNumber, max(fUnitFrom.Id_byLoad) as FromId, max(fUnitTo.Id_byLoad) as ToId'
      ...
      FormatToDateServer_notNULL(StrToDate(StartDateEdit.Text)) + ' and ' +
      FormatToDateServer_notNULL(StrToDate(EndDateEdit.Text) + 3) +
      '                      group by BillDate, BillNumber' +
      '                     ) as CheckBilNumber on CheckBilNumber.BillNumber = Bill.BillNumber and CheckBilNumber.BillDate = Bill.BillDate + isnull(_toolsView_Client_isChangeDate.addDay,0) and CheckBilNumber.ToId=Bill.ToId and Bill.MoneyKindId=zc_mkNal()');
    ...
    Add('     , zc_rvYes() as isFl');
    Add('     , case when GoodsProperty.Id is null then ' +
      FormatToVarCharServer_notNULL('ошибка товар-') +
      '+cast (Bill.BillNumber as TVarCharMedium)+' +
      FormatToVarCharServer_notNULL('-f') +
      '            when GoodsProperty_Detail.KindPackageId is null then ' +
      FormatToVarCharServer_notNULL('ошибка вид-') +
      '+cast (Bill.BillNumber as TVarCharMedium)+' +
      FormatToVarCharServer_notNULL('-f') + '            else ' +
      FormatToVarCharServer_notNULL('') + ' end as errInvNumber');
    Add('     , zc_rvYes() as zc_rvYes');
    Add('     , BillItems.Id_Postgres as Id_Postgres');
    Add('from dba.fBill as Bill');
    Add('     left outer join dba.Bill AS Bill_find on Bill_find.Id = Bill.BillId_byLoad');
    ...
    Add('where Bill.BillDate between ' + FormatToDateServer_notNULL
      ...
      + '  and BillItems.Id is not null' + '  and Bill_find_check.Id is null');
    Add('order by 2,3,1');
    Open;
    cbSale.Caption := '3.1. (' + IntToStr(SaveCount) + ')(' +
      IntToStr(RecordCount) + ') Продажа покупателю';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
    ...
    begin
      ...
      if ((1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0)) and
        (FieldByName('isFl').AsInteger = FieldByName('zc_rvYes').AsInteger) then
        fExecSqFromQuery
          ('update dba.fBillItems set Id_Postgres=zf_ChangeIntToNull(' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ') where Id = ' + FieldByName('ObjectId').AsString);
      //
      if (FieldByName('errInvNumber').AsString <> '') then
      begin
        toStoredProc_two.Params.ParamByName('inId').Value :=
          FieldByName('MovementId_Postgres').AsInteger;
        toStoredProc_two.Params.ParamByName('inInvNumber').Value :=
          FieldByName('errInvNumber').AsString;
        if not myExecToStoredProc_two then;
      end;
      //
      Next;
...
begin
  if (not cbCompleteProductionUnion.Checked) or
    (not cbCompleteProductionUnion.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('order by OperDate,ObjectId');
    Open;
    cbCompleteProductionUnion.Caption := '4.1. (' + IntToStr(RecordCount) +
      ') Производство смешивание';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
...
begin
  Result := 0;
  if (not cbProductionUnion.Checked) or (not cbProductionUnion.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Open;
    Result := RecordCount;
    cbProductionUnion.Caption := '4.1. (' + IntToStr(RecordCount) +
      ') Производство смешивание';
    //
    fStop := (cbOnlyOpen.Checked) and (not cbOnlyOpenMI.Checked);
    if cbOnlyOpen.Checked then
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------
function TMainForm.pLoadDocumentItem_ProductionUnionMaster
  (SaveCount: Integer): Integer;
begin
  Result := 0;
  if (not cbProductionUnion.Checked) or (not cbProductionUnion.Enabled) then
  ...
  begin
    ...
    Open;
    Result := RecordCount;
    cbProductionUnion.Caption := '4.1. (' + IntToStr(SaveCount) + ')(' +
      IntToStr(RecordCount) + ') Производство смешивание';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
    ...
    begin
      ...
      then
        toStoredProc.Params.ParamByName('inPartionClose').Value := true
      else
        toStoredProc.Params.ParamByName('inPartionClose').Value := false;

      toStoredProc.Params.ParamByName('inCount').Value :=
        FieldByName('shCount').AsFloat;
      toStoredProc.Params.ParamByName('inRealWeight').Value :=
        FieldByName('RealWeight').AsFloat;
      toStoredProc.Params.ParamByName('inCuterCount').Value :=
        FieldByName('CuterCount').AsFloat;
      toStoredProc.Params.ParamByName('inPartionGoods').Value :=
        FieldByName('PartionGoods').AsString;
      toStoredProc.Params.ParamByName('inComment').Value :=
        FieldByName('OperComment').AsString;
      toStoredProc.Params.ParamByName('inGoodsKindId').Value :=
        FieldByName('GoodsKindId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inReceiptId').Value :=
        FieldByName('ReceiptId_Postgres').AsInteger;
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------
procedure TMainForm.pLoadDocumentItem_ProductionUnionChild(SaveCount1,
  SaveCount2: Integer);
begin
  if (not cbProductionUnion.Checked) or (not cbProductionUnion.Enabled) then
    exit;
  ...
  begin
    ...
    Add('order by ObjectId');
    Open;
    cbProductionUnion.Caption := '4.1. (' + IntToStr(SaveCount1) + ')(' +
      IntToStr(SaveCount2) + ')(' + IntToStr(RecordCount) +
      ') Производство смешивание';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
    ...
    begin
      ...
      toStoredProc.Params.ParamByName('inParentId').Value :=
        FieldByName('ParentId_Postgres').AsInteger;
      toStoredProc.Params.ParamByName('inAmountReceipt').Value :=
        FieldByName('AmountReceipt').AsFloat;

      if FieldByName('PartionGoodsDate').AsDateTime < StrToDate('01.01.1900')
      then
        toStoredProc.Params.ParamByName('inPartionGoodsDate').Value :=
          StrToDate('01.01.1900')
      else
        toStoredProc.Params.ParamByName('inPartionGoodsDate').Value :=
          FieldByName('PartionGoodsDate').AsDateTime;
      ...
      if (1 = 0) or (FieldByName('Id_Postgres').AsInteger = 0) then
        fExecSqFromQuery
          ('update dba.BillItemsReceipt set Id_Postgres=zf_ChangeIntToNull(' +
          IntToStr(toStoredProc.Params.ParamByName('ioId').Value) +
          ') where Id = ' + FieldByName('ObjectId').AsString);
      //
      Next;
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------
procedure TMainForm.pCompleteDocument_ProductionSeparate(isLastComplete
  : Boolean);
begin
  if (not cbCompleteProductionSeparate.Checked) or
    (not cbCompleteProductionSeparate.Enabled) then
    exit;
  //
  ...
  begin
    ...
    Add('order by OperDate,ObjectId');
    Open;
    cbCompleteProductionSeparate.Caption := '4.2. (' + IntToStr(RecordCount) +
      ') Производство разделение';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
...
begin
  Result := 0;
  if (not cbProductionSeparate.Checked) or (not cbProductionSeparate.Enabled)
  then
    exit;
  //
  ...
  begin
    ...
    Open;
    Result := RecordCount;
    cbProductionSeparate.Caption := '4.2. (' + IntToStr(RecordCount) +
      ') Производство разделение';
    //
    fStop := (cbOnlyOpen.Checked) and (not cbOnlyOpenMI.Checked);
    if cbOnlyOpen.Checked then
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------
function TMainForm.pLoadDocumentItem_ProductionSeparateMaster
  (SaveCount: Integer): Integer;
begin
  Result := 0;
  if (not cbProductionSeparate.Checked) or (not cbProductionSeparate.Enabled)
  ...
  begin
    ...
    Open;
    Result := RecordCount;
    cbProductionSeparate.Caption := '4.2. (' + IntToStr(SaveCount) + ')(' +
      IntToStr(RecordCount) + ') Производство разделение';
    //
    fStop := cbOnlyOpen.Checked;
    if cbOnlyOpen.Checked then
      exit;
    ...
    Gauge.MaxValue := RecordCount;
    //
    toStoredProc.StoredProcName :=
      'gpInsertUpdate_MI_ProductionSeparate_Master';
    toStoredProc.OutputType := otResult;
    toStoredProc.Params.Clear;
    toStoredProc.Params.AddParam('ioId', ftInteger, ptInputOutput, 0);
    toStoredProc.Params.AddParam('inMovementId', ftInteger, ptInput, '');
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------
procedure TMainForm.pLoadDocumentItem_ProductionSeparateChild(SaveCount1,
  SaveCount2: Integer);
begin
  if (not cbProductionSeparate.Checked) or (not cbProductionSeparate.Enabled)
  then
  ...
  begin
    ...
    Add('order by ObjectId');
    Open;
    cbProductionSeparate.Caption := '4.2. (' + IntToStr(SaveCount1) + ')(' +
      IntToStr(SaveCount2) + ')(' + IntToStr(RecordCount) +
      ') Производство разделение';
    //
    fStop := cbOnlyOpen.Checked;
