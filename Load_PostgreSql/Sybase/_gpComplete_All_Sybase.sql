...
BEGIN
     ...
     IF vbMovementDescId = zc_Movement_Inventory() AND EXTRACT ('HOUR' FROM CURRENT_TIMESTAMP) BETWEEN 9 AND 15
     THEN 
         -- IF EXTRACT ('MINUTES' FROM CURRENT_TIMESTAMP) BETWEEN 40 AND 59 OR EXTRACT ('HOUR' FROM CURRENT_TIMESTAMP) <> 8
         IF EXTRACT ('MINUTES' FROM CURRENT_TIMESTAMP) BETWEEN 25 AND 59 OR EXTRACT ('HOUR' FROM CURRENT_TIMESTAMP) <> 9
         THEN 
             RETURN; 
         END IF;
     END IF;
